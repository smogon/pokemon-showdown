...
exports.BattleScripts = {
	randomSeasonalTeam: function (side) {
		let seasonalPokemonList = [
			...
			'throh', 'sawk', 'gothitelle', 'beheeyem', 'bisharp', 'volbeat', 'illumise', 'spinda',
			'cacturne', 'infernape', 'lopunny', 'lucario', 'mienshao', 'pidgeot', 'fearow', 'dodrio',
			'aerodactyl', 'noctowl', 'crobat', 'xatu', 'skarmory', 'swellow', 'staraptor', 'honchkrow',
			'chatot', 'unfezant', 'sigilyph', 'braviary', 'mandibuzz', 'farfetchd', 'pelipper', 'altaria',
			'togekiss', 'swoobat', 'archeops', 'swanna', 'weavile', 'gallade', 'gardevoir', 'ludicolo',
			'snorlax', 'wobbuffet', 'meloetta', 'blissey', 'landorus', 'tornadus', 'golurk', 'bellossom',
			'lilligant', 'probopass', 'roserade', 'leavanny', 'zapdos', 'moltres', 'articuno', 'delibird',
		];
	...
	randomSeasonalWWTeam: function (side) {
		let seasonalPokemonList = [
			...
			'stantler', 'miltank', 'blissey', 'swalot', 'lunatone', 'castform', 'chimecho', 'glalie', 'walrein',
			'regice', 'jirachi', 'bronzong', 'chatot', 'abomasnow', 'weavile', 'togekiss', 'glaceon', 'probopass',
			'froslass', 'rotom-frost', 'uxie', 'mesprit', 'azelf', 'victini', 'vanilluxe', 'sawsbuck', 'beartic',
			'cryogonal', 'chandelure',
		];
		...
		for (let i = 0; i < 6; i++) {
			...
			if (template.id === 'chandelure' || template.randomBattleMoves && template.randomBattleMoves.some(move => bannedMoves.has(move))) {
				...
				if (template.id === 'chandelure') {
					...
					template.abilities['H'] = 'Flash Fire';
				}
			}
