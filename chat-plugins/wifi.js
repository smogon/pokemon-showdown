	...
	change(key, value, user) {
		...
		if (force) {
			...
			if (!this.winner) {
				...
				this.phase = 'ended';
				this.clearTimer();
				this.modlog(`${this.winner.name} won ${this.giver.name}'s giveaway for a "${this.prize}" (OT: ${this.ot} TID: ${this.tid} FC: ${this.fc})`);
...
class LotteryGiveaway extends Giveaway {
	...
	drawLottery() {
		...
		if (force) {
			...
			this.modlog(`${winnerNames} won ${this.giver.name}'s giveaway for "${this.prize}" (OT: ${this.ot} TID: ${this.tid} FC: ${this.fc})`);
