exports.commands = {
    ...
    questiongiveaway: function(target, room, user, connection, cmd) {
        ...
        if (!target) return this.sendReply('|html|/' + cmd + ' <i>User, Prize, Question, Answer</i> - Starts a question giveaway. The question is displayed 1 minute after announcing the prize and the user. Alternate/Multiple answers can be separated with a slash (/).');
        ...
        room.giveaway.timer = setTimeout(function() {
            if (thisroom.giveaway.members.length < 4 + room.giveaway.winnumber) {
                ...
                if (room.giveaway.winnumber > 1) {
                    ...
                    thisroom.add('|html|<center><div class = "broadcast-blue"><font size = 3><b>Drawing time!</b></font><br/><br/>' +
                        'Our lucky winner is <b>' + Users.get(random).name + '!</b> Congratulations!<br/>PM <b>' + Users.get(room.giveaway.user).name + '</b> to claim your prize!');
                    thisroom.update();
                }
            }
            delete room.giveaway;
    ...
    joinlottery: function(target, room, user, connection, cmd) {
        ...
        if (room.giveaway.members.map(userid).indexOf(user.userid) > -1) return this.sendReply('Either you or one of your alts have already joined the lottery.');
