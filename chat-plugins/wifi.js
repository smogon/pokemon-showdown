		...
		switch (toId(command)) {
			case 'help':
				...
				this.sendReplyBox(
					...
					'- guess <small>or</small> /guessanswer <em>answer</em> - Guesses the answer for a question giveaway<br />' +
					'- viewanswer <small>or</small> answer - Allows the giveaway starter or the person giving away the prize view the answer in a question giveaway.<br />' +
					'- grm <small>or</small> remind - Allows you to view the details of a giveaway.<br />' +
			...
			case 'quiz':
			case 'questiongiveaway':
			case 'qg':
			case 'question':
				if (!this.can('mute', null, room)) return false;
				if (room.giveaway) return this.sendReply('There is already a giveaway going on!');
				if (!toId(target)) return this.sendReply('|html|/giveaway ' + command + ' <i>User, Prize, Question, Answer</i> - Starts a question giveaway. The question is displayed 1 minute after announcing the prize and the user. Alternate/Multiple answers can be separated with a slash (/).');
				if (target.match(/,/g) && target.match(/,/g).length > 3) return this.sendReply('You can\'t add commas into any of the command\'s parameters or add extra parameters either.');
				target = target.split(',');
				...
				if (/[^a-z0-9/ ]+/ig.test(answer.toLowerCase())) return this.sendReply('The answer may only be a max of 3 words long and cannot have any special characters in it.');
				...
				if (answer.indexOf("/") > -1) {
					...
					for (var i = 0; i < answer.length; i++) {
						...
						if (!answer[i]) {
							...
							delete room.giveaway;
							return this.sendReply('An answer may only be a max of 3 words long and cannot have any special characters in it.');
						}
					}
			...
			case 'remind':
				...
				if (room.giveaway.type === 'question') {
					...
					if (room.giveaway.started)
						if (!this.canBroadcast()) return;
					return this.sendReply('|html|<center><div class = "broadcast-blue"><font size = 3><b>Giveaway time!</b></font><br/><font size = 1>Giveaway started by ' + Users.get(room.giveaway.starter).name + '</font><br/><br/>' +
					...
					this.sendReply('|html|<center><div class = "broadcast-blue"><font size = 3><b>Giveaway time!</b></font><br/><font size = 1>Giveaway started by ' + room.giveaway.starter + '</font><br/><br/>' +
						...
						'<button name = "send" value = "/giveaway join"><font size = 1><b>Join</b></font></button> <button name = "send" value = "/giveaway end"><font size = 1><b>Leave</b></font></button><br/><br/><font size = 1><b><u>Note:</u> Please do not join if you don\'t have a 3DS and a copy of Pokémon X, Y, ΩR, or αS');
				}
				break;
			...
			case 'changeanswer':
				...
				if (!toId(target)) return this.sendReply("|html|/changeanswer <i>new answer</i> - Allows the starter of the giveaway to change the answer.  Alternate/Multiple answers can be separated with a slash (/).");
				if (/[^a-z0-9/ ]+/ig.test(target.toLowerCase())) return this.sendReply('The answer may only be a max of 3 words long and cannot have any special characters in it.');
				if (target.indexOf("/") > -1) {
					target = target.split("/");
			...
			case 'guess':
				if (!room.giveaway) return this.sendReply('There is no giveaway going on at the moment.');
				if (Users.get(room.giveaway.user).userid === user.userid || Users.get(room.giveaway.user).getAlts().map(toId).indexOf(user.userid) > -1) return this.sendReply("You cannot answer the question when you're the one who's giving away the prize!");
				if (Users.get(room.giveaway.starter).userid === user.userid || Users.get(room.giveaway.starter).getAlts().map(toId).indexOf(user.userid) > -1) return this.sendReply("You cannot answer the question when you're the one who started the giveaway!");
				if (room.giveaway.type !== 'question') return this.sendReply('This is not a question giveaway. There are no questions involved.');
				...
				if (typeof room.giveaway.answer === 'object' ? room.giveaway.answer.map(toId).indexOf(toId(target)) === -1 : toId(target) !== toId(room.giveaway.answer)) {
					...
					room.add('|html|<div class = "broadcast-blue"><center><b>' + user.name + '</b> guessed the correct answer, which was "<b>' + target + '</b>". Congratulations!<br/>' +
			...
			case 'lg':
			case 'lotterygiveaway':
			case 'lottery':
				if (!this.can('mute', null, room)) return false;
				if (room.giveaway) return this.sendReply('There is already a giveaway going on!');
				if (!target) return this.sendReply('|html|/' + cmd + ' <i>User, Prize, Number of Winners (optional)</i> - Starts a lottery giveaway in the room. The number of winners will be set to 1 by default.');
				if (target.match(/,/g) && target.match(/,/g).length > 2) return this.sendReply('You can\'t add commas into any of the command\'s parameters or add extra parameters either.');
				target = target.split(',');
					...
					if (thisroom.giveaway.members.length < 4 + room.giveaway.winnumber) {
						...
						if (room.giveaway.winnumber > 1) {
							...
							thisroom.add('|html|<center><div class = "broadcast-blue"><font size = 3><b>Drawing time!</b></font><br/><br/>' +
								'Our lucky winners are <b>' + format + '!</b> Congratulations!<br/>PM <b>' + Users.get(room.giveaway.user).name + '</b> to claim your prize!');
							thisroom.update();
						} else {
							var randomuser = room.giveaway.members[Math.floor(Math.random() * room.giveaway.members.length)];
							thisroom.add('|html|<center><div class = "broadcast-blue"><font size = 3><b>Drawing time!</b></font><br/><br/>' +
								'Our lucky winner is <b>' + Users.get().name + '!</b> Congratulations!<br/>PM <b>' + Users.get(room.giveaway.user).name + '</b> to claim your prize!');
							thisroom.update();
						}
					}
					delete room.giveaway;
			...
			case 'joinlottery':
				...
				if (Users.get(room.giveaway.user) === user.userid) return this.sendReply("You cannot join the lottery when you're the one who's giving away the prize!");

				if (room.giveaway.members.map(usersToNames).indexOf(user.userid) > -1) return this.sendReply('Either you or one of your alts have already joined the lottery.');
				for (var i = 0; i < room.giveaway.members.length; i++)
					if (user.getAlts().map(toId).indexOf(toId(room.giveaway.members[i])) > -1) return this.sendReply('Either you or one of your alts have already joined the lottery.');
			...
			case 'leavelottery':
				...
				break;
			default:
				this.sendReply("'" + command + "' is not a valid giveaway command.");
				this.parse('/giveaway help');
		}
	},
