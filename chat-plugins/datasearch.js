	...
	for (const alts of searches) {
		...
		for (let mon in dex) {
			...
			if (alts.tiers && Object.keys(alts.tiers).length) {
				...
				if (alts.tiers.LC && !dex[mon].prevo && dex[mon].evos.some(evo => mod.getTemplate(evo).gen <= dex[mon].gen) && !Dex.formats.gen7lc.banlist.includes(dex[mon].species) && !Dex.formats.gen7lc.banlist.includes(dex[mon].species + "-Base") && tier !== 'NFE') continue;
			}
