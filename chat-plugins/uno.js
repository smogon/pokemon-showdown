...
class UNOgamePlayer extends Rooms.RoomGamePlayer {
	...
	sendDisplay() {
		...
		this.sendRoom(
			...
			`${this.game.currentPlayer === this.userid ? `<div style="text-align: center">${draw} <span style="padding-left: 10px;"> ${pass} </span><span style="padding-left: 10px;"></span> ${uno}</div>` : ""}</div>`
		);
	}
}
