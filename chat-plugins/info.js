    ...
    calc: function(target, room, user, connection, cmd, message) {
        ...
        for (var i = 0; i < defenderDetails.length; i++) {
            ...
            if (['assaultvest', 'av'].indexOf(toId(detail)) > -1) {
                if ((Tools.getMove(attacker.move) && Tools.getMove(attacker.move).category === 'Special') && ['psyshock', 'psystrike', 'secretsword'].indexOf(attacker.move) === -1) {
                    defender.multiplier *= 1.5;
                }
            }
            if (['dryskin', 'stormdrain', 'waterabsorb'].indexOf(toId(detail)) > -1 && Tools.getMove(attacker.move).type === 'Water') {
                finalMultiplier = 0;
            }
            if (['flashfire'].indexOf(toId(detail)) > -1 && Tools.getMove(attacker.move).type === 'Fire') {
                finalMultiplier = 0;
            }
            if (['levitate', 'airballoon'].indexOf(toId(detail)) > -1 && Tools.getMove(attacker.move).type === 'Ground') {
                finalMultiplier = 0;
            }
            if (['sapsipper'].indexOf(toId(detail)) > -1 && Tools.getMove(attacker.move).type === 'Grass') {
                finalMultiplier = 0;
            }
            if (['lightningrod', 'voltabsorb'].indexOf(toId(detail)) > -1 && Tools.getMove(attacker.move).type === 'Electric') {
                finalMultiplier = 0;
            }
            if (toId(detail) === 'wonderguard') {
                defender.wonderguard = true;
            }
            if (['heatproof'].indexOf(toId(detail)) > -1 && Tools.getMove(attacker.move).type === 'Fire') {
        ...
        function isStab(move, user) {
            return Tools.getTemplate(user).types.indexOf(Tools.getMove(move).type) > -1;
        }

        function effectiveFactor(move, target) {
            var attackType = Tools.getMove(move).type;
            var types = Tools.getTemplate(target).types
            if (types.length === 2) {
                return Tools.data.TypeChart[types[0]].damageTaken[attackType] * Tools.data.TypeChart[types[1]].damageTaken[attackType];
            }
            return Tools.data.TypeChart[types[0]].damageTaken[attackType];
        }
        var effectiveness = effectiveFactor(attacker.move, defender.id);
        var stabMove = isStab(attacker.move, attacker.id);
        if (attacker.specialability && Tools.getMove(attacker.move).type === 'Normal') {
        ...
        if (Tools.getMove(attacker.move).secondary && attacker.ability === 'sheerforce') {
            basePowerMultiplier *= 1.3;
        }
        if (attacker.toughclaws && Tools.getMove(attacker.move).flags && Tools.getMove(attacker.move).flags.contact) {
            basePowerMultiplier *= 1.3;
        }
        if (attacker.strongjaw && Tools.getMove(attacker.move).flags && Tools.getMove(attacker.move).flags.bite) {
            basePowerMultiplier *= 1.5;
        }
        if (attacker.megalauncher && Tools.getMove(attacker.move).flags && Tools.getMove(attacker.move).flags.pulse) {
            basePowerMultiplier *= 1.5;
        }
        if (attacker.expertBelt && effectiveness > 2) {
            finalMultiplier *= 1.2;
        }
        if (attacker.reckless && (Tools.getMove(attacker.move).recoil || Tools.getMove(attacker.move).hasCustomRecoil)) {
            basePowerMultiplier *= 1.2
        }
        if (attacker.technician && Tools.getMove(attacker.move).basePower <= 60) {
            basePowerMultiplier *= 1.5;
        }
        var tarLevel = defender.level || 100;
        var hpEvs = defender.hpev || 0;
        hpEvs = parseInt(hpEvs);
        var hp = ~~((Tools.getTemplate(defender.id).baseStats.hp * 2 + (hpEvs / 4) + 31) * (tarLevel / 100) + parseInt(tarLevel) + 10);

        function calculateStat(mon, stat, boost) {
            var baseStat = Tools.getTemplate(mon.id).baseStats[stat];
            var BSmodifier = 1
            if (boost && boost === '+') {
                BSmodifier = 1.1;
            }
        ...
        var atk = pokeRound(calculateStat(attacker, convertStat(Tools.getMove(attacker.move).category), attacker.nature) * atkBoost);
        var atkLevel = attacker.level || 100;
        var moveBP = pokeRound(Tools.getMove(attacker.move).basePower * basePowerMultiplier);
        ...
        if (extras) {
            ...
            if (extras.indexOf('vgc') > -1 || extras.indexOf('doubles') > -1) {
                if (toId(Tools.getMove(attacker.move).target.slice(0, 3)) === 'all') baseDamage = ~~(baseDamage * 0.75)
            }
            if (Tools.getMove(attacker.move).type === 'Fire' && extras.indexOf('rain') > -1) {
                baseDamage = ~~(baseDamage * 0.5)
            }
            if (Tools.getMove(attacker.move).type === 'Water' && extras.indexOf('rain') > -1) {
                baseDamage = ~~(baseDamage * 1.5)
            }
            if (Tools.getMove(attacker.move).type === 'Fire' && extras.indexOf('sun') > -1) {
                baseDamage = ~~(baseDamage * 1.5)
            }
            if (Tools.getMove(attacker.move).type === 'Water' && extras.indexOf('sun') > -1) {
                baseDamage = ~~(baseDamage * 0.5)
            }
        }
        var damageRolls = [];
        ...
        if (Tools.getMove(attacker.move).category === 'Status') finalMultiplier = 0;
        for (var i = 0; i < damageRolls.length; i++) {
