...
var luxmusic = Rooms.get('luxmusic');
if (luxmusic && !luxmusic.plugin) {
	luxmusic.plugin = artistOfTheDay;
}

var commands = {
	start: function (target, room, user) {
		if (room.id !== 'luxmusic' || !room.chatRoomData || !this.can('mute', null, room)) return false;
	...
	nom: function (target, room, user) {
		if (room.id !== 'luxmusic' || !room.chatRoomData || !target) return false;
	...
	viewnoms: function (target, room, user) {
		if (room.id !== 'luxmusic' || !room.chatRoomData) return false;
	...
	removenom: function (target, room, user) {
		if (room.id !== 'luxmusic' || !room.chatRoomData || !target || !this.can('mute', null, room)) return false;
	...
	'': function (target, room) {
		if (room.id !== 'luxmusic' || !room.chatRoomData || !this.canBroadcast()) return false;
