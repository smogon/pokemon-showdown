	...
	banword: {
		...
		delete: function (target, room, user) {
			...
			if (words.length > 1) {
				...
				this.sendReply(`Banned phrases succesfully deleted.${room.banwords.length ? `The list is currently: ${room.banwords.join(', ')}` : ``}`);
			} else {
				this.privateModAction(`(The banword '${words[0]}' was removed by ${user.name}.)`);
				this.modlog('UNBANWORD', null, words[0]);
				this.sendReply(`Banned phrase succesfully deleted.${room.banwords.length ? `The list is currently: ${room.banwords.join(', ')}` : ``}`);
			}
