...
class OtdHandler {
	...
	addNomination(user, nomination) {
		...
		if (this.winners.slice(-30).some(entry => toNominationId(entry[this.keys[0]]) === id)) return user.sendTo(this.room, `This ${this.name.toLowerCase()} has already been ${this.id} in the past month.`);
