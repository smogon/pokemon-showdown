...
var commands = exports.commands = {
	...
	dice: function(target, room, user) {
		...
		if (custom) {
			...
			return this.sendReplyBox('Randomly number (1-'+target+'): '+rand);
		}
		else {
			var rand = Math.floor((6)*Math.random()) + 1;
			return this.sendReplyBox('Randomly number: '+rand);
		}
	},
	...
	help: function(target, room, user) {
		...
		if (target === 'all' || target === 'dice' || target === 'roll') {
			matched = true;
			this.sendReply('/dice [optional max number] - Random picks a number between 1 and 6, or between 1 and the number you choose.');
		}
