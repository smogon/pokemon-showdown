...
export const Formats: import("../sim/dex-formats").FormatList = [
	...
	},
	{
		...
		checkCanLearn(move, species, lsetData, set) {
			...
			if (this.checkCanLearn(move, (set as any).crossSpecies))
				return problem;
			return null;
		},
	...
	},
	{
		...
		onValidateSet(set, format, setHas, teamHas) {
			...
			if (
				...
				return [
					`${species.name} is required to hold the ${
						species.requiredItems![0]
					}.`,
				];
			}
		},
		...
		onBegin() {
			for (const pokemon of this.getAllPokemon()) {
				if (pokemon.pokeball.includes("0")) {
					...
					(pokemon as any).pokeball = this.toID(
						pokemon.pokeball.split("0")[0]
					);
				}
			}
		},
		...
		actions: {
			switchIn(pokemon, pos, sourceEffect, isDrag) {
				...
				if (unfaintedActive) {
					...
					let switchCopyFlag: "copyvolatile" | "shedtail" | boolean =
						false;
	...
	},
	{
		...
		actions: {
			modifyDamage(baseDamage, pokemon, target, move, suppressMessages) {
				...
				if (type !== "???") {
					...
					if (
						(pokemon.terastallized || pokemon.m.thirdType) === "Stellar"
					) {
		...
		pokemon: {
			...
			runEffectiveness(move) {
				if (
					(this.terastallized || this.m.thirdType) &&
					move.type === "Stellar"
				)
					return 1;
				let totalTypeMod = 0;
			...
			for (const trademark of teamHas.trademarks.keys()) {
				if (teamHas.trademarks.get(trademark) > 1) {
					problems.push(
						...
						`(You have ${teamHas.trademarks.get(
							trademark
						)} Pok\u00e9mon with ${trademark} as a Trademark.)`
					);
				}
			}
			return problems;
	...
	},
	{
		...
		onBegin() {
			this.add(
				`raw|<div class='broadcast-green'><b>Need help with all of the new moves, abilities, and wacky stuff?<br />Then make sure to check out the <a href="https://tinyurl.com/MonHunShow" target="_blank">MHS Wikia</a> or use /dt!</b></div>`
			);
			this.add("-message", `Welcome to Monster Hunter Showdown!`);
			this.add(
				"-message",
				`Coded by KnivesMK, it reimagines the Monsters from Monster Hunter as Competitive Pokemon!`
			);
			...
			this.add(
				"-message",
				`Special thanks to EggEggEgg for most of the Sprites, and Kestis for the Icons!`
			);
			this.add(
				"-message",
				`(Monster Hunter Showdown is in no way officially affiliated with the Monster Hunter games or CAPCOM and is purely a fan project.)`
			);
		},
	...
	},
	{
		...
		onBegin() {
			this.add(
				`raw|<div class='broadcast-green'><b>Need help with all of the new moves, abilities, and wacky sets?<br />Then make sure to check out the <a href="https://www.smogon.com/forums/threads/chatbats.3760234/post-10645803" target="_blank">ChatBats Compendium</a> or use /dt!</b></div>`
			);
			this.add(`raw|Welcome to ChatBats!`);
			this.add(
				`raw|ChatBats is a Random Battles format created by the Pet Mods room here on Showdown!`
			);
			this.add(
				`raw|If you want to help create new sets, we will host events periodically in the Pet Mods room!`
			);
			this.add(
				`raw|Anyone who is there can help create a new set for a random mon, changing moves, abilities, stats, and even custom formes.`
			);
		},
	},
	{
	...
	},
	{
		...
		onBegin() {
			this.add(
				`raw|<div class="broadcast-blue">The Ladder Achievements Project for Force of the Fallen Random Roulette has officially started! Check <a href="https://www.smogon.com/forums/threads/3773734">this thread</a> to see what the project is all about and the challenges for this cycle!</div>`
			);
		},
	},
		...
		onSwitchIn(pokemon) {
			if (pokemon.m.revivedByMonkeysPaw) {
				pokemon.addVolatile(
					"slowstart",
					null,
					this.dex.conditions.get("monkeypaw")
				);
			}
	...
	},
	{
		...
		onBegin() {
			for (const pokemon of this.getAllPokemon()) {
				if (!(pokemon.set as any).teraCaptain)
	...
	},
	{
		...
		onValidateTeam(team) {
			...
			if (restrictedSpecies.length > 4) {
				problems.push(
					`You can only use up to four restricted Pok\u00e9mon; you have: ${restrictedSpecies.join(
						", "
					)}`
				);
			}
			if (nonRestrictedSpecies.length < 2) {
				problems.push(
					`You must use at least two non-restricted Pok\u00e9mon; you have: ${
						nonRestrictedSpecies.length ? nonRestrictedSpecies[0] : "None"
					}.`
				);
			}
			return problems;
