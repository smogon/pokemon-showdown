...
export const Formats: import('../sim/dex-formats').FormatList = [
	...
	// ── Fork sections ───────────────────────────────────────────────────────
	// The formats for these five sections live in config/custom-formats.ts and
	// are merged in at load by mergeFormatLists (sim/dex-formats.ts:561). The
	// bodyless headers stay HERE because that function keys sections by name and
	// appends into the existing bucket: this copy is what pins each section's
	// position and column. Move one out and its section relocates to the end of
	// the list. If custom-formats.ts goes missing these headers are dropped
	// silently (dex-formats.ts:657) — test/sim/misc/fork-customs.js guards it.
	//
	// Why the upstream sections below carry `column:` values that differ from
	// upstream's: `column` is a RUNNING value in list order, not a grouping key.
	// The client opens a new visual column whenever the number CHANGES in
	// sequence (panel-teamdropdown.tsx:459-491) — columns are runs, not groups.
	// Our sections render leftmost only by coming first, which forces everything
	// downstream to renumber. See 8ec60bf03. This is the one part of our
	// config/formats.ts diff that no seam can absorb.
	{
		section: "Gen 3 Megas",
		column: 1,
	},
