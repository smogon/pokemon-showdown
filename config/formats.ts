...
export const Formats: FormatList = [
	...
	},
	{
		...
		onValidateTeam(team) {
			...
			if (dragonCount !== 1) {
				return [
					`You must have exactly one Koraidon or Miraidon in your team.`,
				];
			}
		},
		onBattleStart() {
	...
	},
	{
		...
		onValidateRule() {
			if (this.format.gameType !== "singles") {
				throw new Error(
					`Shared Power currently does not support ${this.format.gameType} battles.`
				);
			}
		},
		...
		onSwitchIn(pokemon) {
			...
			for (const ability of format.getSharedPower!(pokemon)) {
				if (ability === "noability") {
					this.hint(
						`Mirror Armor and Trace break in Shared Power formats that don't use Shared Power as a base, so they get removed from non-base users.`
					);
				}
				const effect = "ability:" + ability;
	...
	},
	{
		...
		validateSet(set, teamHas) {
			const unreleased = (pokemon: Species) =>
				pokemon.tier === "Unreleased" &&
			...
			if (!teamHas.abilityMap) {
				...
				for (const pokemon of Dex.species.all()) {
					if (
						...
						(unreleased(pokemon) && !this.ruleTable.has("+unobtainable"))
					)
						continue;
					...
					for (const key of Object.values(pokemon.abilities)) {
						...
						if (abilityId in teamHas.abilityMap) {
							teamHas.abilityMap[abilityId][
								pokemon.evos ? "push" : "unshift"
							](pokemon.id);
						} else {
			...
			if (
				...
				return [
					`${species.name} is not obtainable in Generation ${this.dex.gen}.`,
				];
			}
			...
			if (!pokemonWithAbility)
				return [
					`${ability.name} is not available on a legal Pok\u00e9mon.`,
				];
			...
			const validSources: string[] = (teamHas.abilitySources[
				this.toID(set.species)
			] = []); // Evolution families
			...
			for (const donor of pokemonWithAbility) {
				...
				if (
					["Iron Leaves", "Walking Wake"].includes(donorSpecies.name) ||
			...
			if (!validSources.length) {
				...
				return [
					`${name} has an illegal set with an ability from ${
						this.dex.species.get(pokemonWithAbility[0]).name
					}.`,
				];
			}
		...
		onValidateTeam(team, f, teamHas) {
			if (this.ruleTable.has("abilityclause")) {
				...
				const num = parseInt(
					this.ruleTable.valueRules.get("abilityclause")!
				);
				for (const set of team) {
			...
			for (const set of team) {
				const abilitySources =
					teamHas.abilitySources?.[this.toID(set.species)];
			...
			for (const evoFamilies of evoFamilyLists) {
				...
				if (requiredFamilies[familyId] > 1) {
					return [
						...
						`(You inherit more than once from ${
							this.dex.species.get(familyId).name
						}).`,
					];
				}
			}
		...
		onBegin() {
			for (const pokemon of this.getAllPokemon()) {
				if (pokemon.baseAbility.includes("0")) {
					...
					pokemon.baseAbility = this.toID(
						pokemon.baseAbility.split("0")[0]
					);
	...
	},
	{
		...
		onSwitchIn(pokemon) {
			const originalFormeSecies = this.dex.species.get(
				// @ts-ignore
				pokemon.species.originalSpecies
			);
			if (
				originalFormeSecies.exists &&
				pokemon.m.originalSpecies !== originalFormeSecies.baseSpecies
			) {
				// Place volatiles on the Pokémon to show its mega-evolved condition and details
				this.add(
					"-start",
					pokemon,
					originalFormeSecies.requiredItem ||
						originalFormeSecies.requiredMove,
					"[silent]"
				);
		...
		onSwitchOut(pokemon) {
			const oMegaSpecies = this.dex.species.get(
				// @ts-ignore
				pokemon.species.originalSpecies
			);
			if (
				oMegaSpecies.exists &&
				pokemon.m.originalSpecies !== oMegaSpecies.baseSpecies
			) {
				this.add(
					"-end",
					pokemon,
					oMegaSpecies.requiredItem || oMegaSpecies.requiredMove,
					"[silent]"
				);
			}
		},
	...
	},
	{
		...
		onBeforeSwitchIn(pokemon) {
			...
			if (ally && ally.ability !== pokemon.ability) {
				if (
					!pokemon.m.innate &&
					!BAD_ABILITIES.includes(this.toID(ally.ability))
				) {
					pokemon.m.innate = "ability:" + ally.ability;
					if (
						!ngas ||
						ally.getAbility().isPermanent ||
						pokemon.hasItem("Ability Shield")
					) {
						pokemon.volatiles[pokemon.m.innate] = {
							id: pokemon.m.innate,
							target: pokemon,
						};
				...
				if (
					!ally.m.innate &&
					!BAD_ABILITIES.includes(this.toID(pokemon.ability))
				) {
	...
	},
	{
		...
		validateSet(set, teamHas) {
			...
			let problems =
				this.dex.formats
					.get("Obtainable Misc")
					.onChangeSet?.call(this, set, this.format) || null;
			...
			if (
				...
				nonstandard ||
				isCap ||
				species === crossSpecies
			)
				return this.validateSet(set, teamHas);
			if (!species.nfe)
				return [
					`${species.name} cannot cross evolve because it doesn't evolve.`,
				];
			...
			if (item.itemUser?.length) {
				if (
					!item.itemUser.includes(crossSpecies.name) ||
					crossSpecies.name !== species.name
				) {
					return [
		...
		onModifySpecies(species, target, source, effect) {
			...
			for (i in species.baseStats) {
				const statChange =
					crossSpecies.baseStats[i] - crossPrevoSpecies.baseStats[i];
				mixedSpecies.baseStats[i] = this.clampIntRange(
					species.baseStats[i] + statChange,
					1,
					255
				);
			...
			if (crossSpecies.types[1] !== crossPrevoSpecies.types[1]) {
				mixedSpecies.types[1] =
					crossSpecies.types[1] || crossSpecies.types[0];
			}
	...
	},
	{
		...
		validateSet(set, teamHas) {
			...
			if (set.moves.map(this.toID).includes(move.id)) {
				problems.push(
					`Moves in the item slot can't be in the moveslots as well.`
				);
			}
			...
			if (
				...
				for (const abilityName of unSeenAbilities) {
					const banReason = this.ruleTable.check(
						"ability:" + this.toID(abilityName)
					);
					if (banReason) {
						return [
							`${set.name}'s ability ${abilityName} is ${banReason}.`,
						];
					}
				}
			}
	...
	},
	{
		...
		pokemon: {
			getTypes(excludeAdded, preterastallized) {
				...
				const types = this.battle.runEvent(
					"Type",
					this,
					null,
					null,
				...
				if (types.length) {
					if (addTeraType)
						return Array.from(new Set([...types, addTeraType]));
					return types;
				}
		...
		battle: {
			tiebreak() {
				...
				this.add(
					...
					this.sides
						.map(
							(side, i) => `${side.name}: ${notFainted[i]} Pokemon left`
						)
						.join("; ")
				);
	...
	},
	{
			...
			for (const trademark in teamHas.trademarks) {
				if (teamHas.trademarks[trademark] > 1) {
					problems.push(
						...
						`(You have ${teamHas.trademarks[trademark]} Pok\u00e9mon with ${trademark} as a Trademark.)`
					);
				}
			}
			return problems;
		...
		validateSet(set, teamHas) {
			...
			if (ability.category !== "Status") {
				return [
					`${ability.name} is not a status move and cannot be used as a trademark.`,
				];
			}
			// Contingent trademark bans
			if (this.ruleTable.isRestricted(`move:${ability.id}`)) {
				return [
					`${ability.name} is restricted from being used as a trademark.`,
				];
			}
			...
			teamHas.trademarks[ability.name] =
				(teamHas.trademarks[ability.name] || 0) + 1;
			return problems.length ? problems : null;
		},
	},
	...
	},
	{
		...
		onValidateSet(set) {
			...
			if (species.types.includes("Steel")) {
				return [
					`${species.name} is a Steel-type, which is banned from Metronome Battle.`,
				];
			}
			...
			if (
				set.moves.length !== 1 ||
				this.dex.moves.get(set.moves[0]).id !== "metronome"
			) {
				return [
	...
	},
	{
		...
		onBegin() {
			...
			this.add(
				`raw|<div class='broadcast-green'><b>Wondering what all these custom moves, abilities, and items do?<br />Check out the <a href="https://www.smogon.com/articles/super-staff-bros-4" target="_blank">Super Staff Bros 4 Guide</a> or use /ssb to find out!</b></div>`
			);
		...
		onSwitchIn(pokemon) {
			let name: string = this.toID(
				pokemon.illusion ? pokemon.illusion.name : pokemon.name
			);
			if (
	...
	},
	{
		...
		onValidateSet(set) {
			...
			if (species.gen > 8) {
				return [
					`${species.name} is from gen 9, which is banned from [Gen 8] Metronome Battle.`,
				];
			}
			if (species.types.includes("Steel")) {
				return [
		...
		onSwitchIn(pokemon) {
			...
			for (const i in pokemon.species.baseStats) {
				...
				this.add(
					"-start",
					pokemon,
					`${pokemon.species.baseStats[i as keyof StatsTable]}${
						i === "spa" ? "spc" : i
					}`,
					"[silent]"
				);
	...
	},
	{
		...
		threads: [
			`&bullet; <a href="https://www.smogon.com/forums/threads/3717286/">DPP Doubles</a>`,
		],
	...
	},
	{
		name: "[Gen 2] NU",
		threads: [
			`&bullet; <a href="https://www.smogon.com/forums/threads/3642565/">GSC NU</a>`,
		],
	...
	},
	{
		name: "[Gen 2] 1v1",
		threads: [
			`&bullet; <a href="https://www.smogon.com/forums/posts/8031464/">GSC 1v1</a>`,
		],
