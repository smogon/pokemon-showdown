...
export class Battle {
	...
	eachEvent(eventid: string, effect?: Effect | null, relayVar?: boolean) {
		...
		if ((eventid === 'ClimateWeather' || eventid === 'IrritantWeather' ||
			eventid === 'EnergyWeather' || eventid === 'ClearingWeather' ||
			eventid === 'CataclysmWeather') && this.gen >= 7) {
	...
	fieldEvent(eventid: string, targets?: Pokemon[]) {
		...
		while (handlers.length) {
			...
			if (handler.state?.target instanceof Pokemon) {
				...
				} else if (effect.effectType === 'IrritantWeather') {
					expectedStateLocation = handler.state.target.irritantWeatherState;
				} else if (effect.effectType === 'EnergyWeather') {
					expectedStateLocation = handler.state.target.energyWeatherState;
				} else if (effect.effectType === 'ClearingWeather') {
					expectedStateLocation = handler.state.target.clearingWeatherState;
				} else if (effect.effectType === 'CataclysmWeather') {
					expectedStateLocation = handler.state.target.cataclysmWeatherState;
	...
	runEvent(
		...
		for (const handler of handlers) {
			...
			if (
				(effect.effectType === 'ClimateWeather' || eventid === 'ClimateWeather') &&
			...
			if (
				(effect.effectType === 'IrritantWeather' || eventid === 'IrritantWeather') &&
			...
			if (
				(effect.effectType === 'EnergyWeather' || eventid === 'EnergyWeather') &&
			...
			if (
				(effect.effectType === 'ClearingWeather' || eventid === 'ClearingWeather') &&
			...
			if (
				(effect.effectType === 'CataclysmWeather' || eventid === 'CataclysmWeather') &&
	...
	resolvePriority(h: EventListenerWithoutPriority, callbackName: string) {
		...
		if (!handler.subOrder) {
			...
			const effectTypeOrder: { [k in EffectType]?: number } = {
				...
				Rule: 5,
				Ruleset: 5,
				// Poison Touch/Coat: 6, (also includes Perish Body)
				Ability: 7,
				Item: 8,
				// Stall: 9,
			};
			...
			if (handler.effect.effectType === 'Condition') {
				...
				if (handler.effect.name === 'Poison Touch' || handler.effect.name === 'Poison Coat' ||
					handler.effect.name === 'Perish Body') {
					handler.subOrder = 6;
	...
	findFieldEventHandlers(field: Field, callbackName: string, getKey?: 'duration', customHolder?: Pokemon) {
		...
		callback = this.getCallback(field, climateWeather, callbackName);
		...
		callback = this.getCallback(field, irritantWeather, callbackName);
		...
		callback = this.getCallback(field, energyWeather, callbackName);
		...
		callback = this.getCallback(field, clearingWeather, callbackName);
		...
		callback = this.getCallback(field, cataclysmWeather, callbackName);
