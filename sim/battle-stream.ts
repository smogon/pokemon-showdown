...
import { Battle, extractChannelMessages, isSideID } from './battle';
				...
				case 'eval':
					...
					try {
						...
						const player = (input: string) => {
							input = toID(input);
							if (/^p[0-9]+$/.test(input)) return battle.sides[parseInt(input.slice(1)) - 1];
							if (/^[0-9]+$/.test(input)) return battle.sides[parseInt(input) - 1];
...
export function getPlayerStreams(stream: BattleStream) {
	const streams : Record<string, Streams.ObjectReadStream<string>> = {
		...
		/*sides: stream.battle?.sides.map(side => new Streams.ObjectReadWriteStream({
			write(data: string) {
	...
	stream.battle?.sides.forEach(side => 
		streams[side.id] = new Streams.ObjectReadWriteStream({
			write(data: string) {
				void stream.write(data.replace(/(^|\n)/g, `$1>${side.id} `));
			}
		})
	...
	(async () => {
		for await (const chunk of stream) {
			...
			case 'update':
				let nums =  [-1, 0];
				let index = 1;
				Object.keys(streams).filter(name => name.match(/p\d+/)).forEach(side => nums.push(index++));
				const channelMessages = extractChannelMessages(data, nums);
				...
				index = 1;
				Object.keys(streams).filter(name => name.match(/p\d+/)).forEach(side => streams[side].push(channelMessages[index].join('\n')));
				break;
			case 'sideupdate':
		...
		for (const s of Object.values(streams).filter(stream => stream instanceof Streams.ObjectWriteStream || stream instanceof Streams.ObjectReadStream)) {
			s.pushEnd();
		}
	})().catch(err => {
