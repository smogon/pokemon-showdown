...
import { Tags } from "../data/mods/base/tags";
...
export type ModdedFormatData =
	| FormatData
	| (Omit<FormatData, "name"> & { inherit: true });
...
export class RuleTable extends Map<string, string> {
	...
	getTagRules() {
		...
		for (const ruleid of this.keys()) {
			if (/^[+*-]pokemontag:/.test(ruleid)) {
				...
				"+*-".includes(ruleid.charAt(0)) &&
				ruleid.slice(1) === "nonexistent"
			) {
	...
	addComplexTeamBan(
		...
		const complexBanTeamIndex = this.getComplexBanIndex(
			this.complexTeamBans,
			rule
		);
		if (complexBanTeamIndex !== -1) {
			if (this.complexTeamBans[complexBanTeamIndex][2] === Infinity) return;
			this.complexTeamBans[complexBanTeamIndex] = [
				rule,
				source,
				limit,
				bans,
	...
	resolveNumbers(format: Format, dex: ModdedDex) {
		const gameTypeMinTeamSize = ["triples", "rotation"].includes(
			format.gameType as "triples"
		)
			? 3
		...
		this.pickedTeamSize =
			Number(this.valueRules.get("pickedteamsize")) || null;
		...
		this.adjustLevelDown =
			Number(this.valueRules.get("adjustleveldown")) || null;
		...
		if (this.maxTeamSize > 24) {
			throw new Error(
				`Max team size ${this.maxTeamSize}${this.blame(
					"maxteamsize"
				)} is unsupported (we only support up to 24).`
			);
		}
		if (this.maxLevel > 99999) {
			throw new Error(
				`Max level ${this.maxLevel}${this.blame(
					"maxlevel"
				)} is unsupported (we only support up to 99999)`
			);
		}
		if (this.maxMoveCount > 24) {
			...
			throw new Error(
				`Max move count ${this.maxMoveCount}${this.blame(
					"maxmovecount"
				)} is unsupported (we only support up to 24)`
			);
		}
		...
		if (this.minTeamSize && this.minTeamSize < gameTypeMinTeamSize) {
			throw new Error(
				`Min team size ${this.minTeamSize}${this.blame(
					"minteamsize"
				)} must be at least ${gameTypeMinTeamSize} for a ${
					format.gameType
				} game.`
			);
		}
		if (this.pickedTeamSize && this.pickedTeamSize < gameTypeMinTeamSize) {
			throw new Error(
				`Chosen team size ${this.pickedTeamSize}${this.blame(
					"pickedteamsize"
				)} must be at least ${gameTypeMinTeamSize} for a ${
					format.gameType
				} game.`
		...
		if (
			this.minTeamSize &&
			this.pickedTeamSize &&
			this.minTeamSize < this.pickedTeamSize
		) {
			throw new Error(
				`Min team size ${this.minTeamSize}${this.blame(
					"minteamsize"
				)} is lower than chosen team size ${
					this.pickedTeamSize
				}${this.blame("pickedteamsize")}.`
			);
		}
		if (!this.minTeamSize)
			this.minTeamSize = Math.max(
				gameTypeMinTeamSize,
				this.pickedTeamSize || 0
			);
		...
		if (this.maxTeamSize < this.minTeamSize) {
			throw new Error(
				`Max team size ${this.maxTeamSize}${this.blame(
					"maxteamsize"
				)} must be at least min team size ${this.minTeamSize}${this.blame(
					"minteamsize"
				)}.`
			);
		}
		if (this.minLevel > this.maxLevel) {
			throw new Error(
				`Min level ${this.minLevel}${this.blame(
					"minlevel"
				)} should not be above max level ${this.maxLevel}${this.blame(
					"maxlevel"
				)}.`
			);
		}
		if (this.defaultLevel > this.maxLevel) {
			throw new Error(
				`Default level ${this.defaultLevel}${this.blame(
					"defaultlevel"
				)} should not be above max level ${this.maxLevel}${this.blame(
					"maxlevel"
				)}.`
		...
		if (this.defaultLevel < this.minLevel) {
			throw new Error(
				`Default level ${this.defaultLevel}${this.blame(
					"defaultlevel"
				)} should not be below min level ${this.minLevel}${this.blame(
					"minlevel"
				)}.`
			);
		}
		if (this.adjustLevelDown && this.adjustLevelDown >= this.maxLevel) {
			throw new Error(
				`Adjust Level Down ${this.adjustLevelDown}${this.blame(
					"adjustleveldown"
				)} will have no effect because it's not below max level ${
					this.maxLevel
				}${this.blame("maxlevel")}.`
			);
		}
		if (this.adjustLevel && this.valueRules.has("minlevel")) {
			throw new Error(
				`Min Level ${this.minLevel}${this.blame(
					"minlevel"
				)} will have no effect because you're using Adjust Level ${
					this.adjustLevel
				}${this.blame("adjustlevel")}.`
			);
		}
		if (this.evLimit && this.evLimit >= 1512) {
			throw new Error(
				`EV Limit ${this.evLimit}${this.blame(
					"evlimit"
				)} will have no effect because it's not lower than 1512, the maximum possible combination of 252 EVs in every stat (if you currently have an EV limit, use "! EV Limit" to remove the limit).`
			);
		}
		if (this.evLimit && this.evLimit < 0) {
			throw new Error(
				`EV Limit ${this.evLimit}${this.blame(
					"evlimit"
				)} can't be less than 0 (you might have meant: "! EV Limit" to remove the limit, or "EV Limit = 0" to ban EVs).`
			);
		}
		...
		if ((format as any).minSourceGen) {
			throw new Error(
				`minSourceGen is now a rule: "Min Source Gen = NUMBER"`
			);
		}
		...
		if ((format as any).defaultLevel) {
			throw new Error(
				`defaultLevel is now a rule: "Default Level = NUMBER"`
			);
		}
		...
		if ((format as any).maxForcedLevel) {
			throw new Error(
				`maxForcedLevel is now a rule: "Adjust Level Down = NUMBER"`
			);
		}
	}
...
export class Format extends BasicEffect implements Readonly<BasicEffect> {
	...
	declare readonly getSharedPower?: (
		this: Format,
		pokemon: Pokemon
	) => Set<string>;
	declare readonly getSharedItems?: (
		this: Format,
		pokemon: Pokemon
	) => Set<string>;
	...
	constructor(data: AnyObject) {
		...
		this.effectType =
			(Utils.getString(data.effectType) as FormatEffectType) || "Format";
		this.debug = !!data.debug;
		this.rated =
			typeof data.rated === "string" ? data.rated : data.rated !== false;
...
function mergeFormatLists(
	main: FormatList,
	custom: FormatList | undefined
): FormatList {
...
export class DexFormats {
	...
	load(): this {
		...
		let section = "";
		let column = 1;
		for (const [i, formatData] of Formats.entries()) {
			const format = formatData as {
				-readonly [K in keyof FormatData]: FormatData[K];
			};
			...
			if (!id) {
				throw new RangeError(
					`Format #${
						i + 1
					} must have a name with alphanumeric characters, not '${
						format.name
					}'`
				);
			}
			...
			if (!this.dex.dexes[format.mod])
				throw new Error(
					`Format "${format.name}" requires nonexistent mod: '${format.mod}'`
				);
	...
	validate(name: string) {
		...
		const customRules = customRulesString
			.split(",")
			.map((rule) => {
				rule = rule.replace(/[\r\n|]*/g, "").trim();
		...
		for (const rule of ruleset) {
			...
			if (typeof ruleSpec !== "string") {
				if (ruleSpec[0] === "complexTeamBan") {
					const complexTeamBan: ComplexTeamBan = ruleSpec.slice(
						1
					) as ComplexTeamBan;
			...
			if (rule.startsWith("!") && !rule.startsWith("!!")) {
				...
				if (repealDepth === depth) {
					throw new Error(
						`Rule "${rule}" did nothing because "${rule.slice(
							1
						)}" is not in effect`
					);
				}
			...
			if ("+*-".includes(ruleSpec.charAt(0))) {
				if (ruleTable.has(ruleSpec)) {
					throw new Error(
						`Rule "${rule}" in "${format.name}" already exists in "${
							ruleTable.get(ruleSpec) || format.name
						}"`
					);
				}
				for (const prefix of "+*-")
			...
			if (subformat.hasValue) {
				...
				if (
					(subformat.id === "pickedteamsize" ||
						subformat.id === "evlimit") &&
					value === "Auto"
				) {
					// can't be resolved until later
				} else if (
					subformat.hasValue === "integer" ||
					subformat.hasValue === "positive-integer"
				) {
					const intValue = parseInt(value);
					if (isNaN(intValue) || value !== `${intValue}`) {
						throw new Error(
							`In rule "${ruleSpec}", "${value}" must be an integer number.`
						);
					}
				}
				if (subformat.hasValue === "positive-integer") {
					...
					if (parseInt(value) <= 0) {
						throw new Error(
							`In rule "${ruleSpec}", "${value}" must be positive.`
						);
					}
				}
				...
				if (oldValue === value) {
					throw new Error(
						`Rule "${ruleSpec}" is redundant with existing rule "${
							subformat.id
						}=${value}"${ruleTable.blame(subformat.id)}.`
					);
					...
					if (oldValue === undefined) {
						if (
							...
							if (
								this.dex.formats.get(subformat.mutuallyExclusiveWith)
									.ruleset.length
							) {
								throw new Error(`This format does not support "!!"`);
							}
							ruleTable.valueRules.delete(
								subformat.mutuallyExclusiveWith
							);
					...
					if (oldValue !== undefined) {
						throw new Error(
							`Rule "${ruleSpec}" conflicts with "${
								subformat.id
							}=${oldValue}"${ruleTable.blame(
								subformat.id
							)} (Use "!! ${ruleSpec}" to override "${
								subformat.id
							}=${oldValue}".)`
						);
					}
					if (
						...
						const oldRule = `"${
							subformat.mutuallyExclusiveWith
						}=${ruleTable.valueRules.get(
							subformat.mutuallyExclusiveWith
						)}"`;
						throw new Error(
							`Format can't simultaneously have "${ruleSpec}" and ${oldRule}${ruleTable.blame(
								subformat.mutuallyExclusiveWith
							)} (Use "!! ${ruleSpec}" to override ${oldRule}.)`
						);
					}
				}
				...
				if (value !== undefined)
					throw new Error(
						`Rule "${ruleSpec}" should not have a value (no equals sign)`
					);
				if (repealAndReplace)
				...
				if (ruleTable.has(subformat.id) && !repealAndReplace) {
					throw new Error(
						`Rule "${rule}" in "${format.name}" already exists in "${
							ruleTable.get(subformat.id) || format.name
						}"`
					);
				}
			}
				...
				if (!repeals?.has(ruleid)) {
					...
					if (newValue !== undefined) {
						...
						if (
							...
							ruleTable.valueRules.has(
								subSubFormat.mutuallyExclusiveWith
							)
						) {
							// mutually exclusive conflict!
							throw new Error(
								`Rule "${ruleid}=${newValue}" from ${
									subformat.name
								}${subRuleTable.blame(ruleid)} conflicts with "${
									subSubFormat.mutuallyExclusiveWith
								}=${ruleTable.valueRules.get(
									subSubFormat.mutuallyExclusiveWith
								)}"${ruleTable.blame(
									subSubFormat.mutuallyExclusiveWith
								)} (Repeal one with ! before adding another)`
							);
						}
							...
							if (oldValue !== undefined) {
								...
								throw new Error(
									`Rule "${ruleid}=${newValue}" from ${
										...
										ruleid
									)} conflicts with "${ruleid}=${oldValue}"${ruleTable.blame(
										ruleid
									)} (Repeal one with ! before adding another)`
								);
							}
			...
			for (const [
				...
				limit,
				bans,
			] of subRuleTable.complexBans) {
				ruleTable.addComplexBan(
					subRule,
					source || subformat.name,
					limit,
					bans
				);
			}
			for (const [
				...
				limit,
				bans,
			] of subRuleTable.complexTeamBans) {
				ruleTable.addComplexTeamBan(
					subRule,
					source || subformat.name,
					limit,
		...
		switch (rule.charAt(0)) {
			...
			case "+":
				if (rule.slice(1).includes(">") || rule.slice(1).includes("+")) {
					...
					if (
						gtIndex >= 0 &&
						/^[0-9]+$/.test(buf.slice(gtIndex + 1).trim())
					) {
	...
	validateBanRule(rule: string) {
		...
		for (const matchType of matchTypes) {
			...
			if (table.hasOwnProperty(id)) {
				if (matchType === "pokemon") {
					...
					if (
						(species.otherFormes || species.cosmeticFormes) &&
						ruleid !== species.id + toID(species.baseForme)
					) {
		...
		if (matches.length > 1) {
			throw new Error(
				`More than one thing matches "${rule}"; please specify one of: ` +
					matches.join(", ")
			);
		}
		if (matches.length < 1) {
