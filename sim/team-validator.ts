	...
	intersectWith(other: PokemonSources) {
		...
		if (other.tradebackLimitedEggMoves) {
			if (!this.tradebackLimitedEggMoves) {
				...
				this.tradebackLimitedEggMoves.push(
					...other.tradebackLimitedEggMoves
				);
			}
		}
...
export class TeamValidator {
	...
	validateSet(set: PokemonSet, teamHas: AnyObject): string[] | null {
		...
		if (!setSources.sourcesBefore && setSources.sources.length) {
			...
			for (const source of setSources.sources) {
				if (
					this.validateSource(set, source, setSources, outOfBattleSpecies)
				)
					continue;
	...
	validateStats(
		...
		if (setSources.isFromPokemonGo) {
			...
			if (
				set.ivs.atk !== set.ivs.spa &&
				!(canBottleCap && (set.ivs.atk === 31 || set.ivs.spa === 31))
			) {
				problems.push(
					`${name}'s Atk and Spa IVs must match because it is from Pokemon GO.`
				);
			}
			if (
				set.ivs.def !== set.ivs.spd &&
				!(canBottleCap && (set.ivs.def === 31 || set.ivs.spd === 31))
			) {
				problems.push(
					`${name}'s Def and Spd IVs must match because it is from Pokemon GO.`
				);
			}
			if (set.hpType && set.hpType !== "Dark" && set.hpType !== "Ice") {
				problems.push(
					`${name} must have Hidden Power Dark or Ice because it is from Pokemon GO.`
				);
			}
		}
					...
					for (const moveSource of learnset[move]) {
						...
						if (
							...
							if (moveSource.charAt(1) === "E") {
								...
								if (
									eggGen === 2 &&
									this.dex.moves.getByID(move).gen === 1
								)
		...
		if (!pokemonGoData) {
			...
			if (set.shiny && pokemonGoSources.includes("noshiny")) {
				...
				for (const stat in ivs) {
					if (
						Math.floor(ivs[stat as "hp"] / 2) < minIVs &&
						stat !== "spe"
					) {
						problems.push(
							`${name} must have at least ${minIVs} ` +
								(minIVs === 1 ? `IV` : `IVs`) +
		...
		if (
			...
			if (pokemonGoData.LGPERestrictiveMoves) {
				...
				for (const restrictiveMove in pokemonGoData.LGPERestrictiveMoves) {
					const moveLevel =
						pokemonGoData.LGPERestrictiveMoves[restrictiveMove];
					...
					if (restrictiveMovesToID.includes(restrictiveMove)) {
						if (!levelUpMoveCount) {
							...
							return `'s moves ${(
								...
								", "
							)} are incompatible with its Pokemon GO origin.`;
						}
					}
				}
			}
