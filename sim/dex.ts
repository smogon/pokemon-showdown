...
export class ModdedDex {
	 ...
	 * @param growthRate The growth rate curve to use
	 * @returns Total experience needed to reach that level
	 */
	getExpForLevel(level: number, growthRate: GrowthRate = 'mediumfast'): number {
		const n = level;
	 ...
	 * @param victor The victorious Pokemon
	 * @param participants Number of Pokemon that participated
	 * @param trainerBattle Whether this was a trainer battle (1.5x multiplier)
	 * @param expShare Whether Exp Share is active
	 * @param luckyEgg Whether victor is holding Lucky Egg (1.5x multiplier)
	 * @returns Amount of experience to award
	 */
	calculateExpGain(
		defeated: Pokemon,
		victor: Pokemon,
		participants: number = 1,
		trainerBattle: boolean = false,
		expShare: boolean = false,
		luckyEgg: boolean = false
	): number {
		const a = defeated.baseSpecies.baseExp;
		const b = defeated.level;
		const L = victor.level;
		const s = participants;
		const t = trainerBattle ? 1.5 : 1.0;
		const e = expShare ? 1.0 : 1.0;
		const p = luckyEgg ? 1.5 : 1.0;

		// Gen 5+ formula
		const base = Math.floor((a * b) / (5 * s));
		const levelMod = Math.pow((2 * b + 10) / (b + L + 10), 2.5);
		const final = Math.floor(base * levelMod * t * e * p) + 1;
