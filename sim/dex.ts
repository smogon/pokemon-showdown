...
import * as mods from "../data/mods";
import * as textData from "../data/text";
...
export class ModdedDex {
	...
	readonly currentMod: keyof typeof mods;
	...
	dataSearch(
		...
		for (const table of [...searchIn, "Aliases"] as DataType[]) {
			...
			for (const j in searchObj) {
				...
				if (ld <= maxLd) {
					const word =
						(searchObj[j] as DexTable<any>).name ||
						(searchObj[j] as DexTable<any>).species ||
						j;
	...
	loadDataFile(mod: Mod, dataType: DataType | "Aliases"): AnyObject {
		try {
			const modData = mods[mod] as AnyObject;
			if (!modData || typeof modData !== "object") {
				throw new TypeError(
					`${mod}.${dataType}, if it exists, must export a non-null object`
				);
			}
			if (modData[dataType]?.constructor?.name !== "Object") {
				throw new TypeError(
					`${mod}.${dataType}, if it exists, must export an object whose '${dataType}' property is an Object`
				);
			}
	...
	includeMods(): this {
		...
		for (const mod in mods) {
			dexes[mod] = new ModdedDex(mod as Mod);
		}
	...
	loadTextData() {
		...
		dexes["base"].textCache = {
			Pokedex: textData.PokedexText,
			Moves: textData.MovesText,
			Abilities: textData.AbilitiesText,
			Items: textData.ItemsText,
			Default: textData.DefaultText,
		};
