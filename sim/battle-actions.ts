...
export class BattleActions {
	...
	modifyDamage(
		...
		if (pokemon.status === 'brn' && move.category === 'Physical' && (!pokemon.hasAbility('guts') || !pokemon.hasAbility('heatproof'))) {
			if (this.battle.gen < 6 || move.id !== 'facade') {
				baseDamage = this.battle.modify(baseDamage, 0.5);
			}
		}

		if (pokemon.status === 'frz' && move.category === 'Special' && (!pokemon.hasAbility('guts') || !pokemon.hasAbility('heatproof'))) {
			if (this.battle.gen < 6 || move.id !== 'facade') {
				baseDamage = this.battle.modify(baseDamage, 0.5);
			}
		}
