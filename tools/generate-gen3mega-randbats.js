 ...
 * see generate-gen3mega-randbats.LICENSE in this directory.
 */
...
const EXPECTED_TRANSFORMATION_COUNT = 52;
const DEFAULT_OUTPUT_DIRECTORY = path.resolve(
	__dirname, '../data/random-battles/gen3mega/generated'
);
...
function checkArtifacts(artifacts, outputDirectory = DEFAULT_OUTPUT_DIRECTORY) {
	...
	if (stale.length) {
		throw new Error(
			...
			`Run npm run generate-gen3mega-randbats`
		);
	}
	return files;
}
...
function printHelp() {
	process.stdout.write(
		`Usage: node tools/generate-gen3mega-randbats.js [options]\n\n` +
		`Options:\n` +
		...
		`  --output <directory>  Output root (default: server gen3mega/generated)\n`
	);
}
