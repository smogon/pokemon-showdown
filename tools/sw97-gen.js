...
const beta = {}; // defender -> { attacker: code }
for (const raw of R('data/types/type_matchups.asm')) {
	const l = strip(raw);
	const m = l.match(/^db (TYPE_\w+),\s*(TYPE_\w+),\s*(\w+)$/);
	if (m && TYPE[m[1]] && TYPE[m[2]]) { const atk = TYPE[m[1]], def = TYPE[m[2]]; (beta[def] = beta[def] || {})[atk] = VAL[m[3]]; }
}
const allTypes = [...new Set(Object.values(TYPE))]; // 17 real types (TYPE_BIRD folds into Flying)
const betaCode = (def, atk) => (beta[def] && beta[def][atk] !== undefined) ? beta[def][atk] : 0; // absent = neutral
const tc = [];
...
console.log(`Wrote gen2sw97: ${pd.length} species (${newCount} new), ${ls.length} learnsets, ${fd.length} formats-data, ${tc.length} type overrides.`);
