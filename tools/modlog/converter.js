...
var _bettersqlite3 = require('better-sqlite3'); var Database = _bettersqlite3;



 function parseBrackets(line, openingBracket) {
	const brackets = {
		'(': ')',
		'[': ']',
	};
...
function toID(text) {
 ...
 function modernizeLog(line, nextLine) {
	...
	const modernizerTransformations = {
		...
		' was muted by ': (log) => {
			...
			return `${isHour ? 'HOUR' : ''}MUTE: [${muted}] ${getAlts(muted )}${ip ? `[${ip}] ` : ``}by ${actionTaker}${reason ? `: ${reason}` : ``}`;
		},
 ...
 class ModlogConverterSQLite {
	...
	async toTxt() {
		const database = _optionalChain([this, 'access', _6 => _6.isTesting, 'optionalAccess', _7 => _7.db]) || new Database(this.databaseFile, {fileMustExist: true});
		...
		for (const {roomid} of roomids) {
			...
			for (const result of results) {
				const entry = {
					...
					roomID: _optionalChain([result, 'access', _8 => _8.roomid, 'optionalAccess', _9 => _9.replace, 'call', _10 => _10(/^global-/, '')]),
					visualRoomID: result.visual_roomid,
					userid: result.userid,
					autoconfirmedID: result.autoconfirmed_userid,
					alts: _optionalChain([result, 'access', _11 => _11.alts, 'optionalAccess', _12 => _12.split, 'call', _13 => _13(',')]),
					ip: result.ip,
					isGlobal: _optionalChain([result, 'access', _14 => _14.roomid, 'optionalAccess', _15 => _15.startsWith, 'call', _16 => _16('global-')]),
					loggedBy: result.action_taker,
					note: result.note,
					time: result.timestamp,
				};
				const key = _optionalChain([entry, 'access', _17 => _17.roomID, 'optionalAccess', _18 => _18.split, 'call', _19 => _19('-'), 'access', _20 => _20[0]]) || 'global';
	...
	async writeFile(path, text) {
		...
		return _fs.FS.call(void 0, path).write(text);
	}
 ...
 class ModlogConverterTxt {
	...
	constructor(databaseFile, textLogDir, isTesting) {;ModlogConverterTxt.prototype.__init2.call(this);
		this.databaseFile = databaseFile;
		this.textLogDir = textLogDir;
		if (isTesting || Config.nofswriting) {
			this.isTesting = {
	...
	async toSQLite() {
		...
		for (const file of files) {
			...
			const raw = this.isTesting ? this.isTesting.files.get(file) || '' : await _fs.FS.call(void 0, `${this.textLogDir}/${file}`).read();
			const roomid = file.slice(7, -4);
			const entries = raw.split('\n')
				.map((line, index) => {
					const entry = parseModlog(line, raw[index + 1], roomid === 'global');
					if (roomid === 'global' && _optionalChain([entry, 'optionalAccess', _21 => _21.roomID]) === 'global') return null;
					return entry;
				});
		...
		const modlog = new (0, _modlog.Modlog)(this.isTesting ? ':memory:' : `${__dirname}/../../${this.databaseFile}`, true);
 ...
 class ModlogConverter {
	static async convert(from, to, databasePath, textLogDirectoryPath) {
