...
exports.getSerebiiEventdex = function(_) {
	...
	for (var y = 0; y < yearsHref.length; ++y) {
		...
		for (var e = 0; e < entries.length; ++e) {
			...
			// The first column contains the pokemon, it's level and it's gender
			...
			var nationalPokedexNumber = nationalPokedexNumberAndForme[0];
			...
			// Combine the results
			var resultKey = nationalPokedexNumber + (formeLetter ? "-" + formeLetter : "");
			if (!results[resultKey])
				results[resultKey] = new Array();
