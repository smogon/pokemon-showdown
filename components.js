...
var components = exports.components = {
	...
	bpladder: function (target, room, user) {
		if (!this.canBroadcast()) return;

		if (!target) target = 10;
		if (!/[0-9]/.test(target) && target.toLowerCase() !== 'all') target = -1;

		var ladder = Core.bpLadder(Number(target));
		if (ladder === 0) return this.sendReply('No one is ranked yet.');

		return this.sendReply('|raw|<center>' + ladder + 'To view the entire ladder use /bpladder <em>all</em> or to view a certain amount of users use /bpladder <em>number</em></center>');
	...
	tourpoll: function (target, room, user) {
		if (!this.can('poll', null, room)) return false;
		this.parse('/poll Tournament format?, ou, ubers, uu, ru, nu, lc, vgc, monotype, [gen 4] ou, random, 1v1 random, uber random, high tier random, low tier random, lc random, monotype random, generational random, inverse random, hoenn random, hoenn weather random, super smash bros. random, winter wonderland, community random, furry random, metronome 3v3 random, metronome 6v6 random, doubles random, triples random, hackmons challenge cup, [seasonal] random, [gen 2] random, [gen 1] random');
	},
	...
	randompoll: function (target, room, user) {
		if (!this.can('poll', null, room)) return false;
		this.parse('/poll Tournament format?, random, 1v1 random, uber random, high tier random, low tier random, lc random, monotype random, generational random, inverse random, hoenn random, hoenn weather random, super smash bros. random, winter wonderland, community random, furry random, doubles random, triples random, hackmons challenge cup, [seasonal] random, [gen 2] random, [gen 1] random');
	},
