...
function generateTeam(format, prng) {
...
async function runGame(format,) {
	...
	const p1spec = {name: "Bot 1", team: generateTeam(format, prng)};
	const p2spec = {name: "Bot 2", team: generateTeam(format, prng)};
...
(async () => {
	...
	while ((format = getNextFormat())) {
		...
		let seed = prng.seed;
		try {
			const game = runGame(format, prng);
			if (!ASYNC) await game;
			games.push(game);
		} catch (e) {
