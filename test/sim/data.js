...
describe('Dex data', () => {
	...
	it('should have valid Learnsets entries', function () {
		this.timeout(0);
		const mods = [Dex.mod('gen2'), Dex.mod('gen7letsgo'), Dex.mod('gen8bdsp'), Dex.mod('gen8legendsarceus'), Dex];
					...
					for (const [i, eventEntry] of entry.eventData.entries()) {
						if (eventEntry.moves) {
							...
							for (const eventMove of eventEntry.moves) {
								...
								const effectiveMod = ['gen8bdsp', 'gen8legendsarceus'].includes(mod.currentMod) ? mod.currentMod : undefined;
