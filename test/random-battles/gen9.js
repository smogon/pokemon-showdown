...
describe('[Gen 9] Baby Random Battle (slow)', () => {
	const options = {format: 'gen9babyrandombattle'};
	const setsJSON = require(`../../dist/data/random-battles/gen9baby/sets.json`);
	const dex = Dex.forFormat(options.format);

	it('all Pokemon should have 4 moves', function () {
		// This test takes more than 2000ms
		testTeam({...options, rounds: 100}, team => {
			for (const pokemon of team) assert(pokemon.moves.length === 4);
		});
	});
