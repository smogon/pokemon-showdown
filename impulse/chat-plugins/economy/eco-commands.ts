...
/**
 * Represents an active giveaway in a room.
 */
interface Giveaway {
	host: string;
	hostName: string;
...
/**
 * Maps room IDs to their active giveaways.
 */
	...
	eco: {
		...
		giveaway: {
			start(target, room, user): void {
				...
				if (activeGiveaways.has(room.roomid)) {
					return this.errorReply("There is already an active giveaway in this room.");
				}

				const [amountStr, durationStr] = target.split(',').map(s => s.trim());
				const amount = parseInt(amountStr);

				if (!amountStr || isNaN(amount) || amount <= 0) {
					return this.errorReply("Usage: /giveaway start [amount], [duration in seconds (optional)]");
				}
				...
				if (durationStr) {
					...
					if (isNaN(duration) || duration <= 0) {
						return this.errorReply("Duration must be a positive number of seconds.");
					}
				}
				...
				if (duration) {
					giveaway.timer = setTimeout(() => {
						this.parse(`/giveaway end`);
					}, duration * 1000);
				}

				activeGiveaways.set(room.roomid, giveaway);

				const hostNameColor = nameColor(user.name, true, false);
				const durationText = duration ? ` for ${duration} seconds` : '';
				room.add(`|html|<div class="broadcast-blue"><strong>Giveaway Started!</strong><br />` +
					`${hostNameColor} is hosting a giveaway of <strong>${Economy.formatMoney(amount)} ${CURRENCYNAME}</strong>${durationText}!<br />` +
					`Use <code>/giveaway join</code> to enter!</div>`).update();
			},

			join(target, room, user): void {
				if (!room || room.roomid === 'global') {
					return this.errorReply("There is no giveaway in this context.");
				}

				const giveaway = activeGiveaways.get(room.roomid);
				if (!giveaway) {
					return this.errorReply("There is no active giveaway in this room.");
				}

				if (giveaway.host === user.id) {
				...
				room.add(`|html|${userNameColor} has joined the giveaway! (${giveaway.participants.size} participant${giveaway.participants.size !== 1 ? 's' : ''})`).update();
			},
			...
			async end(target, room, user): Promise<void> {
				...
				// Only host or staff can end the giveaway
				if (giveaway.host !== user.id && !user.can('roomowner', null, room)) {
					return this.errorReply("Only the giveaway host or room staff can end the giveaway.");
				}
				...
				if (giveaway.participants.size === 0) {
					activeGiveaways.delete(room.roomid);
					return room.add(`|html|<div class="broadcast-blue"><strong>Giveaway Ended</strong><br />` +
						`No one joined the giveaway. It has been cancelled.</div>`).update();
				}
				...
				// Award the money
				await Economy.updateBalance(winnerId, giveaway.amount);

				const winnerUser = await Economy.getUser(winnerId);
				const winnerDisplayName = Users.getExact(winnerId)?.name || winnerId;
				const winnerNameColor = nameColor(winnerDisplayName, true, false);
				const hostNameColor = nameColor(giveaway.hostName, true, false);

				room.add(`|html|<div class="broadcast-green"><strong>Giveaway Winner!</strong><br />` +
					`Congratulations to ${winnerNameColor} for winning <strong>${Economy.formatMoney(giveaway.amount)} ${CURRENCYNAME}</strong> ` +
					`from ${hostNameColor}'s giveaway!<br />` +
					`New balance: ${Economy.formatMoney(winnerUser.balance)} ${CURRENCYNAME}</div>`).update();

				const winnerSocket = Users.get(winnerId);
				if (winnerSocket) {
					winnerSocket.popup(`|html|Congratulations! You won ${Economy.formatMoney(giveaway.amount)} ${CURRENCYNAME} from ${hostNameColor}'s giveaway! Your new balance is ${Economy.formatMoney(winnerUser.balance)} ${CURRENCYNAME}.`);
				}

				activeGiveaways.delete(room.roomid);
			},

			cancel(target, room, user): void {
				if (!room || room.roomid === 'global') {
					return this.errorReply("There is no giveaway in this context.");
				}
				...
				// Only host or staff can cancel the giveaway
				if (giveaway.host !== user.id && !user.can('roomowner', null, room)) {
					return this.errorReply("Only the giveaway host or room staff can cancel the giveaway.");
				}
				...
				const hostNameColor = nameColor(user.name, true, false);
				room.add(`|html|<div class="broadcast-blue"><strong>Giveaway Cancelled</strong><br />` +
					`${hostNameColor} has cancelled the giveaway.</div>`).update();
			},

			status(target, room, user): void {
				if (!room || room.roomid === 'global') {
					return this.errorReply("There is no giveaway in this context.");
				}
				...
				const giveaway = activeGiveaways.get(room.roomid);
				if (!giveaway) {
					return this.sendReplyBox("There is no active giveaway in this room.");
				}

				const hostNameColor = nameColor(giveaway.hostName, true, false);
				const elapsedTime = Math.floor((Date.now() - giveaway.startTime) / 1000);
				const timeInfo = giveaway.duration ?
					`Time remaining: ${giveaway.duration - elapsedTime} seconds` :
					`Time elapsed: ${elapsedTime} seconds`;

				this.sendReplyBox(`<strong>Active Giveaway</strong><br />` +
					`Host: ${hostNameColor}<br />` +
					`Prize: <strong>${Economy.formatMoney(giveaway.amount)} ${CURRENCYNAME}</strong><br />` +
			...
			help(): void {
				...
				const helpList = [
					{
						cmd: "/giveaway start [amount], [duration]",
						desc: `Starts a giveaway with the specified amount of ${CURRENCYNAME}. Duration in seconds is optional. Requires: &.`,
					},
					{
						cmd: "/giveaway join",
						desc: "Join the active giveaway in the current room.",
					},
					{
						cmd: "/giveaway end",
						desc: "End the giveaway and select a random winner. Only the host or room staff can use this.",
					},
					{
						cmd: "/giveaway cancel",
						desc: "Cancel the active giveaway without selecting a winner. Only the host or room staff can use this.",
					},
					{
						cmd: "/giveaway status",
						desc: "Check the status of the active giveaway in the current room.",
					},
				];
				const html = `<center><strong>Giveaway Commands:</strong></center><hr><ul style="list-style-type:none;padding-left:0;">` +
		...
		help(): void {
			...
			const helpList = [
				...
				{
					cmd: "/giveaway",
					desc: `Manage giveaways in the current room. Use <b>/giveaway help</b> for more info.`,
				},
			];
