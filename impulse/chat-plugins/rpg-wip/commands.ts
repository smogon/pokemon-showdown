...
export const commands: ChatCommands = {
	rpg: {
		...
		travel(target, room, user) {
			...
			if (!targetLocation) return this.sendReply(`|uhtmlchange|rpg-${user.id}|${generateExploreHTML(player, currentLocation, "Error: Invalid location.")}`);

			const connection = currentLocation.connectedLocations.find(c => c.id === targetLocationId);
			if (!connection) return this.sendReply(`|uhtmlchange|rpg-${user.id}|${generateExploreHTML(player, currentLocation, `Error: You can't travel to ${targetLocation.name} from here.`)}`);

			if (connection.requiredBadge && !player.obtainedBadges.includes(connection.requiredBadge)) return this.sendReply(`|uhtmlchange|rpg-${user.id}|${generateExploreHTML(player, currentLocation, `Error: Requires ${connection.requiredBadge}.`)}`);
			if (connection.requiredFlag && !player.storyFlags.has(connection.requiredFlag)) return this.sendReply(`|uhtmlchange|rpg-${user.id}|${generateExploreHTML(player, currentLocation, `Error: Locked.`)}`);
		...
		building(target, room, user) {
			...
			if (!building) return this.sendReply(`|uhtmlchange|rpg-${user.id}|${generateExploreHTML(player, currentLocation, "Error: Building not found.")}`);
			if (building.accessible === false) return this.sendReply(`|uhtmlchange|rpg-${user.id}|${generateExploreHTML(player, currentLocation, "Error: This building is currently locked.")}`);
			if (building.requiredFlag && !player.storyFlags.has(building.requiredFlag)) return this.sendReply(`|uhtmlchange|rpg-${user.id}|${generateExploreHTML(player, currentLocation, "Error: You can't access this building yet.")}`);
		...
		wildpokemon(target, room, user) {
			...
			if (activeParty.length === 0) {
				return this.sendReply(`|uhtmlchange|rpg-${user.id}|${generateExploreHTML(player, location, "Error: Your team has fainted! Heal at a Center.")}`);
			}
		...
		challenge(target, room, user) {
			...
			if (activeParty.length === 0) {
				return this.sendReply(`|uhtmlchange|rpg-${user.id}|${generateExploreHTML(player, location, "Error: You must heal your Pokémon before challenging a trainer!")}`);
			}
		...
		battleaction: {
			// [STEP 5: REFACTORED] - All errorReplies replaced with generateBattleHTML updates
			move(target, room, user) {
				...
				if (attackerSlotIndex !== 0 && attackerSlotIndex !== 1) {
					return this.sendReply(`|uhtmlchange|rpg-${user.id}|${generateBattleHTML(battle, ["Error: Invalid attacker slot."])}`);
				}

				const attackerSlot = battle.playerSlots[attackerSlotIndex];
				if (!attackerSlot || attackerSlot.pokemon.hp <= 0) {
					return this.sendReply(`|uhtmlchange|rpg-${user.id}|${generateBattleHTML(battle, ["Error: This Pokémon cannot move."])}`);
				}
				if (battle.pendingActions[attackerSlotIndex]) {
					return this.sendReply(`|uhtmlchange|rpg-${user.id}|${generateBattleHTML(battle, ["Error: Action already queued."])}`);
				}

				const moveData = getMove(moveId);
				if (!moveData.exists) {
					return this.sendReply(`|uhtmlchange|rpg-${user.id}|${generateBattleHTML(battle, [`Error: Move '${moveId}' not found.`])}`);
				}
			...
			selecttarget(target, room, user) {
				...
				if (isNaN(attackerSlotIndex) || !moveId) return this.sendReply(`|uhtmlchange|rpg-${user.id}|${generateBattleHTML(battle, ["Error: Invalid selection."])}`);
			...
			forceswitch(target, room, user) {
				...
				if (isNaN(slotToFill) || !pokemonId) return this.sendReply(`|uhtmlchange|rpg-${user.id}|${generateBattleHTML(battle, ["Error: Invalid switch command."])}`);
				...
				if (partyIndex === -1) return this.sendReply(`|uhtmlchange|rpg-${user.id}|${generateBattleHTML(battle, ["Error: Invalid Pokemon or it has fainted."])}`);
				if (battle.playerSlots.some(s => s?.pokemon.id === pokemonId)) return this.sendReply(`|uhtmlchange|rpg-${user.id}|${generateBattleHTML(battle, ["Error: This Pokemon is already in battle."])}`);
				if (battle.playerSlots[slotToFill] !== null && !battle.pendingPivot) return this.sendReply(`|uhtmlchange|rpg-${user.id}|${generateBattleHTML(battle, ["Error: This slot is not empty."])}`);
			...
			playerswitch(target, room, user) {
				...
				if (isNaN(slotToSwitchOut) || !pokemonIdIn) return this.sendReply(`|uhtmlchange|rpg-${user.id}|${generateBattleHTML(battle, ["Error: Invalid switch command."])}`);

				const outgoingSlot = battle.playerSlots[slotToSwitchOut];
				if (!outgoingSlot || outgoingSlot.pokemon.hp <= 0) return this.sendReply(`|uhtmlchange|rpg-${user.id}|${generateBattleHTML(battle, ["Error: Slot empty or fainted."])}`);
				...
				if (!incomingPokemon) return this.sendReply(`|uhtmlchange|rpg-${user.id}|${generateBattleHTML(battle, ["Error: Invalid Pokemon."])}`);
				if (battle.playerSlots.some(s => s?.pokemon.id === pokemonIdIn)) return this.sendReply(`|uhtmlchange|rpg-${user.id}|${generateBattleHTML(battle, ["Error: Pokemon already in battle."])}`);
			...
			// [STEP 5: REFACTORED]
				...
				if (!ballId || isNaN(targetSlotIndex)) return this.sendReply(`|uhtmlchange|rpg-${user.id}|${generateBattleHTML(battle, ["Error: Invalid catch command."])}`);
				if (battle.battleType.includes('trainer')) return this.sendReply(`|uhtmlchange|rpg-${user.id}|${generateBattleHTML(battle, ["You can't steal another Trainer's Pokémon!"])}`);
				...
				if (ballItem?.category !== 'pokeball' || ballItem.quantity < 1) return this.sendReply(`|uhtmlchange|rpg-${user.id}|${generateBattleHTML(battle, ["Error: You don't have that ball!"])}`);

				if (player.party.length >= 6 && player.pc.size >= 100) return this.sendReply(`|uhtmlchange|rpg-${user.id}|${generateBattleHTML(battle, ["Error: Storage full!"])}`);
