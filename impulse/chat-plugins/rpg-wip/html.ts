...
import { getPlayerData } from './lib/player'; // We will export this from core.ts
...
export function generateStarterSelectionConfirmationHTML(species: any, tempSlot: ActivePokemonSlot, startingLocation: any): string {
	return `<div class="infobox">` +
...
export function generateLearnMoveResultHTML(message: string, tempSlot: ActivePokemonSlot): string {
	return `<div class="infobox"><h2>Move Learning Result</h2><p>${message}</p>${generatePokemonInfoHTML(tempSlot, true)}${generateBottomNavigation()}</div>`;
}

export function generateLearnEggMoveResultHTML(pokemon: RPGPokemon, newMoveData: any, tempSlot: ActivePokemonSlot): string {
	return `<div class="infobox"><h2>Move Learned!</h2><p><strong>${pokemon.species}</strong> learned <strong>${newMoveData.name}</strong>!</p>${generatePokemonInfoHTML(tempSlot)}<p><button name="send" value="/rpg party" class="button">Back to Party</button></p>${generateBottomNavigation()}</div>`;
}
...
export function generatePartyHTML(player: PlayerData, createActivePokemonSlot: (pokemon: RPGPokemon) => ActivePokemonSlot): string {
	...
	partyHTML += `${generateBottomNavigation()}</div>`;
	return partyHTML;
}

export function generateUseItemPokemonSelectionHTML(player: PlayerData, item: any): string {
	let html = `<div class="infobox"><h2>Use ${item.name}</h2><p>Select a Pokemon to use this item on:</p>`;
	const isRevival = ['revive', 'maxrevive', 'revivalherb'].includes(item.id);
	const isHealing = ['potion', 'superpotion', 'hyperpotion', 'maxpotion', 'fullrestore', 'freshwater', 'sodapop', 'lemonade', 'moomoomilk', 'tea', 'energyroot', 'energypowder', 'berryjuice'].includes(item.id);
	const isStatusHeal = ['antidote', 'paralyzeheal', 'awakening', 'burnheal', 'iceheal', 'fullheal', 'healpowder'].includes(item.id);
	const isPPRestore = ['ether', 'maxether', 'elixir', 'maxelixir'].includes(item.id);
	const isVitamin = ['hpup', 'protein', 'iron', 'calcium', 'zinc', 'carbos'].includes(item.id);
	for (const pokemon of player.party) {
		let show = false;
		...
		if (show) {
			html += `<div style="border: 1px solid #ccc; padding: 8px; margin: 5px; border-radius: 5px; display: flex; justify-content: space-between; align-items: center;"><div><strong>${pokemon.species}</strong> (Lvl ${pokemon.level})<br>${details}</div><button name="send" value="/rpg useitem ${item.id} ${pokemon.id}" class="button">Use</button></div>`;
		}
	}
...
export function generateItemUseSuccessHTML(message: string, tempSlot: ActivePokemonSlot): string {
...
export function generateMiscItemPokemonSelectionHTML(player: PlayerData, item: any): string {
	let html = `<div class="infobox"><h2>Use ${item.name}</h2><p>Select a Pokémon to use this item on:</p>`;
	for (const pokemon of player.party) {
		let canUse = true;
		let details = `(Lvl ${pokemon.level})`;
		if (item.id === 'rarecandy' || item.id.startsWith('expcandy')) {
		...
		if (canUse) {
			html += `<div style="border: 1px solid #ccc; padding: 8px; margin: 5px; border-radius: 5px; display: flex; justify-content: space-between; align-items: center;"><div><strong>${pokemon.species}</strong> ${details}</div><button name="send" value="/rpg useitem ${item.id} ${pokemon.id}" class="button">Use</button></div>`;
		}
	}
	html += `<p><button name="send" value="/rpg items" class="button">Back to Items</button></p></div>`;
	return html;
...
export function generateNoLearnableEggMovesHTML(pokemon: RPGPokemon): string {
	return `<div class="infobox"><h2>No Moves Available</h2><p><strong>${pokemon.species}</strong> either has no Egg Moves or already knows all of them.</p><p><button name="send" value="/rpg items" class="button">Back to Items</button></p></div>`;
}

export function generateEvolutionHTML(message: string, tempSlot: ActivePokemonSlot, hasPendingMoves: boolean): string {
	let resultHTML = `<div class="infobox"><h2>Item Used!</h2><p>${message}</p>${generatePokemonInfoHTML(tempSlot, true)}`;
	if (hasPendingMoves) {
...
export function generateEvolutionErrorHTML(pokemon: RPGPokemon, itemId: string): string {
	return `<div class="infobox"><p style="color: red; font-weight: bold;">It had no effect... (${pokemon.species} is not compatible with this item).</p><p><button name="send" value="/rpg useitem ${itemId}" class="button">Try Again</button> <button name="send" value="/rpg items" class="button">Back to Items</button></p></div>`;
}

export function generateRestorePPResultHTML(pokemon: RPGPokemon, item: any, moveData: any, restored: number, tempSlot: ActivePokemonSlot): string {
	return `<div class="infobox"><h2>Item Used!</h2><p>You used an <strong>${item.name}</strong> on <strong>${pokemon.species}</strong>!</p><p><strong>${moveData.name}</strong>'s PP was restored by ${restored}.</p>${generatePokemonInfoHTML(tempSlot, true)}<p><button name="send" value="/rpg party" class="button">Back to Party</button><button name="send" value="/rpg items" class="button">Back to Items</button></p></div>`;
}

export function generateDepositPokemonConfirmationHTML(pokemon: RPGPokemon): string {
	return `<div class="infobox"><h2>Pokemon Deposited</h2><p><strong>${pokemon.species}</strong> has been deposited into the PC!</p><p><button name="send" value="/rpg pc" class="button">View PC</button><button name="send" value="/rpg party" class="button">Back to Party</button></p></div>`;
}

export function generateWithdrawPokemonConfirmationHTML(pokemon: RPGPokemon, tempSlot: ActivePokemonSlot): string {
	return `<div class="infobox"><h2>Pokemon Withdrawn</h2><p><strong>${pokemon.species}</strong> has been withdrawn from the PC!</p>${generatePokemonInfoHTML(tempSlot, true)}<p><button name="send" value="/rpg pc" class="button">View PC</button><button name="send" value="/rpg party" class="button">Back to Party</button></p></div>`;
}

export function generateBuyConfirmationHTML(quantity: number, item: any, totalCost: number, player: PlayerData): string {
	return `<div class="infobox"><h2>Purchase Complete!</h2><p>You bought <strong>${quantity}x ${item.name}</strong> for ₽${totalCost}!</p><p><strong>Money remaining:</strong> ₽${player.money}</p><p><button name="send" value="/rpg shop" class="button">Continue Shopping</button> <button name="send" value="/rpg items" class="button">View Inventory</button></p>${generateBottomNavigation()}</div>`;
}

export function generateTravelHTML(currentLocation: any, player: PlayerData): string {
...
export function generateArrivalHTML(targetLocation: any, events: any[], player: PlayerData): string {
	if (events.length > 0) {
		...
		if (firstEvent.type === 'dialogue') {
			...
			if (firstEvent.pokemon) {
				eventHTML += `<p>${firstEvent.pokemon.species} joined your party!</p>`;
			}
			eventHTML += `<p><button name="send" value="/rpg explore" class="button">Continue</button></p>`;
			...
			eventHTML += `<p>A wild ${firstEvent.pokemon.shiny ? '✨ ' : ''}${species.name} (Lv. ${firstEvent.pokemon.level}) appeared!</p>`;
			eventHTML += `<p><button name="send" value="/rpg scriptedbattle ${`scripted_wild_${firstEvent.id}`}" class="button">⚔️ Battle!</button></p>`;
...
export function generateBuildingHTML(building: any, player: PlayerData, NPC_DATABASE: any, TRAINER_DATABASE: any): string {
...
export function generateGiveItemSelectionHTML(player: PlayerData, pokemon: RPGPokemon): string {
...
export function generateGiveItemPokemonSelectionHTMLWrapper(player: PlayerData): string {
...
export function generateGiveItemConfirmationHTML(pokemon: RPGPokemon, item: any, tempSlot: ActivePokemonSlot): string {
	return `<div class="infobox"><h2>Item Given</h2><p><strong>${pokemon.species}</strong> is now holding the <strong>${item.name}</strong>!</p>${generatePokemonInfoHTML(tempSlot, true, true)}<p><button name="send" value="/rpg party" class="button">Back to Party</button></p></div>`;
}
...
export function generateTakeItemConfirmationHTML(pokemon: RPGPokemon, item: any, tempSlot: ActivePokemonSlot): string {
	return `<div class="infobox"><h2>Item Taken</h2><p>You took the <strong>${item.name}</strong> from <strong>${pokemon.species}</strong>.</p>${generatePokemonInfoHTML(tempSlot, true, true)}<p><button name="send" value="/rpg party" class="button">Back to Party</button></p></div>`;
}

export function generateNicknameConfirmationHTML(oldNickname: string, pokemon: RPGPokemon, tempSlot: ActivePokemonSlot): string {
	return `<div class="infobox"><h2>Nickname Changed!</h2><p>Changed <strong>${oldNickname}</strong>'s name to <strong>${pokemon.nickname}</strong>!</p>${generatePokemonInfoHTML(tempSlot, true, true)}<p><button name="send" value="/rpg party" class="button">Back to Party</button></p></div>`;
}

export function generateNPCListHTML(availableNPCs: [string, any][]): string {
...
export function generateNPCDialogueHTML(npc: any, player: PlayerData): string {
	let dialogueHTML = `<div class="infobox">` +
		`<h2>${npc.name}</h2>` +
		`<p>"${npc.dialogue}"</p>`;
	if (npc.action) {
		const action = npc.action;
		const actionCompleted = player.completedNPCActions.has(npc.id);
		if (!action.onceOnly || !actionCompleted) {
			...
			case 'giveitem':
				if (action.itemId && action.quantity) {
					...
					dialogueHTML += `<button name="send" value="/rpg npcaction ${npc.id}" class="button">✅ Accept Gift</button> `;
				}
				break;
			case 'givepokemon':
				if (action.pokemon) {
					...
					if (player.party.length >= 6 && player.pc.size >= 100) {
						...
						dialogueHTML += `<button name="send" value="/rpg npcaction ${npc.id}" class="button">✅ Accept Pokémon</button> `;
					}
				}
				break;
			case 'exchangeitems':
				if (action.requiredItem && action.requiredQuantity && action.itemId && action.quantity) {
					...
					if (hasRequired >= action.requiredQuantity) {
						...
						dialogueHTML += `<button name="send" value="/rpg npcaction ${npc.id}" class="button">🔄 Make Trade</button> `;
					} else {
			...
			case 'takeitem':
				if (action.itemId && action.quantity) {
					...
					if (hasItem >= action.quantity) {
						dialogueHTML += `<p style="color: green;">✅ You have the required items!</p>`;
						dialogueHTML += `<button name="send" value="/rpg npcaction ${npc.id}" class="button">🎁 Give Items</button> `;
					} else {
			...
			case 'heal':
				...
				dialogueHTML += `<button name="send" value="/rpg npcaction ${npc.id}" class="button">💊 Heal Party</button> `;
				break;
			case 'choosestarter':
				if (player.party.length > 0) {
					...
					dialogueHTML += `<button name="send" value="/rpg starterchoice ${npc.id}" class="button">👀 View Available Starters</button> `;
				}
				break;
			}
		} else {
...
export function generateNPCActionHTML(npc: any, result: any, player: PlayerData, tempSlot?: ActivePokemonSlot): string {
	let resultHTML = `<div class="infobox"><h2>${npc.name}</h2>`;
	switch (npc.action.type) {
	case 'giveitem':
		resultHTML += `<p>"Here you go!"</p><p style="color: green;">✅ You received <strong>${result.itemName} x${result.quantity}</strong>!</p>`;
		break;
	case 'givepokemon':
		resultHTML += `<p>"Take good care of this Pokémon!"</p><p style="color: green;">✅ <strong>${result.speciesName}</strong> joined your party!</p>`;
		if (tempSlot) resultHTML += generatePokemonInfoHTML(tempSlot, true);
		break;
	case 'exchangeitems':
		resultHTML += `<p>"Here's your trade!"</p><p style="color: green;">✅ Traded <strong>${result.requiredItemName} x${result.requiredQuantity}</strong> for <strong>${result.rewardItemName} x${result.rewardQuantity}</strong>!</p>`;
		break;
	case 'takeitem':
		resultHTML += `<p>"Thank you so much!"</p><p style="color: green;">✅ Gave <strong>${result.itemName} x${result.quantity}</strong> to ${npc.name}!</p><p style="color: gold;">💰 Received <strong>₽${result.reward}</strong> as thanks!</p>`;
		break;
	case 'heal':
		resultHTML += `<p>"${result.message}"</p><p style="color: green;">✅ Your Pokémon have been restored to full health!</p>`;
		break;
	default:
		resultHTML += `<p style="color: orange;">⚠️ This NPC action type (${npc.action.type}) is not yet integrated into the command system.</p><p><em>The handler exists in npc-actions.ts but needs to be wired up here.</em></p>`;
		break;
	}
	resultHTML += `<hr />` +
		`<p><button name="send" value="/rpg npc ${npc.id}" class="button">Talk Again</button> ` +
...
export function generateStarterChoiceHTML(npc: any): string {
	...
	for (const starterId of allStarters) {
		...
		if (species.exists) {
			html += `<div style="text-align: center; padding: 10px; border: 1px solid #ccc; border-radius: 5px;">` +
				`<strong>${species.name}</strong><br>` +
				`<small>Type: ${species.types.join('/')}</small><br>` +
				`<button name="send" value="/rpg starterchoice ${npc.id} ${starterId}" class="button" style="margin-top: 5px;">Choose ${species.name}</button>` +
				`</div>`;
		}
	}
	html += `</div><p style="margin-top: 15px;"><button name="send" value="/rpg npc ${npc.id}" class="button">← Back</button></p>` +
		generateBottomNavigation() +
		`</div>`;
	return html;
}

export function generateStarterChoiceConfirmationHTML(npc: any, result: any, tempSlot: ActivePokemonSlot): string {
...
export function generateDBSaveConfirmationHTML(player: PlayerData): string {
	return `<div class="infobox">` +
...
export function generateDBLoadConfirmationHTML(loadedPlayer: PlayerData): string {
	return `<div class="infobox">` +
...
export function generateDBLoadErrorHTML(): string {
	return `<div class="infobox">` +
		`<h2>No Save Found</h2>` +
...
export function generateDBDeleteConfirmationHTML(): string {
	return `<div class="infobox">` +
...
export function generateHelpHTML(): string {
	const helpList = [
	...
	return `<div class="infobox" style="max-height: 380px; overflow-y: auto;"><center><strong>RPG Commands</strong></center><hr><ul style="list-style-type:none;padding-left:0;">` +
...
export function generateCatchErrorHTML(): string {
...
export function generateCatchSuccessHTML(
	...
	if (ballId === 'healball') {
		successMessage += `<p>${caughtPokemon.species} was fully healed!</p>`;
	}
