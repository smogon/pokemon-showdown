...
const formatTime = (date: Date): string => date.toISOString().replace('T', ' ').slice(0, 19);

const getDurationMs = (value: number, unit: string): number => {
	const units: { [key: string]: number } = { minute: 60000, hour: 3600000, day: 86400000 };
...
export class ExpSystem {
	...
	private static isOnCooldown(userid: string): boolean {
		return Date.now() - (this.cooldowns[userid] || 0) < EXP_COOLDOWN;
	}

	static async writeExp(userid: string, amount: number): Promise<void> {
		const id = toID(userid);
		await ExpDB.upsert({ _id: id }, { _id: id, exp: amount, level: this.getLevel(amount), lastUpdated: new Date() });
	}

	static async readExp(userid: string): Promise<number> {
		const doc = await ExpDB.findOne({ _id: toID(userid) });
		return doc ? doc.exp : DEFAULT_EXP;
	}

	static async hasExp(userid: string, amount: number): Promise<boolean> {
		return (await ExpDB.findOne({ _id: toID(userid), exp: { $gte: amount } })) !== null;
	}

	static async hasLevel(userid: string, level: number): Promise<boolean> {
		return (await ExpDB.findOne({ _id: toID(userid), level: { $gte: level } })) !== null;
	}

	static async addExp(userid: string, amount: number, reason?: string, by?: string): Promise<number> {
		const id = toID(userid);
		if (!by && this.isOnCooldown(id)) return await this.readExp(id);
		...
		await ExpDB.findOneAndUpdate(
			{ _id: id },
			{ $inc: { exp: gainedAmount }, $set: { level: newLevel, lastUpdated: new Date() }, $setOnInsert: { _id: id } },
			{ upsert: true, returnDocument: 'after' }
		);

		if (!by) this.cooldowns[id] = Date.now();
		if (newLevel > currentLevel) this.notifyLevelUp(id, newLevel, currentLevel);
		return newExp;
	}
	...
	static notifyLevelUp(userid: string, newLevel: number, oldLevel: number): void {
		...
		user.popup(
			...
			`<div style="font-size: 1.2em; margin: 10px 0;">You are now <b style="color: #e74c3c;">Level ${newLevel}</b>!</div>` +
			`<div style="margin: 10px 0; font-style: italic;">You advanced from Level ${oldLevel} to Level ${newLevel}</div>` +
			`<div style="margin-top: 15px; font-size: 0.9em; opacity: 0.8;">Keep chatting and participating to earn more ${EXP_UNIT}!</div>` +
			`</div>`
		);
	}
	...
	static checkDoubleExpStatus(room?: Room | null, user?: User): void {
		...
		const message = DOUBLE_EXP ?
			`<div class="broadcast-green"><b>Double EXP has been enabled${user ? ` by ${nameColor(user.name, true, true)}` : ''}!</b><br>` +
			`Duration: ${DOUBLE_EXP_END_TIME ? `until ${formatTime(new Date(DOUBLE_EXP_END_TIME))} UTC` : 'No duration specified'}<br>` +
			`All EXP gains will now be doubled.</div>` :
			`<div class="broadcast-green"><b>Double EXP has been ${DOUBLE_EXP_END_TIME ? 'ended' : 'disabled'}${user ? ` by ${nameColor(user.name, true, true)}` : ''}!</b><br>` +
			`All EXP gains will now be normal.</div>`;
	...
	static async takeExp(userid: string, amount: number, reason?: string, by?: string): Promise<number> {
		...
		const result = await ExpDB.findOneAndUpdate(
			...
			{ $inc: { exp: -amount }, $set: { lastUpdated: new Date() } },
			{ returnDocument: 'after' }
		);

		if (result) {
			const newLevel = this.getLevel(result.exp);
			if (result.level !== newLevel) await ExpDB.updateOne({ _id: id }, { $set: { level: newLevel } });
			return result.exp;
		}
	...
	static getLevel(exp: number): number {
		...
		let level = 1, totalExp = MIN_LEVEL_EXP;
		while (exp >= totalExp) {
	...
	static getExpForNextLevel(level: number): number {
		...
		for (let i = 1; i < level; i++) totalExp += Math.floor(MIN_LEVEL_EXP * MULTIPLIER ** i);
		return totalExp;
	}
}
...
export const pages: Chat.PageTable = {
	async expladder(args, user): Promise<string> {
		...
		const data = richest.map(([userid, exp], index) => [
			...
			ExpSystem.getLevel(exp).toString(),
		]);

		return generateThemedTable(`Exp Ladder`, ['Rank', 'User', 'Level'], data);
	},
};
	...
	exp: {
		...
		async level(target, room, user): Promise<void> {
			...
			this.sendReplyBox(
				...
				`EXP needed for Level ${currentLevel + 1}: ${nextLevelExp - currentExp} ${EXP_UNIT}`
			);
		},
		...
		async give(target, room, user): Promise<void> {
			...
			if (targetUser.connected) {
				targetUser.popup(
					...
					`Reason: ${reason}<br>You are now Level ${newLevel} (${newExp}/${expForNext} ${EXP_UNIT})`
				);
			}
		},
		...
		async reset(target, room, user): Promise<void> {
			...
			this.sendReplyBox(`${nameColor(user.name, true, true)} reset ${nameColor(targetUser.name, true, true)}'s EXP to ${DEFAULT_EXP} ${EXP_UNIT} (Level 0) (${reason}).`);

			this.modlog('RESETEXP', targetUser, `${DEFAULT_EXP} ${EXP_UNIT}`, { by: user.id, reason });
			if (targetUser.connected) {
				targetUser.popup(
					`|html|Your ${EXP_UNIT} has been reset to <b>${DEFAULT_EXP}</b> (Level 0) by <b>${nameColor(user.name, true, true)}</b>.<br>Reason: ${reason}`
				);
			}
		},
		...
		async resetall(target, room, user): Promise<void> {
			...
			if (room) {
				room.add(
					...
					`<b>${nameColor(user.name, true, true)}</b> has reset all ${EXP_UNIT} to <b>${DEFAULT_EXP}</b> (Level 0).<br>Reason: ${reason}` +
					`</div></center>`
				).update();
			}
		},
		...
		toggledouble(target, room, user): void {
			...
			if (!match) return this.errorReply('Invalid format. Use: number + unit (minutes/hours/days)');
		...
		help(target, room, user): void {
			if (!this.runBroadcast()) return;
			const cmds = [
				["/exp level [user]", "Check a user's EXP, level, and EXP needed for the next level. (Default: yourself)"],
				["/exp give [user], [amount], [reason]", `Give a specified amount of ${EXP_UNIT} to a user. Requires: &.`],
			...
			this.sendReplyBox(
				`<center><strong>EXP System Commands:</strong></center><hr><ul style="list-style-type:none;padding-left:0;">` +
