// Pokemon Showdown - Clans Interface & Types (@author PrinceSky-Git)
export type DefaultClanRankID = 'owner' | 'leader' | 'officer' | 'member';
export type ClanPermission = 'canInvite' | 'canDeinvite' | 'canKick' | 'canPromote' | 'canDemote' | 'canEditDesc' | 'canEditIcon' | 'canEditTag' | 'canSetMotw' | 'canManageChat' | 'canEditRanks' | 'canAnnounce' | 'canWar';
...
export interface Clan {
	...
	tag: string;
	owner: ID;
	members: {[userid: string]: ClanMember};
	ranks: {[rankId: string]: CustomClanRank};
	created: number;
	desc: string;
	memberOfTheWeek: ID;
	inviteOnly: boolean;
...
export type ClanLogType = 'CREATE' | 'DELETE' | 'JOIN' | 'LEAVE' | 'KICK' | 'INVITE' | 'DEINVITE' | 'PROMOTE' | 'DEMOTE' | 'SET_MOTW' | 'SET_DESC' | 'SET_TAG' | 'SET_ICON' | 'SET_INVITEONLY' | 'RANK_CREATE' | 'RANK_DELETE' | 'ADMIN_POINTS' | 'ADMIN_TOURWIN' | 'ADMIN_EVENTWIN' | 'ADMIN_CLEARMEMBERS' | 'ADMIN_TRANSFEROWNER' | 'ADMIN_CLEARINVITES' | 'ADMIN_KICKALL' | 'ADMIN_SETICON' | 'ADMIN_SETTAG' | 'ADMIN_SETDESC' | 'ADMIN_RESETSTATS' | 'ADMIN_REMOVELEVEL' | 'ADMIN_ADDLEVEL' | 'ADMIN_SETLEVEL' | 'RANK_EDIT';
