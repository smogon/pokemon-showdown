// Pokemon Showdown - Clans Commands
import {Clans, UserClans, ClanLogs, ClanBans, ClanPointsLogs, ClanWars, ClanBattleLogs, type ClanDoc} from './database';
import type {Clan, ClanPermissions, CustomClanRank, ClanStats} from './interface';
import {generateThemedTable} from '../../utils';
import {K_FACTOR, getExpectedScore, calculateElo, to, toDurationString, logClanActivity, hasClanPermission} from './utils';
import {FS} from '../../../lib';
import {warCommands} from './war-commands';

export const ALL_PERMISSIONS: ClanPermissions = {canInvite: true, canDeinvite: true, canKick: true, canPromote: true, canDemote: true, canEditDesc: true, canEditIcon: true, canEditTag: true, canSetMotw: true, canManageChat: true, canEditRanks: true, canAnnounce: true, canWar: true};

export const DEFAULT_RANKS: {[rankId: string]: CustomClanRank} = {
	owner: {id: 'owner' as ID, name: 'Owner', permissionLevel: 100, permissions: ALL_PERMISSIONS},
	leader: {id: 'leader' as ID, name: 'Leader', permissionLevel: 50, permissions: {canInvite: true, canDeinvite: true, canKick: true, canPromote: true, canDemote: true, canEditDesc: true, canSetMotd: true, canManageChat: true, canAnnounce: true, canWar: true}},
	officer: {id: 'officer' as ID, name: 'Officer', permissionLevel: 25, permissions: {canInvite: true, canDeinvite: true}},
	member: {id: 'member' as ID, name: 'Member', permissionLevel: 10, permissions: {}}};
export const DEFAULT_STATS: ClanStats = {tourWins: 0, eventWins: 0, totalPointsEarned: 0, clanBattleWins: 0, clanBattleLosses: 0, elo: 1000, lastWarChallenge: 0};
...
export const commands: Chat.ChatCommands = {
	clan: {
		async create(target, room, user) {
			...
			if (!ownerId) return this.errorReply("Specify clan name and owner's ID: /clan create [Clan Name], [Owner ID]");
			const clanId = toID(clanName);
			if (!clanId) return this.errorReply("Specify a clan name.");
			if (clanId.length < 3 || clanId.length > 20) return this.errorReply("Clan ID must be 3-20 characters.");
			if (clanName.length > 30) return this.errorReply("Clan name must be 30 characters or less.");
			const ownerUser = Users.getExact(ownerId);
			if (!ownerUser) return this.errorReply(`Owner '${ownerId}' not found. User must be online.`);
			const [existingClan, ownerClanInfo] = await Promise.all([Clans.findOne({_id: clanId}), UserClans.findOne({_id: ownerId})]);
			if (existingClan) return this.errorReply(`Clan '${clanId}' exists.`);
			if (ownerClanInfo?.memberOf) return this.errorReply(`'${ownerId}' is already in a clan.`);
			const now = Date.now();
			const chatRoomId = toID(`${clanId}`) as RoomID;
			const newClan: ClanDoc = {_id: clanId, name: clanName, tag: clanId.slice(0, 5).toUpperCase(), owner: ownerId, members: {[ownerId]: {rank: 'owner' as ID, joinDate: now, totalPointsContributed: 0}}, ranks: DEFAULT_RANKS, created: now, desc: `Welcome to ${clanName}!`, memberOfTheWeek: '' as ID, inviteOnly: false, invites: [], points: 0, level: 1, chatRoom: chatRoomId, icon: '', lastActive: now, stats: DEFAULT_STATS};
			try {
				await Clans.insertOne(newClan);
				await UserClans.upsert({_id: ownerId}, {$set: {memberOf: clanId}});
				await logClanActivity(clanId, user.id, 'CREATE', {target: ownerId, note: `Clan ${clanName} created for ${ownerId}.`});
				let newRoom = Rooms.get(chatRoomId);
				if (!newRoom) {
					const roomSettings: RoomSettings = {title: clanName, auth: {}, creationTime: Date.now(), modjoin: '+', desc: newClan.desc};
					newRoom = Rooms.createChatRoom(chatRoomId, clanName, roomSettings);
				...
				if (ownerUser.connected) {
					...
					ownerUser.popup(`|html|<div class="infobox"><div class="infobox-message">${user.name} created clan <b>${clanName}</b> for you! You are owner (#).</div><br /><center><button class="button" name="join" value="${newRoom.roomid}">Go to Clan Room: #${newRoom.roomid}</button></center></div>`);
				}
				this.sendReply(`Clan "${clanName}" created! Owner: ${ownerId}.`);
				this.room.add(`|html|<div class="infobox"><center>Clan "${clanName}" created! Owner: ${ownerId}.</center></div>`);
			} catch (e) {
				this.errorReply("Error creating clan.");
		...
		async delete(target, room, user) {
			...
			if (!clanId) return this.errorReply("Specify a clan ID.");
			...
			try {
				if (chatRoom) {
					chatRoom.add(`|html|<div class="broadcast-red"><b>Clan room being deleted by ${user.name}.</b></div>`).update();
					await new Promise<void>(resolve => setTimeout(resolve, 1000));
					for (const userid in chatRoom.users) chatRoom.users[userid].leaveRoom(chatRoom);
					Rooms.global.deregisterChatRoom(chatRoom.roomid);
					Rooms.global.delistChatRoom(chatRoom.roomid);
					chatRoom.destroy();
					FS('config/chatrooms.json').writeUpdate(() => JSON.stringify(Rooms.global.settingsList).replace(/\{"title":/g, '\n{"title":').replace(/\]$/, '\n]'));
				}
				await Clans.deleteOne({_id: clanId});
				...
				await ClanBattleLogs.deleteMany({$or: [{winningClan: clanId}, {losingClan: clanId}]});
				await ClanWars.deleteMany({clans: clanId});
				if (ownerUser?.connected) {
					ownerUser.popup(`|html|<div class="broadcast-red"><b>Clan "${clan.name}" was deleted by ${user.name}.</b><br />All data, members, logs, and room ${clan.chatRoom} removed.</div>`);
				}
				const memberIds = Object.keys(clan.members);
				for (const memberId of memberIds) {
					if (memberId === ownerId) continue;
					const member = Users.getExact(memberId);
					if (member?.connected) member.popup(`|html|<div class="broadcast-red"><b>Clan "${clan.name}" was deleted by ${user.name}.</b></div>`);
				}
				this.sendReply(`Clan "${clan.name}" (${clanId}) deleted.`);
				Monitor.log(`[Clans] ${user.name} deleted clan: ${clan.name} (${clanId}) with ${memberIds.length} members`);
			} catch (e) {
				this.errorReply("Error deleting clan. Deletion may be partial.");
				Monitor.crashlog(e as Error, "Clan deletion command", {clanId, clanName: clan.name, user: user.name});
				this.privateModAction(`(CRITICAL: Clan deletion failed for ${clanId}. Manual verification required.)`);
			}
		},
		async join(target, room, user) {
			this.checkChat();
			if (!user.named) return this.errorReply("Login to join clan.");
			const clanId = toID(target);
			const userId = user.id;
			if (!clanId) return this.errorReply("Specify clan ID to join.");
			const [clan, userClanInfo] = await Promise.all([Clans.findOne({_id: clanId}), UserClans.findOne({_id: userId})]);
			if (!clan) return this.errorReply(`Clan '${clanId}' not found.`);
			if (userClanInfo?.memberOf) return this.errorReply(`You're already in clan '${userClanInfo.memberOf}'.`);
			const isInvited = clan.invites.some(invite => invite.userid === userId);
			if (clan.inviteOnly && !isInvited) return this.errorReply(`Clan '${clan.name}' is invite-only.`);
			if (!clan.inviteOnly && userClanInfo?.invites?.includes(clanId)) {
				await UserClans.updateOne({_id: userId}, {$pull: {invites: clanId}});
			}
			await Clans.updateOne({_id: clanId}, {$pull: {invites: {userid: userId}}});
			await Clans.updateOne({_id: clanId}, {$set: {[`members.${userId}`]: {rank: 'member' as ID, joinDate: Date.now(), totalPointsContributed: 0}}});
			await UserClans.upsert({_id: userId}, {$set: {memberOf: clanId}, $pull: {invites: clanId}});
			await logClanActivity(clanId, userId, 'JOIN', {target: userId, note: `User joined.`});
			...
			if (clanRoom) {
				...
				clanRoom.add(`|html|<div class="infobox">${user.name} joined and granted Voice.</div>`).update();
			}
			this.sendReply(`Joined clan '${clan.name}'!`);
			if (clanRoom) this.sendReply(`Auto-joined chatroom: #${clanRoom.roomid}`);
		},
		async leave(target, room, user) {
			this.checkChat();
			if (!user.named) return this.errorReply("Login to leave clan.");
			const userId = user.id;
			const userClanInfo = await UserClans.findOne({_id: userId});
			const clanId = userClanInfo?.memberOf;
			if (!clanId) return this.errorReply("You're not in a clan.");
			const clan = await Clans.findOne({_id: clanId});
			if (!clan) return this.errorReply(`Clan '${clanId}' not found.`);
			if (clan.owner === userId) return this.errorReply("You're owner. Transfer ownership first or delete clan.");
			await Clans.updateOne({_id: clanId}, {$unset: {[`members.${userId}`]: ""}});
			await UserClans.updateOne({_id: userId}, {$unset: {memberOf: 1}});
			await logClanActivity(clanId, userId, 'LEAVE', {target: userId, note: `User left.`});
			const clanRoom = Rooms.get(clan.chatRoom);
			if (clanRoom) {
				clanRoom.auth.delete(userId);
				clanRoom.saveSettings();
				clanRoom.add(`|html|<div class="infobox"><center>${user.name} left.</center></div>`).update();
			}
			user.leaveRoom(clanRoom.roomid, this.connection);
			this.sendReply(`Left clan '${clan.name}'.`);
		},
		async kick(target, room, user) {
			this.checkChat();
			if (!user.named) return this.errorReply("Login to kick users.");
			const targetId = toID(target);
			const kickerId = user.id;
			if (!targetId) return this.errorReply("Specify user to kick.");
			if (targetId === kickerId) return this.errorReply("Can\'t kick yourself.");
			...
			if (!hasClanPermission(clan, kickerId, 'canKick')) {
				return this.errorReply(`Rank (${clan.members[kickerId]?.rank}) lacks permission to kick users.`);
			}
			if (!clan.members[targetId]) {
				return this.errorReply(`'${targetId}' not in ${clan.name}.`);
			}
			if (clan.owner === targetId) {
				return this.errorReply("Can\'t kick owner.");
			}
			...
			if (targetRank.permissionLevel >= kickerRank.permissionLevel) {
				return this.errorReply("Can\'t kick users equal/higher rank than yours.");
			}
			await Clans.updateOne({_id: clanId },
				{ $unset: { [`members.${targetId}`]: "" } });
			await UserClans.updateOne({_id: targetId },
				{ $unset: { memberOf: 1 } });
			await logClanActivity(clanId, kickerId, 'KICK', {
				target: targetId,
				note: `User was kicked.`});
			const clanRoom = Rooms.get(clan.chatRoom);
			if (clanRoom) {
				...
				clanRoom.add(`|html|<div class="infobox"><center>${targetId} was kicked from clan by ${user.name}.</center></div>`).update();
			}
			const targetUser = Users.getExact(targetId);
			if (targetUser?.connected) {
				targetUser.popup(`|html|<div class="infobox">Kicked from clan <b>${clan.name}</b> by ${user.name}.</div>`);
			}
			this.sendReply(`Kicked '${targetId}' from ${clan.name}.`);
		},
		async invite(target, room, user) {
			this.checkChat();
			if (!user.named) return this.errorReply("Login to send invites.");
			const targetId = toID(target);
			const inviterId = user.id;
			if (!targetId) return this.errorReply("Specify user to invite.");
			if (targetId === inviterId) return this.errorReply("Can\'t invite yourself.");
			...
			if (targetClanInfo?.memberOf) {
				return this.errorReply(`'${targetId}' is in clan '${targetClanInfo.memberOf}'.`);
			}
			const clan = await Clans.findOne({_id: clanId });
			if (!clan) return this.errorReply(`Clan '${clanId}' not found.`);
			if (!hasClanPermission(clan, inviterId, 'canInvite')) {
				return this.errorReply(`Rank (${clan.members[inviterId]?.rank}) lacks permission to invite users.`);
			}
			const targetUser = Users.getExact(targetId);
			if (clan.invites.some(invite => invite.userid === targetId)) {
				return this.errorReply(`'${targetId}' already invited to ${clan.name}.`);
			}
			const newInvite = {
				userid: targetId,
				actor: inviterId,
			...
			await UserClans.upsert({_id: targetId },
				{ $addToSet: { invites: clanId } });
			await logClanActivity(clanId, inviterId, 'INVITE', {
				target: targetId,
				note: `Invited user.`});
			this.sendReply(`Invited '${targetId}' to join ${clan.name}.`);
			if (targetUser?.connected) {
				const buttonHtml = `<button class="button" name="send" value="/clan join ${clanId}">Join ${clan.name}</button>`;
				targetUser.popup(`|html|<div class="infobox"><div class="infobox-message">Invited to clan <b>${clan.name}</b> by ${user.name}.</div><br /><center>${buttonHtml}</center></div>`);
			} else {
				this.sendReply(`'${targetId}' offline, will see invite on login.`);
			}
			const clanRoom = Rooms.get(clan.chatRoom);
			if (clanRoom) {
				clanRoom.add(`|html|<div class="infobox"><center>${user.name} invited ${targetId}.</center></div>`).update();
			}
		},
		async deinvite(target, room, user) {
			this.checkChat();
			if (!user.named) return this.errorReply("Login to revoke invites.");
			const targetId = toID(target);
			const actorId = user.id;
			if (!targetId) return this.errorReply("Specify user to deinvite.");
			...
			if (!hasClanPermission(clan, actorId, 'canDeinvite')) {
				return this.errorReply(`Rank (${clan.members[actorId]?.rank}) lacks permission to revoke invites.`);
			}
			const inviteExists = clan.invites.some(invite => invite.userid === targetId);
			if (!inviteExists) {
				return this.errorReply(`'${targetId}' not invited to ${clan.name}.`);
			}
			await Clans.updateOne({_id: clanId },
			...
			await logClanActivity(clanId, actorId, 'DEINVITE', {
				target: targetId,
				note: `Revoked user's invite.`});
			this.sendReply(`Revoked ${targetId}'s invite to ${clan.name}.`);
			const targetUser = Users.getExact(targetId);
			if (targetUser?.connected) {
				targetUser.popup(`|html|<div class="infobox">Your invite to join clan <b>${clan.name}</b> has been revoked by ${user.name}.</div>`);
			}
			const clanRoom = Rooms.get(clan.chatRoom);
			if (clanRoom) {
				clanRoom.add(`|html|<div class="infobox"><center>${user.name} revoked ${targetId}'s invite.</center></div>`).update();
			}
		},
		async invites(target, room, user) {
			this.runBroadcast();
			this.checkChat();
			if (!user.named) return this.errorReply("Login to check invites.");
			const userId = user.id;
			const userClanInfo = await UserClans.findOne({_id: userId });
		...
		async inviteonly(target, room, user) {
			this.checkChat();
			if (!user.named) return this.errorReply("Login to toggle invite-only mode.");
			...
			if (value === 'on' || value === 'true' || value === '1') {
				...
				return this.errorReply("/clan inviteonly [on/off/toggle]");
			}
			const actorClanInfo = await UserClans.findOne({_id: actorId });
			const clanId = actorClanInfo?.memberOf;
			if (!clanId) return this.errorReply("You\'re not in a clan.");
			...
			if (!hasClanPermission(clan, actorId, 'canManageChat')) {
				return this.errorReply(`Rank (${clan.members[actorId]?.rank}) lacks permission to change invite-only mode.`);
			}
			...
			if (clanRoom) {
				...
				clanRoom.add(`|html|<div class="infobox"><center>${user.name} changed clan setting: The clan ${statusText}.</center></div>`).update();
			}
		...
		async announce(target, room, user) {
			this.checkChat();
			if (!user.named) return this.errorReply("Login to announce.");
			const message = target.trim();
			if (!message) return this.errorReply("Specify a message to announce.");
			const actorId = user.id;
			const actorClanInfo = await UserClans.findOne({_id: actorId });
			const clanId = actorClanInfo?.memberOf;
			if (!clanId) return this.errorReply("You\'re not in a clan.");
			...
			if (!hasClanPermission(clan, actorId, 'canAnnounce')) {
				return this.errorReply(`Rank (${clan.members[actorId]?.rank}) lacks permission to send announcements.`);
			}
			const memberIds = Object.keys(clan.members);
			let sentTo = 0;
			for (const memberId of memberIds) {
				...
				if (memberUser?.connected) {
					memberUser.popup(`|html|<div class="infobox"><b>Announcement</b><br /><hr />${message}<br /><hr /><small>Sent by ${user.name} (${clan.name})</small></div>`);
					sentTo++;
				}
			}
			await logClanActivity(clanId, actorId, 'ANNOUNCE', {
				note: `Announcement sent to ${sentTo} member(s): "${message}"`});
			this.sendReply(`Announcement sent to ${sentTo} online member(s) .`);
		},
		async createrank(target, room, user) {
			this.checkChat();
			if (!user.named) return this.errorReply("Login to create ranks.");
			const [rankId, rankName, permissionLevel] = target.split(',').map(s => s.trim());
			const actorId = user.id;
			if (!rankId || !rankName || !permissionLevel) {
				return this.errorReply("/clan createrank [rank id], [rank name], [level]");
			}
			...
			if (isNaN(permLevel) || permLevel < 0 || permLevel > 99) {
				return this.errorReply("Level must be a number between 0 and 99.");
			}
			...
			if (!hasClanPermission(clan, actorId, 'canEditRanks')) {
				return this.errorReply(`Rank (${clan.members[actorId]?.rank}) lacks permission to create ranks.`);
			}
			if (clan.ranks[rankIdFormatted]) {
				return this.errorReply(`A rank with ID '${rankIdFormatted}' exists.`);
			}
			const actorRank = clan.ranks[clan.members[actorId].rank];
			if (permLevel >= actorRank.permissionLevel) {
				return this.errorReply("Can\'t create ranks lvequal to or higher than your own.");
			}
			...
			await logClanActivity(clanId, actorId, 'RANK_CREATE', {
				...
				note: `Created rank '${rankName}' lv${permLevel}.`});
			this.sendReply(`Created the rank '${rankName}' (${rankIdFormatted}) lv${permLevel}.`);
			const clanRoom = Rooms.get(clan.chatRoom);
			if (clanRoom) {
				clanRoom.add(`|html|<div class="infobox"><center>${user.name} created rank '${rankName}' lv${permLevel}.</center></div>`).update();
			}
		},
		async deleterank(target, room, user) {
			this.checkChat();
			if (!user.named) return this.errorReply("Login to delete ranks.");
			const rankId = toID(target);
			const actorId = user.id;
			if (!rankId) return this.errorReply("Specify rank ID you wish to delete.");
			const actorClanInfo = await UserClans.findOne({_id: actorId });
			const clanId = actorClanInfo?.memberOf;
			if (!clanId) return this.errorReply("You\'re not in a clan.");
			const clan = await Clans.findOne({_id: clanId });
			if (!clan) return this.errorReply(`Clan '${clanId}' not found.`);
			if (!hasClanPermission(clan, actorId, 'canEditRanks')) {
				return this.errorReply(`Rank (${clan.members[actorId]?.rank}) lacks permission to delete ranks.`);
			}
			if (!clan.ranks[rankId]) {
				return this.errorReply(`Rank '${rankId}' does not exist.`);
			}
			if (['owner', 'leader', 'officer', 'member'].includes(rankId)) {
				return this.errorReply("Can\'t delete default ranks.");
			}
			const rankToDelete = clan.ranks[rankId];
			const actorRank = clan.ranks[clan.members[actorId].rank];
			if (rankToDelete.permissionLevel >= actorRank.permissionLevel) {
				return this.errorReply("Can\'t delete ranks lvequal to or higher than your own.");
			}
			const membersWithRank = Object.entries(clan.members).filter(([, member]) => member.rank === rankId);
			if (membersWithRank.length > 0) {
				return this.errorReply(`Cannot delete rank '${rankId}' because ${membersWithRank.length} member(s) have this rank. Reassign them first.`);
			}
			await Clans.updateOne({_id: clanId },
				{ $unset: { [`ranks.${rankId}`]: "" } });
			...
			this.sendReply(`Deleted the rank '${rankToDelete.name}' (${rankId}).`);
			const clanRoom = Rooms.get(clan.chatRoom);
			if (clanRoom) {
		...
		async editrank(target, room, user) {
			this.checkChat();
			if (!user.named) return this.errorReply("Login to edit ranks.");
			...
			if (!clan) return this.errorReply(`Clan '${clanId}' not found.`);
			if (!hasClanPermission(clan, actorId, 'canEditRanks')) {
				return this.errorReply(`Rank (${clan.members[actorId]?.rank}) lacks permission to edit ranks.`);
			}
			if (!clan.ranks[rankIdFormatted]) {
				return this.errorReply(`Rank '${rankIdFormatted}' does not exist.`);
			}
			if (rankIdFormatted === 'owner') {
				return this.errorReply("Can\'t edit the owner rank permissions.");
			}
			const rankToEdit = clan.ranks[rankIdFormatted];
			const actorRank = clan.ranks[clan.members[actorId].rank];
			if (rankToEdit.permissionLevel >= actorRank.permissionLevel) {
				return this.errorReply("Can\'t edit ranks lvequal to or higher than your own.");
			}
			...
			this.sendReply(`Edited rank '${rankToEdit.name}': ${permission} now ${permValue}.`);
			const clanRoom = Rooms.get(clan.chatRoom);
			if (clanRoom) {
		...
		async ranks(target, room, user) {
			this.runBroadcast();
			this.checkChat();
			if (!user.named) return this.errorReply("Login to view ranks.");
			const actorId = user.id;
			const actorClanInfo = await UserClans.findOne({_id: actorId });
			const clanId = actorClanInfo?.memberOf;
			if (!clanId) return this.errorReply("You\'re not in a clan.");
		...
		async promote(target, room, user) {
			this.checkChat();
			if (!user.named) return this.errorReply("Login to promote users.");
			const [targetUsername, newRankId] = target.split(',').map(s => s.trim());
			const targetId = toID(targetUsername);
			const actorId = user.id;
			const newRank = toID(newRankId);
			if (!targetId || !newRank) {
				return this.errorReply("/clan promote [username], [rank id]");
			}
			if (targetId === actorId) return this.errorReply("Can\'t promote yourself.");
			const actorClanInfo = await UserClans.findOne({_id: actorId });
			const clanId = actorClanInfo?.memberOf;
			if (!clanId) return this.errorReply("You\'re not in a clan.");
			const clan = await Clans.findOne({_id: clanId });
			...
			if (!hasClanPermission(clan, actorId, 'canPromote')) {
				return this.errorReply(`Rank (${clan.members[actorId]?.rank}) lacks permission to promote users.`);
			}
			if (!clan.members[targetId]) {
				return this.errorReply(`'${targetId}' not in ${clan.name}.`);
			}
			if (clan.owner === targetId) {
				return this.errorReply("Can\'t promote owner.");
			}
			...
			if (targetNewRank.permissionLevel >= actorRank.permissionLevel) {
				return this.errorReply("Can\'t promote users to a rank equal/higher than yours.");
			}
			if (targetCurrentRank.permissionLevel >= actorRank.permissionLevel) {
				return this.errorReply("Can\'t promote users with a rank equal/higher than yours.");
			}
			if (targetNewRank.permissionLevel <= targetCurrentRank.permissionLevel) {
				return this.errorReply("The new rank must have higher level than the current rank. Use /clan demote to lower rank.");
			}
			...
			const targetUser = Users.getExact(targetId);
			if (targetUser?.connected) {
				targetUser.popup(`|html|<div class="infobox">Promoted to <b>${targetNewRank.name}</b> in ${clan.name} by ${user.name}.</div>`);
			}
			this.sendReply(`Promoted '${targetId}' from ${targetCurrentRank.name} to ${targetNewRank.name}.`);
		},
		async demote(target, room, user) {
			this.checkChat();
			if (!user.named) return this.errorReply("Login to demote users.");
			const [targetUsername, newRankId] = target.split(',').map(s => s.trim());
			const targetId = toID(targetUsername);
			const actorId = user.id;
			const newRank = toID(newRankId);
			if (!targetId || !newRank) {
				return this.errorReply("/clan demote [username], [rank id]");
			}
			if (targetId === actorId) return this.errorReply("Can\'t demote yourself.");
			const actorClanInfo = await UserClans.findOne({_id: actorId });
			const clanId = actorClanInfo?.memberOf;
			if (!clanId) return this.errorReply("You\'re not in a clan.");
			const clan = await Clans.findOne({_id: clanId });
			...
			if (!hasClanPermission(clan, actorId, 'canDemote')) {
				return this.errorReply(`Rank (${clan.members[actorId]?.rank}) lacks permission to demote users.`);
			}
			if (!clan.members[targetId]) {
				return this.errorReply(`'${targetId}' not in ${clan.name}.`);
			}
			if (clan.owner === targetId) {
				return this.errorReply("Can\'t demote owner.");
			}
			if (!clan.ranks[newRank]) {
				return this.errorReply(`Rank '${newRank}' does not exist.`);
			}
			...
			const targetNewRank = clan.ranks[newRank];
			if (targetNewRank.permissionLevel >= actorRank.permissionLevel) {
				return this.errorReply("Can\'t demote users to a rank equal/higher than yours.");
			}
			if (targetCurrentRank.permissionLevel >= actorRank.permissionLevel) {
				return this.errorReply("Can\'t demote users with a rank equal/higher than yours.");
			}
			if (targetNewRank.permissionLevel >= targetCurrentRank.permissionLevel) {
				return this.errorReply("The new rank must have lower level than the current rank. Use /clan promote to raise rank.");
			}
			const oldRank = clan.members[targetId].rank;
			await Clans.updateOne({_id: clanId },
				{ $set: { [`members.${targetId}.rank`]: newRank as ID } });
			...
			const targetUser = Users.getExact(targetId);
			if (targetUser?.connected) {
				targetUser.popup(`|html|<div class="infobox">Demoted to <b>${targetNewRank.name}</b> in ${clan.name} by ${user.name}.</div>`);
			}
			this.sendReply(`Demoted '${targetId}' from ${targetCurrentRank.name} to ${targetNewRank.name}.`);
		},
		async transfer(target, room, user) {
			this.checkChat();
			if (!user.named) return this.errorReply("Login to transfer ownership.");
			const targetId = toID(target);
			const actorId = user.id;
			if (!targetId) return this.errorReply("Specify user to transfer ownership to.");
			if (targetId === actorId) return this.errorReply("You\'re already the owner.");
			const actorClanInfo = await UserClans.findOne({_id: actorId });
			const clanId = actorClanInfo?.memberOf;
			if (!clanId) return this.errorReply("You\'re not in a clan.");
			const clan = await Clans.findOne({_id: clanId });
			...
			if (clan.owner !== actorId) {
				return this.errorReply("Only owner can transfer ownership.");
			}
			if (!clan.members[targetId]) {
				return this.errorReply(`'${targetId}' not in ${clan.name}.`);
			}
			...
			const targetUser = Users.getExact(targetId);
			if (targetUser?.connected) {
				targetUser.popup(`|html|<div class="infobox">You now owner of <b>${clan.name}</b>! Ownership from ${user.name}.</div>`);
			}
			this.sendReply(`Transferred ownership of ${clan.name} to '${targetId}'. You are now a Leader.`);
		},
		async members(target, room, user) {
			this.runBroadcast();
			this.checkChat();
			if (!user.named) return this.errorReply("Login to view clan members.");
			let clanId: ID;
			if (target) {
				clanId = toID(target);
			} else {
		...
		async setdesc(target, room, user) {
			this.checkChat();
			if (!user.named) return this.errorReply("Login to set clan description.");
			const description = target.trim();
			const actorId = user.id;
			if (!description) return this.errorReply("Specify a description.");
			...
			if (!hasClanPermission(clan, actorId, 'canEditDesc')) {
				return this.errorReply(`Rank (${clan.members[actorId]?.rank}) lacks permission to edit clan description.`);
			}
			...
			if (clanRoom) {
				...
				clanRoom.saveSettings();
				clanRoom.add(`|html|<div class="infobox"><center>${user.name} updated desc.</center></div>`).update();
			}
			this.sendReply(`Updated clan description to: "${description}"`);
		},
		async settag(target, room, user) {
			this.checkChat();
			if (!user.named) return this.errorReply("Login to set clan tag.");
			const tag = target.trim().toUpperCase();
			const actorId = user.id;
			if (!tag) return this.errorReply("Specify a tag.");
			...
			if (!hasClanPermission(clan, actorId, 'canEditTag')) {
				return this.errorReply(`Rank (${clan.members[actorId]?.rank}) lacks permission to edit clan tag.`);
			}
			...
			const clanRoom = Rooms.get(clan.chatRoom);
			if (clanRoom) {
				clanRoom.add(`|html|<div class="infobox"><center>${user.name} updated clan tag from "${oldTag}" to "${tag}".</center></div>`).update();
			}
			this.sendReply(`Updated clan tag from "${oldTag}" to "${tag}".`);
		},
		async setmotw(target, room, user) {
			this.checkChat();
			if (!user.named) return this.errorReply("Login to set MOTW.");
			const targetId = toID(target);
			const actorId = user.id;
			if (!targetId) return this.errorReply("Specify a user.");
			const actorClanInfo = await UserClans.findOne({_id: actorId });
			const clanId = actorClanInfo?.memberOf;
			if (!clanId) return this.errorReply("You\'re not in a clan.");
			const clan = await Clans.findOne({_id: clanId });
			...
			if (!hasClanPermission(clan, actorId, 'canSetMotw')) {
				return this.errorReply(`Rank (${clan.members[actorId]?.rank}) lacks permission to set MOTW.`);
			}
			if (!clan.members[targetId]) {
				return this.errorReply(`'${targetId}' not in ${clan.name}.`);
			}
			if (clan.memberOfTheWeek === targetId) {
				return this.errorReply(`'${targetId}' is already the MOTW for ${clan.name}.`);
			}
			...
			if (clanRoom) {
				...
				clanRoom.saveSettings();
				clanRoom.add(`|html|<div class="infobox"><center>${user.name} set <b>${targetId}</b> as the MOTW!</center></div>`).update();
			}
			const targetUser = Users.getExact(targetId);
			if (targetUser?.connected) {
				targetUser.popup(`|html|<div class="infobox">Now <b>MOTW</b> in ${clan.name} by ${user.name}!</div>`);
			}
			this.sendReply(`Set '${targetId}' as the MOTW for ${clan.name}.`);
		},
		async seticon(target, room, user) {
			this.checkChat();
			if (!user.named) return this.errorReply("Login to set clan icon.");
			const iconUrl = target.trim();
			const actorId = user.id;
			if (!iconUrl) return this.errorReply("Specify an icon URL.");
			...
			if (!hasClanPermission(clan, actorId, 'canEditIcon')) {
				return this.errorReply(`Rank (${clan.members[actorId]?.rank}) lacks permission to edit clan icon.`);
			}
			...
			const clanRoom = Rooms.get(clan.chatRoom);
			if (clanRoom) {
				clanRoom.add(`|html|<div class="infobox"><center>${user.name} updated clan icon.</center></div>`).update();
			}
			this.sendReply(`Updated clan icon.`);
		},
		...
		async logs(target, room, user) {
			this.checkChat();
			if (!user.named) return this.errorReply("Login to view clan logs.");
			...
			if (clanId) {
				...
				if (!clanId) {
					return this.errorReply(
						"You are not currently a member of any clan. /clan logs [clan id], [limit] (admin only)"
					);
				}
			}
			const clan = await Clans.findOne({_id: clanId });
			...
			if (logs.length === 0) {
				return user.popup(`|html|<div class="infobox"><center>No logs for ${clan.name}.</center></div>`);
			}
		...
		async pointslog(target, room, user) {
			this.checkChat();
			if (!user.named) return this.errorReply("Login to view clan points logs.");
			...
			const clan = await Clans.findOne({_id: clanId });
			if (!clan) return this.errorReply(`Clan '${clanId}' not found.`);
			const logs = await ClanPointsLogs.find({ clanId },
				{ limit, sort: { timestamp: -1 } });
			if (!logs.length) {
				return user.popup(`|html|<div class="infobox"><center>No points logs for ${clan.name}.</center></div>`);
			}
		...
		async profile(target, room, user) {
			this.runBroadcast();
			this.checkChat();
			if (!user.named) return this.errorReply("Login to view clan profiles.");
			let clanId: ID;
			if (target) {
				clanId = toID(target);
			} else {
			...
			html += `<strong>MOTW:</strong> ${motwName}<br />`;
		...
		async battlelogs(target, room, user) {
			...
			const clan = await Clans.findOne({_id: clanId });
			if (!clan) return this.errorReply(`Clan '${clanId}' not found.`);
			const logs = await ClanBattleLogs.find({ $or: [{ winningClan: clanId }, { losingClan: clanId }] },
				{ limit: 50, sort: { timestamp: -1 } });
			if (!logs.length) {
				return this.sendReplyBox(`No battle logs for ${clan.name}.`);
			}
		...
		async addpoints(target, room, user) {
			...
			if (isNaN(amount) || amount <= 0) {
				return this.errorReply(
					"/clan addpoints [amount], [reason] (amount must be a positive number)"
				);
			}
			const actorClanInfo = await UserClans.findOne({_id: user.id });
			const clanId = actorClanInfo?.memberOf;
		...
		async removepoints(target, room, user) {
			...
			if (isNaN(amount) || amount <= 0) {
				return this.errorReply(
					"/clan removepoints [amount], [reason] (amount must be a positive number)"
				);
			}
			const actorClanInfo = await UserClans.findOne({_id: user.id });
			const clanId = actorClanInfo?.memberOf;
		...
		async setlevel(target, room, user) {
			...
			if (isNaN(level) || level < 1) {
				return this.errorReply(
					"/clan setlevel [level] (level must be a positive integer)"
				);
			}
			const actorClanInfo = await UserClans.findOne({_id: user.id });
			const clanId = actorClanInfo?.memberOf;
		...
		async addlevel(target, room, user) {
			...
			if (isNaN(amount) || amount <= 0) return this.errorReply("/clan addlevel [amount] (must be positive integer)");
			const actorClanInfo = await UserClans.findOne({_id: user.id });
			const clanId = actorClanInfo?.memberOf;
			if (!clanId) return this.errorReply("You\'re not in a clan.");
			const clan = await Clans.findOne({_id: clanId });
		...
		async removelevel(target, room, user) {
			...
			if (isNaN(amount) || amount <= 0) {
				return this.errorReply(
					"/clan removelevel [amount] (must be positive integer)"
				);
			}
			const actorClanInfo = await UserClans.findOne({_id: user.id });
			const clanId = actorClanInfo?.memberOf;
		...
		async setdescadmin(target, room, user) {
			...
			if (!clanId || !desc) return this.errorReply("/clan setdescadmin [clan id], [desc]");
		...
		async settagadmin(target, room, user) {
			...
			if (!clanId || !tag) return this.errorReply("/clan settagadmin [clan id], [tag]");
		...
		async seticonadmin(target, room, user) {
			...
			if (!clanId || !iconUrl) return this.errorReply("/clan seticonadmin [clan id], [icon url]");
		...
		async kickall(target, room, user) {
			...
			if (!clanId) return this.errorReply("/clan kickall [clan id]");
			const clan = await Clans.findOne({_id: clanId });
			if (!clan) return this.errorReply(`Clan '${clanId}' not found.`);
			const ownerId = clan.owner;
			...
			await UserClans.updateMany({ memberOf: clanId, _id: { $in: membersToKick } },
				{ $unset: { memberOf: 1 } });
			await ClanLogs.insertOne({
				clanId,
				timestamp: Date.now(),
		...
		async clearinvites(target, room, user) {
			this.checkCan('roomowner');
			const clanId = toID(target.trim());
			if (!clanId) return this.errorReply("/clan clearinvites [clan id]");
			const clan = await Clans.findOne({_id: clanId });
			if (!clan) return this.errorReply(`Clan '${clanId}' not found.`);
			const invitedUserIds = clan.invites.map(i => i.userid);
			await Clans.updateOne({_id: clanId }, { $set: { invites: [] } });
			if (invitedUserIds.length) {
				await UserClans.updateMany({_id: { $in: invitedUserIds } },
					{ $pull: { invites: clanId } });
			}
			await ClanLogs.insertOne({
				clanId,
		...
		async export(target, room, user) {
			this.checkCan('roomowner');
			const clanId = toID(target.trim());
			if (!clanId) return this.errorReply("/clan export [clan id]");
			const clan = await Clans.findOne({_id: clanId });
			if (!clan) return this.errorReply(`Clan '${clanId}' not found.`);
		...
		async transferadmin(target, room, user) {
			...
			if (!clanId || !newOwnerId) return this.errorReply("/clan transferadmin [clan id], [new owner]");
			const clan = await Clans.findOne({_id: clanId });
			if (!clan) return this.errorReply(`Clan '${clanId}' not found.`);
			if (!clan.members[newOwnerId]) return this.errorReply(`User '${newOwnerId}' not in the clan.`);
		...
		async banuser(target, room, user) {
			...
			if (!bannedId) return this.errorReply("/clan banuser [username]");
			await ClanBans.upsert({_id: bannedId }, { $set: { banned: true } });
			this.sendReply(`User '${bannedId}' now banned from joining clans.`);
		},
		async unbanuser(target, room, user) {
			this.checkCan('roomowner');
			const bannedId = toID(target.trim());
			if (!bannedId) return this.errorReply("/clan unbanuser [username]");
		...
		async clearlogs(target, room, user) {
			this.checkCan('roomowner');
			const clanId = toID(target.trim());
			if (!clanId) return this.errorReply("/clan clearlogs [clan id]");
			await ClanLogs.deleteMany({ clanId });
			await ClanPointsLogs.deleteMany({ clanId });
		...
		async clearmembers(target, room, user) {
			this.checkCan('roomowner');
			const clanId = toID(target.trim());
			if (!clanId) return this.errorReply("/clan clearmembers [clan id]");
			const clan = await Clans.findOne({_id: clanId });
			if (!clan) return this.errorReply(`Clan '${clanId}' not found.`);
			const ownerId = clan.owner;
			...
			await UserClans.updateMany({ memberOf: clanId, _id: { $in: membersToRemove } },
				{ $unset: { memberOf: 1 } });
			await ClanLogs.insertOne({
				clanId,
				timestamp: Date.now(),
		...
		help() {
			...
			const helpList = [
				{ cmd: "/clan create [Clan Name], [Owner ID]", desc: "Create a new clan. Req: &." },
				{ cmd: "/clan delete [Clan ID]", desc: "Delete a clan. Req: &." },
				{ cmd: "/clan join [Clan ID]", desc: "Join a clan." },
				{ cmd: "/clan leave", desc: "Leave your current clan." },
				{ cmd: "/clan kick [user]", desc: "Remove a member." },
				{ cmd: "/clan invite [user]", desc: "Invite a user." },
				...
				{ cmd: "/clan transfer [user]", desc: "Transfer ownership." },
				...
				{ cmd: "/clan setmotw [user]", desc: "Set MOTW." },
				...
				{ cmd: "/clan addpoints [amount], [reason]", desc: "Add points to clan. Req: &." },
				{ cmd: "/clan removepoints [amount], [reason]", desc: "Remove points from clan. Req: &." },
				{ cmd: "/clan addtourwins [amount]", desc: "Add tour wins. Req: &." },
				{ cmd: "/clan removetourwins [amount]", desc: "Remove tour wins. Req: &." },
				{ cmd: "/clan addeventwins [amount]", desc: "Add event wins. Req: &." },
				{ cmd: "/clan removeeventwins [amount]", desc: "Remove event wins. Req: &." },
				{ cmd: "/clan setlevel [level]", desc: "Set clan level. Req: &." },
				{ cmd: "/clan addlevel [amount]", desc: "Add levels. Req: &." },
				{ cmd: "/clan removelevel [amount]", desc: "Remove levels. Req: &." },
				{ cmd: "/clan resetstats", desc: "Reset clan stats. Req: &." },
				{ cmd: "/clan setdescadmin [clan id], [desc]", desc: "Set description for any clan. Req: &." },
				{ cmd: "/clan settagadmin [clan id], [tag]", desc: "Set tag for any clan. Req: &." },
				{ cmd: "/clan seticonadmin [clan id], [icon url]", desc: "Set icon for any clan. Req: &." },
				{ cmd: "/clan kickall [clan id]", desc: "Kick all members except owner. Req: &." },
				{ cmd: "/clan clearinvites [clan id]", desc: "Clear all clan invites. Req: &." },
				{ cmd: "/clan export [clan id]", desc: "Export clan data. Req: &." },
				{ cmd: "/clan transferadmin [clan id], [new owner]", desc: "Transfer ownership. Req: &." },
				{ cmd: "/clan banuser [username]", desc: "Ban user from joining clans. Req: &." },
				{ cmd: "/clan unbanuser [username]", desc: "Unban user. Req: &." },
				{ cmd: "/clan clearlogs [clan id]", desc: "Clear all logs for clan. Req: &." },
				{ cmd: "/clan clearmembers [clan id]", desc: "Remove all members except owner. Req: &." },
			];
