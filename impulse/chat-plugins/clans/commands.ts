	...
	clan: {
		...
		war: {
			...
			async history(target, room, user) {
				...
				for (const war of wars) {
					...
					let result = '';
					if (myScore > opponentScore) {
						result = `<strong style="color:green;">Victory</strong>`;
					} else if (myScore < opponentScore) {
						result = `<strong style="color:red;">Defeat</strong>`;
					} else {
						result = `<strong style="color:gray;">Tie</strong>`;
					}
					...
					dataRows.push([
						...
						`${myScore} - ${opponentScore}`,
						`Best of ${war.bestOf}`,
						to(new Date(war.endDate || war.startDate), { date: true }),
					]);
				}
			...
			async stats(target, room, user) {
				...
				const [completedWars, activePendingWars] = await Promise.all([
					ClanWars.find({ clans: clanId, status: 'completed' }, {}),
					ClanWars.find({ clans: clanId, status: { $in: ['active', 'pending'] } }, {}),
				]);
				...
				for (const war of completedWars) {
					...
					if (myScore > opponentScore) warWins++;
					else if (myScore < opponentScore) warLosses++;
					else warTies++;
				}

				const totalWars = completedWars.length;
				const warWinrate = totalWars > 0 ? ((warWins / totalWars) * 100).toFixed(1) : '0.0';

				let html = `<div class="infobox" style="max-width: 500px;"><center><strong>${clan.name} War Statistics</strong></center><hr>`;
				html += `<strong>Current ELO:</strong> ${elo}<br />`;
				html += `<strong>Battle Record:</strong> ${wins}W - ${losses}L (${winrate}% win rate)<br />`;
				html += `<strong>Total Battles:</strong> ${totalBattles}<br />`;
				html += `<hr>`;
				html += `<strong>War Record:</strong> ${warWins}W - ${warLosses}L - ${warTies}T (${warWinrate}% win rate)<br />`;
				html += `<strong>Total Wars:</strong> ${totalWars}<br />`;
				html += `<strong>Active/Pending Wars:</strong> ${activePendingWars.length}<br />`;
				html += `</div>`;
			...
			async cancel(target, room, user) {
				...
				if (!hasClanPermission(myClan, user.id, 'canWar')) {
					return this.errorReply("You do not have permission in your clan to cancel war challenges.");
				}

				const targetClanId = toID(target);
				if (!targetClanId) return this.errorReply("Specify the clan ID to cancel the challenge with. Usage: /clan war cancel [clanid]");

				const war = await ClanWars.findOne({
					clans: [clanId, targetClanId], // We must be the challenger
					status: 'pending',
				});

				if (!war) {
					return this.errorReply(`You do not have a pending war challenge with '${targetClanId}'.`);
				}
				...
				this.sendReply(`You have cancelled the war challenge with ${targetClan.name}.`);
			...
			async record(target, room, user) {
				...
				if (!wars.length) {
					return this.sendReplyBox(`${clan1.name} and ${clan2.name} have never fought in a war.`);
				}
				...
				let html = `<div class="infobox" style="max-width: 400px;"><center><strong>Head-to-Head Record</strong></center><hr>`;
				html += `<center><strong>${clan1.name}</strong> vs <strong>${clan2.name}</strong></center><br />`;
				html += `<strong>Record:</strong> ${clan1Wins}W - ${clan2Wins}L - ${ties}T<br />`;
				html += `<strong>Total Wars:</strong> ${wars.length}<br />`;
				html += `</div>`;
			...
			async rematch(target, room, user) {
				...
				if (!targetClanId) return this.errorReply("Specify a clan ID for rematch. Usage: /clan war rematch [clanid], [bestof]");
				if (!bestOf) return this.errorReply("You must specify a 'Best of' number.");
				if (bestOf < 1 || bestOf % 2 === 0) {
					return this.errorReply("'Best of' must be an odd number (3, 5, 7, etc.).");
				}
				...
				if (!previousWar) {
					return this.errorReply(`Your clan has never fought ${targetClan.name}. Use /clan war challenge instead.`);
				}
				...
				// Rematch bypasses cooldown - no need to check lastWarChallenge
				// Update last war challenge timestamp
				await Clans.updateOne(
					{ _id: clanId },
					{ $set: { 'stats.lastWarChallenge': Date.now() } }
				...
				this.sendReply(`You have challenged ${targetClan.name} to a rematch (Best of ${bestOf})! Cooldown bypassed.`);

				const targetRoom = Rooms.get(targetClan.chatRoom);
				if (targetRoom) {
					targetRoom.add(`|html|<div class="infobox">Your clan has been challenged to a <strong>REMATCH - Best of ${bestOf} WAR</strong> by <strong>${myClan.name}</strong>! ` +
						`A user with permissions can use <strong>/clan war accept ${clanId}</strong> or <strong>/clan war deny ${clanId}</strong>.</div>`).update();
				}
			},
			...
			async forfeitadmin(target, room, user) {
				...
				try {
					await Promise.all([
						...
						logClanActivity(loserClanId, user.id, 'ADMIN_RESETSTATS', {
							target: winnerClanId,
							note: `[ADMIN] ${user.name} force forfeited war. ELO changed from ${Math.floor(loserOldElo)} to ${Math.floor(newLoserElo)}.`,
						}),
						logClanActivity(winnerClanId, user.id, 'ADMIN_RESETSTATS', {
							target: loserClanId,
							note: `[ADMIN] ${user.name} awarded war win. ELO changed from ${Math.floor(winnerOldElo)} to ${Math.floor(newWinnerElo)}.`,
						}),
					]);
				} catch (e) {
					this.errorReply("An error occurred while forfeiting the war. Please try again.");
					Monitor.crashlog(e as Error, "Clan War Admin Forfeit", {
						warId: war._id,
						winnerClanId,
						loserClanId,
					});
				...
				const lossMessage = `|html|<div class="broadcast-red"><strong>WAR FORFEITED. (-${eloChange} ELO)</strong><br />` +
									`Admin ${user.name} has forfeited the war. ${winnerClan.name} wins. ` +
									`New Clan ELO: ${Math.floor(newLoserElo)}</div>`;
				...
				this.sendReply(`Forfeited the war: ${loserClan.name} forfeits to ${winnerClan.name}. ELO updated.`);
			},
			...
			async extend(target, room, user) {
				...
				if (!opponentClanId) return this.errorReply("Specify the opponent clan ID. Usage: /clan war extend [clanid], [newbestof]");
				if (!newBestOf || newBestOf < 1 || newBestOf % 2 === 0) {
					return this.errorReply("'Best of' must be an odd number (3, 5, 7, etc.).");
				}
				if (newBestOf > 101) return this.errorReply("'Best of' cannot be higher than 101.");
				...
				const extendConfirmations = war.extendConfirmations || [];
				const alreadyConfirmed = extendConfirmations.some(c => c.clanId === clanId && c.newBestOf === newBestOf);

				if (alreadyConfirmed) {
					return this.errorReply(`Your clan has already confirmed extending to Best of ${newBestOf}.`);
				}

				// Check if opponent has confirmed the same extension
				const opponentConfirmed = extendConfirmations.find(c => 
					c.clanId === toID(opponentClanId) && c.newBestOf === newBestOf
				);

				if (opponentConfirmed) {
					// Both clans confirmed - extend the war
					await ClanWars.updateOne(
						{ _id: war._id },
						{ 
							$set: { bestOf: newBestOf },
							$unset: { extendConfirmations: 1 }
						}
					);

					const message = `|html|<div class="broadcast-blue"><strong>War Extended!</strong><br />Both clans agreed to extend the war to Best of ${newBestOf}.</div>`;
					...
					const opponentRoom = Rooms.get(opponentClan.chatRoom);
					if (opponentRoom) {
						opponentRoom.add(`|html|<div class="infobox"><strong>${myClan.name}</strong> has proposed to extend the war to <strong>Best of ${newBestOf}</strong>. ` +
							`A user with permissions can use <strong>/clan war extend ${clanId}, ${newBestOf}</strong> to confirm.</div>`).update();
					}
				}
			},
			...
			async pause(target, room, user) {
				...
				const opponentConfirmed = pauseConfirmations.includes(opponentClanId);

				if (opponentConfirmed) {
					// Both clans confirmed - pause the war
					await ClanWars.updateOne(
						{ _id: war._id },
						{ 
							$set: { paused: true },
							$unset: { pauseConfirmations: 1 }
						}
					);

					const message = `|html|<div class="broadcast-blue"><strong>War Paused!</strong><br />Battle tracking is now paused. Use /clan war resume to continue.</div>`;
					...
					this.sendReply(`You have proposed pausing the war. Waiting for ${opponentClan.name} to confirm.`);

					const opponentRoom = Rooms.get(opponentClan.chatRoom);
					if (opponentRoom) {
						opponentRoom.add(`|html|<div class="infobox"><strong>${myClan.name}</strong> has proposed to pause the war. ` +
							`A user with permissions can use <strong>/clan war pause ${clanId}</strong> to confirm.</div>`).update();
					}
				}
			},
			...
			async resume(target, room, user) {
				...
				// Check if opponent has confirmed
				const opponentConfirmed = resumeConfirmations.includes(opponentClanId);

				if (opponentConfirmed) {
					// Both clans confirmed - resume the war
					await ClanWars.updateOne(
						{ _id: war._id },
						{ 
							$unset: { paused: 1, resumeConfirmations: 1 }
						}
					);

					const message = `|html|<div class="broadcast-green"><strong>War Resumed!</strong><br />Battle tracking has resumed.</div>`;
					...
					this.sendReply(`You have proposed resuming the war. Waiting for ${opponentClan.name} to confirm.`);

					const opponentRoom = Rooms.get(opponentClan.chatRoom);
					if (opponentRoom) {
						opponentRoom.add(`|html|<div class="infobox"><strong>${myClan.name}</strong> has proposed to resume the war. ` +
							`A user with permissions can use <strong>/clan war resume ${clanId}</strong> to confirm.</div>`).update();
					}
				}
			},
			...
			async mvp(target, room, user) {
				...
				// Get all battle logs where this clan won
				const battleLogs = await ClanBattleLogs.find(
					{ winningClan: clanId },
					{ limit: 10000 }
				);
				...
				const playerWins: { [userId: string]: number } = {};
				for (const log of battleLogs) {
					playerWins[log.winner] = (playerWins[log.winner] || 0) + 1;
				}
				...
				const headerRow = ['Rank', 'Player', 'War Battle Wins'];
				const dataRows: string[][] = [];
				const title = `${clan.name} War Battle MVPs`;

				topPlayers.forEach(([userId, wins], i) => {
					dataRows.push([
						`<strong>${i + 1}</strong>`,
						userId,
						`<strong>${wins}</strong>`,
		...
		help() {
			...
			const helpList = [
				...
				{ cmd: "/clan war tie [clanid]", desc: "Propose or confirm ending the war as a tie (requires both clans to confirm)." },
				{ cmd: "/clan war forfeit [clanid]", desc: "Forfeit an active war, granting them the ELO win." },
				{ cmd: "/clan war forceend [clanid]", desc: "Force end an active war (admin only). Requires: &." },
				{ cmd: "/clan war forcetie [clanid]", desc: "Force tie an active war (admin only). Requires: &." },
				{ cmd: "/clan war resetcooldown [clanid]", desc: "Reset war challenge cooldown for a clan (admin only). Requires: &." },
				{ cmd: "/clan war status [clanid]", desc: "View your clan's active and pending wars." },
				{ cmd: "/clan war ladder [page], [sortby]", desc: "View the clan war ladder. Sort by: elo, wins, losses, winrate." },
