...
interface TCGConfig {
	MAX_CARD_QUANTITY: number;
	CREDITS_PER_DUPLICATE: number;
	PACK_COST: number;
	HIT_CHANCE: number;
	MAX_FAVORITE_CARDS: number;
}

declare const global: { __impulseTCGConfig?: TCGConfig; };

if (!global.__impulseTCGConfig) {
	global.__impulseTCGConfig = {
		MAX_CARD_QUANTITY: 10,
		CREDITS_PER_DUPLICATE: 1,
		PACK_COST: 0,
		HIT_CHANCE: 0.9,
		MAX_FAVORITE_CARDS: 10,
	};
}

export const TCGConfig = global.__impulseTCGConfig;
...
export async function addCardsToCollection(user: User, pack: TcgCard[]): Promise<{ creditsAwarded: number }> {
	...
	for (const [cardId, { card, count }] of cardCounts.entries()) {
		...
		if (newQty > TCGConfig.MAX_CARD_QUANTITY) {
			const excess = newQty - TCGConfig.MAX_CARD_QUANTITY;
			creditsToAward += (excess * TCGConfig.CREDITS_PER_DUPLICATE);
			finalQty = TCGConfig.MAX_CARD_QUANTITY;
		}
...
async function generatePackFromCache(setId: string): Promise<TcgCard[]> {
  ...
  try {
    ...
    if (Math.random() < TCGConfig.HIT_CHANCE) {
