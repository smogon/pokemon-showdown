...
export const collectionCommands: ChatCommands = {
	async collection(target, room, user) {
		...
		try {
			const { filter, queryDescription, page, commandString, targetUserId } = parseCollectionQuery(target, user.id); // Now defined locally
	...
	async recalculatestats(target, room, user) {
		...
		if (targetUserId) {
			// If a user is specified, only admins can run it for others
			this.checkCan('bypassall');
		} else {
			// If no user specified, defaults to the command user
