// Pokemon Showdown - TCG Utilities - @author PrinceSky-Git
...
export const MAX_PACK_SIZE_FOR_SETS_CALC = 50;
...
const COMMON_R = ['Common'];
const UNCOMMON_R = ['Uncommon'];
const REVERSE_R = [
	...
	'Promo', 'Black Star Promo',
];
export const RAREST_R = [
...
const FALLBACK_R = ['Common'];

function getCardPool(card: TcgCard): RarityPool | null {
	if (RAREST_R.includes(card.rarity)) return 'rarest';
	if (REVERSE_R.includes(card.rarity)) return 'reverseRare';
	if (UNCOMMON_R.includes(card.rarity)) return 'uncommon';
	if (COMMON_R.includes(card.rarity)) return 'common';
	return null;
}

export async function initializeCache(): Promise<{ cardCount: number, setCount: number }> {
	console.log('TCG cache init...');
	clearCache();
	const collection = ImpulseDB<TcgCard>('tcg_cards');
	const allCards = await collection.find({});
	if (allCards.length === 0) {
		console.error('TCG cache init failed: no cards in DB');
	...
	for (const card of allCards) {
		...
		if (COMMON_R.includes(card.rarity) || FALLBACK_R.includes(card.rarity)) {
	...
	console.log(`TCG cache complete: ${cardsCache.size} cards, ${setsCache.size} sets`);
...
export async function calculateSetsCompleted(userId: string): Promise<number> {
	const allSets = await tcgCardsCollection.aggregate<{
	...
	const userSetCounts = await userCollectionsCollection.aggregate<{ _id: string, uniqueCount: number }>([
		{ $match: { userId } },
...
export async function addCardsToCollection(user: User, pack: TcgCard[]): Promise<{ creditsAwarded: number }> {
	...
	if (totalQty > 0 || totalUnique > 0 || credits > 0) {
		...
		const shouldRecalculateSets = totalUnique > 0 && pack.length < MAX_PACK_SIZE_FOR_SETS_CALC;
...
function generatePackFromCache(setId: string): TcgCard[] {
	...
	try {
		...
		if (pack.length < MAX_PACK_SIZE) {
			throw new Error(`Could not generate full ${MAX_PACK_SIZE}-card pack. DB may lack unique cards across fallback pools`);
		}
		return pack;
	} catch (err) {
...
async function getRandomCardsDB(
	collection: ImpulseCollection<TcgCard>, setId: string | null, rarities: string[],
	size: number, excludeIds: string[] = []
): Promise<TcgCard[]> {
...
async function getWeightedRandomCardDB(
	collection: ImpulseCollection<TcgCard>, setId: string, rarities: string[], excludeIds: string[] = []
...
async function generatePackFromDB(setId: string): Promise<TcgCard[]> {
	...
	try {
		const [commons, uncommons] = await Promise.all([
			getRandomCardsDB(tcgCardsCollection, setId, COMMON_R, 3, excludeIds),
			getRandomCardsDB(tcgCardsCollection, setId, UNCOMMON_R, 3, excludeIds),
		]);
		pack.push(...commons, ...uncommons);
		excludeIds = pack.map(c => c.cardId);
		const rare = await getRandomCardsDB(tcgCardsCollection, setId, REVERSE_R, 1, excludeIds);
		if (rare.length > 0) {
			pack.push(rare[0]);
			excludeIds.push(rare[0].cardId);
		} else {
			const fb = await getRandomCardsDB(tcgCardsCollection, setId, FALLBACK_R, 1, excludeIds);
			if (fb.length > 0) {
				pack.push(fb[0]);
				excludeIds.push(fb[0].cardId);
			}
		...
		if (Math.random() < HIT_CHANCE) {
			eighth = await getWeightedRandomCardDB(tcgCardsCollection, setId, RAREST_R, excludeIds);
		}
		if (!eighth) {
			const fb = await getRandomCardsDB(tcgCardsCollection, setId, REVERSE_R, 1, excludeIds);
			if (fb.length > 0) eighth = fb[0];
		}
		if (!eighth) {
			const fb = await getRandomCardsDB(tcgCardsCollection, setId, FALLBACK_R, 1, excludeIds);
		...
		if (missing > 0) {
			excludeIds = pack.map(c => c.cardId);
			const promoFill = await getRandomCardsDB(tcgCardsCollection, setId, REVERSE_R, missing, excludeIds);
			pack.push(...promoFill);
			const stillMissing = MAX_PACK_SIZE - pack.length;
			if (stillMissing > 0) {
				excludeIds = pack.map(c => c.cardId);
				pack.push(...await getRandomCardsDB(tcgCardsCollection, null, FALLBACK_R, stillMissing, excludeIds));
			}
		}
		if (pack.length > MAX_PACK_SIZE) pack.length = MAX_PACK_SIZE;
		if (pack.length < MAX_PACK_SIZE) {
			...
			if (finalMissing > 0) {
				excludeIds = pack.map(c => c.cardId);
				pack.push(...await getRandomCardsDB(tcgCardsCollection, null, FALLBACK_R, finalMissing, excludeIds));
			}
		}
		if (pack.length < MAX_PACK_SIZE) {
			throw new Error(`Could not generate full ${MAX_PACK_SIZE}-card pack. DB may lack unique cards across fallback pools`);
