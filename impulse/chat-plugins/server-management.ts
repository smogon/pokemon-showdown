...
export const commands: Chat.ChatCommands = {
  ...
  async terminal(target, room, user) {
    this.canUseConsole();
    
    const command = target.trim();
    
    if (!command) {
      return this.errorReply('Usage: /terminal [command]\nExample: /terminal ls -la');
    }
    ...
    for (const dangerous of dangerousCommands) {
      if (lowerCommand.includes(dangerous.toLowerCase())) {
        return this.errorReply('⚠️ This command is blocked for safety reasons.');
      }
    }
    
    try {
      this.sendReply(`Executing: ${command}`);
      
      // Set a timeout of 30 seconds for command execution
      ...
      let resultMessage = '<div class="infobox">';
      resultMessage += '<strong>[TERMINAL]</strong><br>';
      resultMessage += `<strong>Command:</strong> <code>${Chat.escapeHTML(command)}</code><br>`;
      resultMessage += `<strong>User:</strong> <username>${user.id}</username><br><br>`;
      
      if (stdout) {
        resultMessage += '<strong>Output:</strong><br>';
        resultMessage += '<details open><summary>Show/Hide Output</summary>';
        resultMessage += '<pre style="background: #1e1e1e; color: #4ec9b0; padding: 10px; border-radius: 4px; max-height: 400px; overflow: auto; font-size: 12px;">';
        resultMessage += Chat.escapeHTML(stdout);
        resultMessage += '</pre>';
        resultMessage += '</details>';
      }
      ...
      if (stderr) {
        resultMessage += '<br><strong>Error Output:</strong><br>';
        resultMessage += '<details open><summary>Show/Hide Errors</summary>';
        resultMessage += '<pre style="background: #2d2d2d; color: #f48771; padding: 10px; border-radius: 4px; max-height: 400px; overflow: auto; font-size: 12px;">';
        resultMessage += Chat.escapeHTML(stderr);
        resultMessage += '</pre>';
        resultMessage += '</details>';
      }
      ...
      if (!stdout && !stderr) {
        resultMessage += '<em>Command executed successfully with no output.</em>';
      }
      
      resultMessage += '</div>';
      
      this.sendReplyBox(resultMessage);
      notifyStaff("Terminal command executed", command, user);
      
    } catch (err: any) {
      const errorMsg = err.message || err.toString();
      
      let errorMessage = '<div class="message-error">';
      errorMessage += '<strong>❌ Command Failed</strong><br><br>';
      
      if (err.killed) {
        errorMessage += '⚠️ <strong>Command timed out (30s limit exceeded)</strong><br><br>';
      }
      
      errorMessage += `<strong>Command:</strong> <code>${Chat.escapeHTML(command)}</code><br><br>`;
      errorMessage += '<strong>Error:</strong><br>';
      errorMessage += '<pre style="background: #2d2d2d; color: #f48771; padding: 10px; border-radius: 4px; max-height: 300px; overflow: auto;">';
      errorMessage += Chat.escapeHTML(errorMsg);
      errorMessage += '</pre>';
      errorMessage += '</div>';
      
      this.sendReplyBox(errorMessage);
      notifyStaff("Terminal command failed", command, user, errorMsg.slice(0, 200));
    }
  },
  ...
  githelp(target, room, user) {
    ...
    this.sendReplyBox(
      ...
      `<ul>` +
      `<li><code>/gitconfig [name], [email]</code> - Configure git identity (required before first commit)</li>` +
      ...
      `1. <code>/gitconfig Your Name, your@email.com</code> - Set your identity<br><br>` +
  ...
  terminalhelp(target, room, user) {
    if (!this.runBroadcast()) return;
    this.sendReplyBox(
      `<div><b><center>Terminal Commands</center></b><br>` +
      `<ul>` +
      `<li><code>/terminal [command]</code> OR <code>/exec [command]</code> - Execute any Ubuntu terminal command</li>` +
      `</ul>` +
      `<small>All commands require Console/Owner permission.</small><br>` +
      `<small>⚠️ Commands timeout after 30 seconds. Max output: 5MB.</small><br>` +
      `<small>🛡️ Dangerous commands (rm -rf /, dd, etc.) are blocked for safety.</small><br><br>` +
      `<strong>Examples:</strong><br>` +
      `• <code>/terminal ls -la</code> - List files<br>` +
      `• <code>/terminal pm2 status</code> - Check PM2 status<br>` +
      `• <code>/terminal systemctl status mongod</code> - Check MongoDB status<br>` +
      `• <code>/terminal df -h</code> - Check disk space<br>` +
      `• <code>/terminal free -h</code> - Check memory usage<br>` +
      `• <code>/terminal ps aux | grep node</code> - Find Node processes<br>` +
      `</div>`
    );
  },
  ...
  fmhelp(target, room, user) {
    ...
    this.sendReplyBox(
      ...
      `<li><code>/terminalhelp</code> - View terminal execution commands</li>` +
      `</ul>` +
      `<small>All commands require Console/Owner permission.</small>` +
      `</div>`
    );
