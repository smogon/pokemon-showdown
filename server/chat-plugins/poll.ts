...
export class Poll {
	...
	deselect(user: User, option: number) {
		...
		if (!pendingVote || !pendingVote.includes(option)) {
			return user.sendTo(this.room, this.room.translate`That option is not selected.`);
		}
	...
	submit(user: User) {
		...
		if (userid in this.voters || ip in this.voterIps) {
			...
			return user.sendTo(this.room, this.room.translate`You have already voted for this poll.`);
		}
		const selected = this.pendingVotes[userid];
		if (!selected) return user.sendTo(this.room, this.room.translate`No options selected.`);
	...
	generateVotes(user: User | null) {
		const iconText = this.isQuiz ?
			`<i class="fa fa-question"></i> ${this.room.translate`Quiz`}` :
			`<i class="fa fa-bar-chart"></i> ${this.room.translate`Poll`}`;
		...
		if (this.multiPoll) {
			...
			const submitButton = pendingVotes.length ? `<button class="button" value="/poll submit" name="send" title="${this.room.translate`Submit your vote`}"><strong>${this.room.translate`Submit`}</strong></button>` : `<button class="button" value="/poll results" name="send" title="${this.room.translate`View results - you will not be able to vote after viewing results`}">(${this.room.translate`View results`})</button`;
			output += `<div style="margin-top: 7px; padding-left: 12px">${submitButton}</div>`;
			output += `</div>`;
		} else {
			for (const [num, option] of this.options) {
				output += `<div style="margin-top: 5px"><button class="button" style="text-align: left" value="/poll vote ${num}" name="send" title="${this.room.translate`Vote for ${num}`}. ${Utils.escapeHTML(option.name)}">${num}. <strong>${this.getOptionMarkup(option)}</strong></button></div>`;
			}
			output += `<div style="margin-top: 7px; padding-left: 12px"><button value="/poll results" name="send" title="${this.room.translate`View results - you will not be able to vote after viewing results`}"><small>(${this.room.translate`View results`})</small></button></div>`;
			output += `</div>`;
		}
	...
	generateResults(ended = false, option: number[] | null = null) {
		const iconText = this.isQuiz ?
			`<i class="fa fa-question"></i> ${this.room.translate`Quiz`}` :
			`<i class="fa fa-bar-chart"></i> ${this.room.translate`Poll`}`;
		const icon = `<span style="border:1px solid #${ended ? '777;color:#555' : '6A6;color:#484'};border-radius:4px;padding:0 3px">${iconText}${ended ? ' ' + this.room.translate`ended` : ""}</span> <small>${this.totalVotes} ${this.room.translate`votes`}</small>`;
		...
		if (!option && !ended) {
			output += `div><small>(${this.room.translate`You can't vote after viewing results`})</small></div>`;
		}
		output += '</div>';
	...
	onConnect(user: User, connection: Connection | null = null) {
		...
		this.room.send(`|uhtmlchange|poll${this.pollNumber}|<div class="infobox">(${this.room.translate`The poll has ended &ndash; scroll down to see the results`})</div>`);
