...
class Giveaway {
	...
	constructor(host: User, giver: User, room: ChatRoom | GameRoom, ot: string, tid: string, fc: string, prize: string) {
		...
		this.joined = Object.create(null);
	...
	static getSprite(text: string): [Set<string>, string] {
		...
		const mons = new Map();
		let output = '';
		const monIDs = new Set() as Set<string>;
		...
		if (mons.size) {
			for (const [key, value] of mons) {
				...
				if (value.otherForms) {
					for (const aestheticForm of value.otherForms) {
						if (text.includes(aestheticForm)) {
							spriteid += '-' + aestheticForm.substr(key.length);
	...
	static updateStats(monIDs: Set<string>) {
		for (const mon of monIDs) {
			if (!stats[mon]) stats[mon] = [] as number[];
...
class QuestionGiveaway extends Giveaway {
	...
	winner?: User;
	wifiGiveaway: QuestionGiveaway;

	constructor(host: User, giver: User, room: ChatRoom, ot: string, tid: string, fc: string, prize: string, question: string, answers: string[]) {
		...
		this.answered = Object.create(null); // userid: number of guesses
		this.wifiGiveaway = this;
	...
	change(key: string, value: string, user: User) {
		...
		delete this.room.giveaways.wifi;
	}
...
class LotteryGiveaway extends Giveaway {
	...
	constructor(host: User, giver: User, room: ChatRoom, ot: string, tid: string, fc: string, prize: string, winners: number) {
		super(host, giver, room, ot, tid, fc, prize);
...
class GtsGiveaway {
	...
	constructor(room: ChatRoom, giver: User, amount: number, summary: string, deposit: string, lookfor: string) {
	...
	stopDeposits() {
		...
		delete this.room.giveaways.gts;
		return this.left;
	}
	...
	static linkify(text: string) {
		...
		for (const i in Dex.data.Pokedex) {
			...
			if (res) {
				const num = Dex.data.Pokedex[i].num < 100 ? (Dex.data.Pokedex[i].num < 10 ? `00${Dex.data.Pokedex[i].num}` : `0${Dex.data.Pokedex[i].num}`) : Dex.data.Pokedex[i].num;
...
export const commands: ChatCommands = {
	...
	question(target, room, user) {
		...
		if (room.giveaways.wifi) return this.errorReply("There is already a giveaway going on!");
		...
		room.giveaways.wifi = new QuestionGiveaway(user, targetUser, room as ChatRoom, ot, tid, fc, prize, question, answers);
	...
	changequestion(target, room, user, conn, cmd) {
		...
		if (!room.giveaways.wifi) return this.errorReply("There is no giveaway going on at the moment.");
		if (room.giveaways.wifi.type !== 'question') return this.errorReply("This is not a question giveaway.");

		target = target.trim();
		if (!target) return this.errorReply("You must include a question or an answer.");
		room.giveaways.wifi.change(cmd.substr(6), target, user);
	},
	showanswer: 'viewanswer',
	viewanswer(target, room, user) {
		if (room.roomid !== 'wifi') return false;
		const giveaway = room.giveaways.wifi;
	...
	guess(target, room, user) {
		...
		if (!room.giveaways.wifi) return this.errorReply("There is no giveaway going on at the moment.");
		if (room.giveaways.wifi.type !== 'question') return this.errorReply("This is not a question giveaway.");
		room.giveaways.wifi.guessAnswer(user, target);
	},
	...
	lottery(target, room, user) {
		...
		room.giveaways.wifi = new LotteryGiveaway(user, targetUser, room as ChatRoom, ot, tid, fc, prize, numWinners);
	...
	gts: {
		...
		start(target, room, user) {
			if (room.roomid !== 'wifi' || !target) return false;
			if (room.giveaways.gts) return this.errorReply("There is already a GTS giveaway going on!");

			const [giver, amountStr, summary, deposit, lookfor] = target.split(target.includes('|') ? '|' : ',').map(param => param.trim());
			...
			room.giveaways.gts = new GtsGiveaway(room as ChatRoom, targetUser, amount, summary, deposit, lookfor);
		...
		left(target, room, user) {
			if (room.roomid !== 'wifi') return false;
			if (!room.giveaways.gts) return this.errorReply("There is no GTS giveaway going on!");
			if (!user.can('warn', null, room) && user !== room.giveaways.gts.giver) return this.errorReply("Only the host or a staff member can update GTS giveaways.");
			if (!target) {
				if (!this.runBroadcast()) return;
				// @ts-ignore
				const output = `The GTS giveaway from ${room.giveaways.gts.giver} has ${room.giveaways.gts.left} Pokémon remaining!`;
				// @ts-ignore
				if (room.giveaways.gts.sent.length) output += `Last winners: ${room.giveaways.gts.sent.join(', ')}`;
			...
			if (newamount > room.giveaways.gts.left) return this.errorReply("The new amount must be lower than the old amount.");
			if (newamount < room.giveaways.gts.left - 1) this.modlog(`GTS GIVEAWAY`, null, `set from ${room.giveaways.gts.left} to ${newamount} left`);

			room.giveaways.gts.updateLeft(newamount);
		},
		sent(target, room, user) {
			...
			room.giveaways.gts.updateSent(target);
		},
		full(target, room, user) {
			if (room.roomid !== 'wifi') return false;
			if (!room.giveaways.gts) return this.errorReply("There is no GTS giveaway going on!");
			if (!user.can('warn', null, room) && user !== room.giveaways.gts.giver) return this.errorReply("Only the host or a staff member can update GTS giveaways.");
			if (room.giveaways.gts.noDeposits) return this.errorReply("The GTS giveaway was already set to not accept deposits.");

			room.giveaways.gts.stopDeposits();
		},
		end(target, room, user) {
			if (room.roomid !== 'wifi') return this.errorReply("This command can only be used in the Wi-Fi room.");
			if (!room.giveaways.gts) return this.errorReply("There is no GTS giveaway going on at the moment.");
			if (!this.can('warn', null, room)) return false;

			if (target && target.length > 300) {
				return this.errorReply("The reason is too long. It cannot exceed 300 characters.");
			}
			const amount = room.giveaways.gts.end(true);
	...
	ban(target, room, user) {
		...
		if (room.giveaways.wifi) room.giveaways.wifi.kickUser(targetUser);
	...
	stop: 'end',
		...
		if (room.roomid !== 'wifi') return this.errorReply("This command can only be used in the Wi-Fi room.");
		if (!room.giveaways.wifi) return this.errorReply("There is no giveaway going on at the moment.");
		if (!this.can('warn', null, room) && user.id !== room.giveaways.wifi.host.id) return false;

		if (target && target.length > 300) {
			return this.errorReply("The reason is too long. It cannot exceed 300 characters.");
		}
		room.giveaways.wifi.end(true);
	...
	remind(target, room, user) {
		...
		if (giveaway.type === 'question') {
			...
			room.giveaways.wifi.send(room.giveaways.wifi.generateQuestion());
		} else {
			room.giveaways.wifi.display();
		}
	},
