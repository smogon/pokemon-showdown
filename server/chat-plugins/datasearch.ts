	...
	weathergy(target, room, user, connection, cmd, message) {
		...
		const weathergies: {
			...
			sun: {
				...
				strongWindsBoosted: "The Fire-type damage multiplier becomes 1.6x, and the Water-type damage multiplier becomes 0.4x.",
			...
			rain: {
				...
				strongWindsBoosted: "The Water-type damage multiplier becomes 1.6x, and the Fire-type damage multiplier becomes 0.4x.",
			...
			bloodmoon: {
				...
				desc: "During the effect, Fairy-type moves have their damage multiplied by 0.5x. Status moves used by Dark-type Pokemon have their priority increased by 1.",
				strongWindsBoosted: "Moves with 60 or less base power used by Dark-type Pokemon always result in a critical hit, and Dark-type Pokemon are immune to Status moves that are not self-targeting.",
			...
			dragonforce: {
				...
				strongWindsBoosted: "Dragon-type attacks have their damage multiplied by 1.5x instead of 1.25x. Non-Dragon-type Pokemon still lose 10% of their maximum HP after successfully landing a damaging move.",
			...
			magnetosphere: {
				...
				desc: "During the effect, Steel-type Pokemon become non-grounded. Strong Winds-boosted Dust Storm takes priority over this effect.",
				strongWindsBoosted: "Steel-type Pokemon gain a flat 20% increase to Attack and Special Attack at the end of each turn they are active. These boosts are cleared when Strong Winds or Magnetosphere ends, or when the Pokemon switches out.",
				setupAbility: "Ferroflux",
				setupMove: "Magnetize",
			},
			strongwinds: {
				names: ["Strong Winds", "Galeforce", "Flying", "Winds", "Wind"],
				desc: "Clears all active Weathergies on activation. If other Weathergies are activated after Strong Winds, they gain the boosted effects described for as long as Strong Winds remains active. During the effect, wind moves ignore accuracy checks and the Speed of non-grounded Pokemon is multiplied by 1.25x.",
...
function runDexsearch(target: string, cmd: string, message: string, isTest: boolean) {
	...
	for (const mon of Object.values(dex).sort()) {
		...
		const isRegionalForm = (["Alola", "Galar", "Hisui", "Kaskade", "Amaze-All"].includes(mon.forme) ||
			mon.forme.startsWith("Paldea")) &&
