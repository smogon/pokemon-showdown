	...
	async randommove(target, room, user, connection, cmd, message) {
		...
		for (const arg of targets) {
			...
			if (Number.isInteger(num)) {
				...
				qty = num;
				if (qty < 1 || 30 < qty) throw new Chat.ErrorMessage("Number of random Pok\u00e9mon Moves must be between 1 and 15.");
	...
	async randompokemon(target, room, user, connection, cmd, message) {
		...
		for (const arg of targets) {
			...
			if (Number.isInteger(num)) {
				...
				qty = num;
				if (qty < 1 || 30 < qty) throw new Chat.ErrorMessage("Number of random Pok\u00e9mon must be between 1 and 15.");
				targetsBuffer.push(`random${qty}`);
			} else {
				targetsBuffer.push(arg);
			}
