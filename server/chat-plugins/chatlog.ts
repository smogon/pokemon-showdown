...
export const LogViewer = new class {
	...
	async day(roomid: RoomID, day: string, opts?: string) {
		...
		const roomid = `view-chatlog-${roomid}--${prevDay}${opts ? `--${opts}` : ''}`;
		buf += `<p><a roomid="${roomid}" class="blocklink" style="text-align:center">▲<br />${prevDay}</a></p>` +
		...
		if (day !== LogReader.today()) {
			...
			const logRoomid = `view-chatlog-${roomid}--${nextDay}${opts ? `--${opts}` : ''}`;
			buf += `<p><a roomid="${logRoomid}" class="blocklink" style="text-align:center">${nextDay}<br />▼</a></p>`;
		}
