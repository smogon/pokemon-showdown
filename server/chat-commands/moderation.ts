	...
	roomauth(target, room, user, connection, cmd) {
		...
		while (curRoom.parent) {
			...
			if (curRoom.parent) {
				...
				buffer.push(`${curRoom.title} is a ${roomType}subroom of ${curRoom.parent.title}, so ${curRoom.parent.title} users${inheritedUserType}${also} have authority in this room. The names in **bold** are online.`);
			}
		...
		if (!curRoom.settings.isPrivate) {
			buffer.push(`${curRoom.title} is a public room, so global auth with no relevant roomauth will have authority in this room. The names in **bold** are online.`);
