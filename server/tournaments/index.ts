...
function createTournament(
	...
	if (format.effectType !== 'Format' || !format.tournamentShow) {
		...
		const valid = Object.values(Dex.formats).filter(f => f.tournamentShow).map(f => f.name).sort(f => f.section).join(', ');
		output.sendReplyBox(`<div class="chat"><details class="readmore code" style="white-space: pre-wrap; display: table; tab-size: 3"><summary>Valid Formats: </summary>${valid}</details></div>`);
		return;
	}
