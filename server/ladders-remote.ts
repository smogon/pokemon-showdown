...
export class LadderStore {
	...
	async updateRating(p1name: string, p2name: string, p1score: number, room: AnyObject) {
		...
		if (error) {
			...
			room.addQueue(`|error|This server's request IP ${data.errorip} is not a registered server.`);
			room.addQueue(`|error|You should be using ladders.js and not ladders-remote.js for ladder tracking.`);
			room.update();
			problem = true;
		}
