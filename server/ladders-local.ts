...
class LadderStore {
	...
	Promise<[number, number | undefined, number | undefined] | [number, null, null] | undefined> {
		...
		try {
			...
			room.addRaw(Chat.html`${p1name}'s rating: ${Math.round(p1elo)} &rarr; ` +
			`<strong>${Math.round(p1newElo)}</strong><br />(${reasons})`);

			reasons = '' + (Math.round(p2newElo) - Math.round(p2elo)) + ' for ' + (p2score > 0.9 ? 'winning' : (p2score < 0.1 ? 'losing' : 'tying'));
			if (reasons.charAt(0) !== '-') reasons = '+' + reasons;
			room.addRaw(Chat.html`${p2name}'s rating: ${Math.round(p2elo)} &rarr; ` +
			`<strong>${Math.round(p2newElo)}</strong><br />(${reasons})`);
