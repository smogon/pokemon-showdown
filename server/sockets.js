...
if (cluster.isMaster) {
	...
	cluster.on('exit', (worker, code, signal) => {
		if (code === null && signal !== null) {
			...
			require(/** @type {any} */('../.lib-dist/crashlogger'))(new Error(`Worker ${worker.id} abruptly died with code ${code} and signal ${signal}`), "The main process");
	...
	if (Config.crashguard) {
		...
		process.on('uncaughtException', err => {
			require(/** @type {any} */('../.lib-dist/crashlogger'))(err, `Socket process ${cluster.worker.id} (${process.pid})`);
		});
	}
	...
	if (Config.ssl) {
		...
		try {
			...
			try {
				...
				require(/** @type {any} */('../.lib-dist/crashlogger'))(new Error(`Failed to read the configured SSL private key PEM file:\n${e.stack}`), `Socket process ${cluster.worker.id} (${process.pid})`);
			}
		} catch (e) {
		...
		try {
			...
			try {
				...
				require(/** @type {any} */('../.lib-dist/crashlogger'))(new Error(`Failed to read the configured SSL certificate PEM file:\n${e.stack}`), `Socket process ${cluster.worker.id} (${process.pid})`);
			}
		} catch (e) {
	...
	if (Config.wsdeflate) {
		try {
			...
			require(/** @type {any} */('../.lib-dist/crashlogger'))(new Error("Dependency permessage-deflate is not installed or is otherwise unaccessable. No message compression will take place until server restart."), "Sockets");
		}
	}
