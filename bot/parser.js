...
exports.parse = {
	...
	processChatData: function(user, room, connection, msg) {
		...
		if (config.allowmute && this.hasRank(this.ranks[room] || ' ', '%@&#~') && config.whitelist.indexOf(user) === -1) {
			...
			var stretchMatch = msg.toLowerCase().match(/(.)\1{59,}/g) || msg.toLowerCase().match(/(..+)\1{38,}/g); // matches the same character (or group of characters) 8 (or 5) or more times in a row
