...
function generateThemedTable(
  ...
  let output = `<div class="pokemon-table-container" style="max-width: 100%; max-height: 380px; overflow: auto;">`;
  output += `<h3 class="pokemon-table-title" style="max-width: 100%;">${title}</h3>`;
  if (styleBy) {
    output += `<p class="pokemon-table-by" style="max-width: 100%;">Style By ${styleBy}</p>`;
  }
  output += `<table class="pokemon-table" style="max-width: 100%;">`;
  output += `<thead><tr class="pokemon-table-header">`;
  headerRow.forEach(header => {
    output += `<th style="max-width: 100%;">${header}</th>`;
  });
  output += `</tr></thead><tbody>`;

  dataRows.forEach(row => {
    output += `<tr class="pokemon-table-row">`;
    row.forEach(cell => {
      output += `<td style="max-width: 100%;">${cell}</td>`;
    });
    output += `</tr>`;
  });
