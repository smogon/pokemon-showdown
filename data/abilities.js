...
let BattleAbilities = {
	...
	"battlebond": {
		...
		onSourceFaint(target, source, effect) {
			if (effect && effect.effectType === 'Move' && source.template.id === 'greninja' && source.hp && !source.transformed && source.side.foe.pokemonLeft) {
		...
		onModifyMove(move, attacker) {
			if (move.id === 'watershuriken' && attacker.template.name === 'Greninja-Ash') {
				move.multihit = 3;
			}
		},
		id: "battlebond",
	...
	"flowergift": {
		...
		onUpdate(pokemon) {
			if (!pokemon.isActive || pokemon.baseTemplate.baseName !== 'Cherrim' || pokemon.transformed) return;
			if (['sunnyday', 'desolateland'].includes(pokemon.effectiveWeather())) {
				if (pokemon.template.id !== 'cherrimsunshine') {
		...
		onAllyModifyAtk(atk, pokemon) {
			if (this.effectData.target.baseTemplate.baseName !== 'Cherrim') return;
			if (['sunnyday', 'desolateland'].includes(pokemon.effectiveWeather())) {
				return this.chainModify(1.5);
			}
		},
	...
	"gulpmissile": {
		...
		onDamagingHit(damage, target, source, move) {
			if (move.effectType === 'Move' && ['cramorantgulping', 'cramorantgorging'].includes(target.template.id) && !target.transformed && !target.isSemiInvulnerable()) {
				this.damage(source.baseMaxhp / 4, source, target);
				if (target.template.id === 'cramorantgulping') {
		...
		onAnyDamage(damage, target, source, effect) {
			if (effect && effect.id === 'surf' && source.hasAbility('gulpmissile') && source.template.name === 'Cramorant' && !source.transformed) {
	...
	"hungerswitch": {
		...
		onResidual(pokemon) {
			if (pokemon.template.baseName !== 'Morpeko' || pokemon.transformed) return;
			let targetForme = pokemon.template.name === 'Morpeko' ? 'Morpeko-Hangry' : 'Morpeko';
	...
	"iceface": {
		...
		onDamagePriority: 1,
		onStart(pokemon) {
			if (this.field.isWeather('hail') && pokemon.template.id === 'eiscuenoice' && !pokemon.transformed) {
		...
		onDamage(damage, target, source, effect) {
			if (effect && effect.effectType === 'Move' && effect.category === 'Physical' && target.template.id === 'eiscue' && !target.transformed) {
		...
		onEffectiveness(typeMod, target, type, move) {
			if (!target) return;
			if (move.category !== 'Physical' || target.template.id !== 'eiscue' || target.transformed || (target.volatiles['substitute'] && !(move.flags['authentic'] || move.infiltrates))) return;
			if (!target.runImmunity(move.type)) return;
			return 0;
		},
		onUpdate(pokemon) {
			if (pokemon.template.id === 'eiscue' && this.effectData.busted) {
	...
	"powerconstruct": {
		...
		onResidualOrder: 27,
		onResidual(pokemon) {
			if (pokemon.baseTemplate.baseName !== 'Zygarde' || pokemon.transformed || !pokemon.hp) return;
			if (pokemon.template.id === 'zygardecomplete' || pokemon.hp > pokemon.maxhp / 2) return;
	...
	"schooling": {
		...
		onStart(pokemon) {
			if (pokemon.baseTemplate.baseName !== 'Wishiwashi' || pokemon.level < 20 || pokemon.transformed) return;
			if (pokemon.hp > pokemon.maxhp / 4) {
				if (pokemon.template.id === 'wishiwashi') {
					pokemon.formeChange('Wishiwashi-School');
				}
			} else {
				if (pokemon.template.id === 'wishiwashischool') {
		...
		onResidualOrder: 27,
		onResidual(pokemon) {
			if (pokemon.baseTemplate.baseName !== 'Wishiwashi' || pokemon.level < 20 || pokemon.transformed || !pokemon.hp) return;
			if (pokemon.hp > pokemon.maxhp / 4) {
				if (pokemon.template.id === 'wishiwashi') {
					pokemon.formeChange('Wishiwashi-School');
				}
	...
	"shieldsdown": {
		...
		onStart(pokemon) {
			if (pokemon.baseTemplate.baseName !== 'Minior' || pokemon.transformed) return;
			if (pokemon.hp > pokemon.maxhp / 2) {
				if (pokemon.template.id === 'minior') {
		...
		onResidualOrder: 27,
		onResidual(pokemon) {
			if (pokemon.baseTemplate.baseName !== 'Minior' || pokemon.transformed || !pokemon.hp) return;
			if (pokemon.hp > pokemon.maxhp / 2) {
				if (pokemon.template.id === 'minior') {
					pokemon.formeChange('Minior-Meteor');
				}
	...
	"stancechange": {
		...
		onBeforeMove(attacker, defender, move) {
			if (attacker.template.baseName !== 'Aegislash' || attacker.transformed) return;
			if (move.category === 'Status' && move.id !== 'kingsshield') return;
			let targetSpecies = (move.id === 'kingsshield' ? 'Aegislash' : 'Aegislash-Blade');
			if (attacker.template.name !== targetSpecies) attacker.formeChange(targetSpecies);
		},
		id: "stancechange",
		name: "Stance Change",
		rating: 4.5,
	...
	"zenmode": {
		...
		onResidualOrder: 27,
		onResidual(pokemon) {
			if (pokemon.baseTemplate.baseName !== 'Darmanitan' || pokemon.transformed) {
				return;
			}
		...
		onEnd(pokemon) {
			...
			if (pokemon.template.baseName === 'Darmanitan' && pokemon.template.battleOnly) {
				pokemon.formeChange(/** @type {string} */ (pokemon.template.battleOnly), this.effect, false, '[silent]');
			}
		},
		effect: {
			onStart(pokemon) {
				if (!pokemon.template.name.includes('Galar')) {
					if (pokemon.template.id !== 'darmanitanzen') pokemon.formeChange('Darmanitan-Zen');
				} else {
					if (pokemon.template.id !== 'darmanitangalarzen') pokemon.formeChange('Darmanitan-Galar-Zen');
				}
			},
			onEnd(pokemon) {
