...
exports.BattleAbilities = {
	...
	"darkaura": {
		...
		onAnyBasePower: function(basePower, attacker, defender, move) {
			...
			for (var p in attacker.side.active) {
				if (attacker.side.active[p] && attacker.side.active[p].effectiveAbility().id === 'aurabreak') {
					reverseAura = true;
					this.debug('Reversing Dark Aura due to Aura Break');
				}
			}
			for (var p in defender.side.active) {
				if (defender.side.active[p] && defender.side.active[p].effectiveAbility().id === 'aurabreak') {
					reverseAura = true;
					this.debug('Reversing Dark Aura due to Aura Break');
				}
			}
	...
	"minus": {
		...
		onModifySpA: function(spa, pokemon) {
			...
			for (var i=0; i<allyActive.length; i++) {
				if (allyActive[i] && allyActive[i].position !== pokemon.position && !allyActive[i].fainted && (allyActive[i].effectiveAbility().id === 'minus' || allyActive[i].effectiveAbility().id === 'plus')) {
					return this.chainModify(1.5);
				}
			}
		},
	...
	"shadowtag": {
		...
		onFoeModifyPokemon: function(pokemon) {
			if (pokemon.effectiveAbility().id !== 'shadowtag') {
				pokemon.tryTrap();
			}
		},
		onFoeMaybeTrapPokemon: function(pokemon) {
	...
	"slowstart": {
		...
		effect: {
			...
			onModifyAtkPriority: 5,
			onModifyAtk: function(atk, pokemon) {
				if (pokemon.effectiveAbility().id !== 'slowstart') {
	...
	"unaware": {
		...
		onSourceModifyMove: function(move, user, target) {
			if (user.effectiveAbility().id in {'moldbreaker':1, 'turboblaze':1, 'teravolt':1}) return;
	...
	"unburden": {
		...
		effect: {
			onModifySpe: function(speMod, pokemon) {
				if (pokemon.effectiveAbility().id !== 'unburden') {
	...
	"zenmode": {
		...
		effect: {
			...
			onUpdate: function(pokemon) {
				if (pokemon.effectiveAbility().id !== 'zenmode') {
