	...
	randomSet(template, slot = 1, teamDetails = {}, isDoubles = false) {
		...
		if (abilities[1]) {
			...
			if (abilities.includes('Galvanize') && !!counter['Normal']) {
				ability = 'Galvanize';
			} else if (abilities.includes('Guts') && ability !== 'Quick Feet' && (hasMove['facade'] || (hasMove['protect'] && !template.isMega && !abilities.includes('No Guard')) || (hasMove['rest'] && hasMove['sleeptalk']))) {
				ability = 'Guts';
