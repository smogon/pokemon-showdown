exports.BattleItems = {
	...
	"apicotberry": {
		...
		onUpdate: function(pokemon) {
			if (pokemon.hp <= pokemon.maxhp/4|| (pokemon.hp <= pokemon.maxhp/2 && pokemon.effectiveAbility().id === 'gluttony')) {
				pokemon.eatItem();
			}
		},
		onEat: function(pokemon) {
	...
	"fullincense": {
		...
		onModifyPriority: function(priority, pokemon) {
			if (pokemon.effectiveAbility().id !== 'stall') {
				return priority - 0.1;
			}
		},
		num: 316,
	...
	"metronome": {
		...
		effect: {
			...
			onBeforeMove: function(pokemon, target, move) {
				if (pokemon.effectiveItem().id !== 'metronome') {
