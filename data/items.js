...
let BattleItems = {
	"abomasite": {
		...
		onTakeItem(item, source) {
			if (item.megaEvolves === source.baseTemplate.baseName) return false;
			return true;
		},
		num: 674,
		gen: 6,
	...
	"adamantorb": {
		...
		onBasePower(basePower, user, target, move) {
			if (move && user.baseTemplate.name === 'Dialga' && (move.type === 'Steel' || move.type === 'Dragon')) {
	...
	"blueorb": {
		...
		spritenum: 41,
		onSwitchIn(pokemon) {
			if (pokemon.isActive && pokemon.baseTemplate.name === 'Kyogre') {
		...
		onTakeItem(item, source) {
			if (source.baseTemplate.baseName === 'Kyogre') return false;
			return true;
		},
		itemUser: ["Kyogre"],
		num: 535,
	...
	"deepseascale": {
		...
		onModifySpD(spd, pokemon) {
			if (pokemon.baseTemplate.name === 'Clamperl') {
	...
	"eviolite": {
		...
		onModifyDef(def, pokemon) {
			...
			if (pokemon.baseTemplate.nfe || pokemon.baseTemplate.name === 'Slowpoke-Galar') {
				return this.chainModify(1.5);
			}
		},
		onModifySpDPriority: 2,
	...
	"leek": {
		...
		onModifyCritRatio(critRatio, user) {
			if (["Farfetch'd", "Sirfetch'd"].includes(user.baseTemplate.baseName)) {
				return critRatio + 2;
			}
		},
	...
	"lightball": {
		...
		onModifyAtkPriority: 1,
		onModifyAtk(atk, pokemon) {
			if (pokemon.baseTemplate.baseName === 'Pikachu') {
				return this.chainModify(2);
			}
		},
		onModifySpAPriority: 1,
	...
	"luckypunch": {
		...
		onModifyCritRatio(critRatio, user) {
			if (user.baseTemplate.name === 'Chansey') {
				return critRatio + 2;
			}
		},
		itemUser: ["Chansey"],
	...
	"lustrousorb": {
		...
		onBasePowerPriority: 6,
		onBasePower(basePower, user, target, move) {
			if (move && user.baseTemplate.name === 'Palkia' && (move.type === 'Water' || move.type === 'Dragon')) {
				return this.chainModify([0x1333, 0x1000]);
			}
		},
		itemUser: ["Palkia"],
	...
	"metalpowder": {
		...
		onModifyDefPriority: 2,
		onModifyDef(def, pokemon) {
			if (pokemon.template.name === 'Ditto' && !pokemon.transformed) {
				return this.chainModify(2);
			}
		},
		itemUser: ["Ditto"],
	...
	"redorb": {
		...
		spritenum: 390,
		onSwitchIn(pokemon) {
			if (pokemon.isActive && pokemon.baseTemplate.name === 'Groudon') {
				this.queue.insertChoice({choice: 'runPrimal', pokemon: pokemon});
			}
		},
		onPrimal(pokemon) {
		...
		onTakeItem(item, source) {
			if (source.baseTemplate.baseName === 'Groudon') return false;
			return true;
		},
		itemUser: ["Groudon"],
		num: 534,
	...
	"stick": {
		...
		spritenum: 475,
		onModifyCritRatio(critRatio, user) {
			if (user.baseTemplate.name === 'Farfetch\'d') {
				return critRatio + 2;
			}
		},
		itemUser: ["Farfetch'd"],
	...
	"thickclub": {
		...
		onModifyAtkPriority: 1,
		onModifyAtk(atk, pokemon) {
			if (pokemon.baseTemplate.baseName === 'Cubone' || pokemon.baseTemplate.baseName === 'Marowak') {
				return this.chainModify(2);
			}
		},
