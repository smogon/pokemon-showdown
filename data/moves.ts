...
export const Moves: {[moveid: string]: MoveData} = {
	...
	courtchange: {
		...
		onHitField(target, source) {
			const sideConditions = [
				'mist', 'lightscreen', 'reflect', 'spikes', 'safeguard', 'tailwind', 'toxicspikes', 'stealthrock', 'livewire', 'permafrost', 'hotcoals', 'waterpledge', 'firepledge', 'grasspledge', 'stickyweb', 'auroraveil', 'gmaxsteelsurge', 'gmaxcannonade', 'gmaxvinelash', 'gmaxwildfire',
			];
			let success = false;
	...
	defog: {
		...
		onHit(target, source, move) {
			...
			const removeAll = [
				'spikes', 'toxicspikes', 'stealthrock', 'livewire', 'permafrost', 'hotcoals', 'stickyweb', 'gmaxsteelsurge',
			];
			for (const targetCondition of removeTarget) {
				if (target.side.removeSideCondition(targetCondition)) {
					if (!removeAll.includes(targetCondition)) continue;
	...
	gmaxwindrage: {
		...
		self: {
			onHit(source) {
				let success = false;
				const removeTarget = [
					'reflect', 'lightscreen', 'auroraveil', 'safeguard', 'mist', 'spikes', 'toxicspikes', 'stealthrock', 'livewire', 'permafrost', 'hotcoals','stickyweb',
				];
				const removeAll = ['spikes', 'toxicspikes', 'stealthrock', 'livewire', 'permafrost', 'hotcoals', 'stickyweb', 'gmaxsteelsurge'];
				for (const targetCondition of removeTarget) {
	...
	relicsong: {
		...
		onAfterMoveSecondarySelf(pokemon, target, move) {
			if (move.willChangeForme) {
				...
				const meloettaForme = "-Delta-Mime"
	...
	wish: {
		...
		flags: {snatch: 1, heal: 1},
		onTry(source) {
			if (!source.side.getSlotCondition(source, 'orbitalwish')) {
				if (!source.side.addSlotCondition(source, 'wish')) return false;
			} else {
				return false;
			}
			if (source.hasAbility('periodicorbit')) {
				if (!source.side.addSlotCondition(source, 'orbitalwish')) return false;
			}
		},
		secondary: null,
		target: "self",
	...
	monkeyarms: {
		...
		beforeTurnCallback(pokemon) {
			for (const side of this.sides) {
				if (side.hasAlly(pokemon)) continue;
				side.addSideCondition('monkeyarms', pokemon);
				const data = side.getSideConditionData('monkeyarms');
				if (!data.sources) {
					data.sources = [];
				}
				data.sources.push(pokemon);
		...
		onTryHit(target, pokemon) {
			target.side.removeSideCondition('monkeyarms');
		},
		condition: {
			duration: 1,
			onBeforeSwitchOut(pokemon) {
				this.debug('Monkey Arms start');
				let alreadyAdded = false;
				pokemon.removeVolatile('destinybond');
				for (const source of this.effectState.sources) {
					if (!source.isAdjacent(pokemon) || !this.queue.cancelMove(source) || !source.hp) continue;
					if (!alreadyAdded) {
						this.add('-activate', pokemon, 'move: Monkey Arms');
						alreadyAdded = true;
					}
					// Run through each action in queue to check if the Pursuit user is supposed to Mega Evolve this turn.
					// If it is, then Mega Evolve before moving.
					...
					this.actions.runMove('monkeyarms', source, source.getLocOf(pokemon));
				}
			},
		},
		secondary: null,
