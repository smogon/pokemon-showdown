exports.BattleScripts = {
	...
	randomSet: function (template, slot, noMega) {
		...
		do {
			...
			for (var k = 0; k < moves.length; k++) {
				...
				case 'willowisp':
					...
					break;
				case 'toxic':
					if (hasMove['thunderwave'] || hasMove['willowisp'] || hasMove['yawn'] || hasMove['spore'] || hasMove['sleeppowder'] || hasMove['stunspore'] || hasMove['hypnosis'] || hasMove['nuzzle']) rejected = true;
					if (hasMove['rest'] && hasMove['sleeptalk']) rejected = true;
					break;
				}
		...
		// medium priority
			...
			item = 'Lustrous Orb';
		} else if (slot === 0 && !(ability in {Multiscale:1, Regenerator:1, Sturdy:1}) && !counter['recoil'] && template.baseStats.def + template.baseStats.spd + template.baseStats.hp < 300) {
			item = 'Focus Sash';
		...
		do {
			...
			for (var k = 0; k < moves.length; k++) {
				...
				case 'willowisp':
					...
					break;
				case 'toxic':
					if (hasMove['thunderwave'] || hasMove['willowisp'] || hasMove['scald'] || hasMove['yawn'] || hasMove['spore'] || hasMove['sleeppowder'] || hasMove['nuzzle']) rejected = true;
					break;
				}
