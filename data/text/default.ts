export const DefaultText: { [id: IDEntry]: DefaultText } = {
	...
	ui: {
		...
		usuallyMovesFirst: "Usually moves first (priority +{PRIORITY}).",
		almostAlwaysMovesFirst: "Nearly always moves first (priority +{PRIORITY}).",
		almostAlwaysMovesLast: "Nearly always moves last (priority −{PRIORITY}).",
		failsIfHP: "Fails if current HP is {HP}.",
		koSelfIfHP: "KOs yourself if current HP is exactly {HP}.",
		transformedInto: "(Transformed into {SPECIES})",
		changedForme: "(Changed forme: {SPECIES})",
		possibleIllusion: "Possible Illusion #{NUMBER}",
		pixels: "({HP}/{MAXHP} pixels)",
		wouldTakeIfAbilityRemoved: "Would take if ability removed: {PERCENT}%",
		nextDamage: "Next damage: {PERCENT}%",
		turnsAsleep: "Turns asleep: {NUMBER}",
		illusionWarning: "(More than 4 moves is usually a sign of Illusion Zoroark/Zorua.)",
		...
		revealed: "(revealed)",
		range: "{LOW} to {HIGH}",
		beforeStatStages: "(before stat stage changes)",
		beforeExternalModifiers: "(before external modifiers)",
		effectivenessVs: "{EFFECT} vs. {POKEMON}",
		basePowerVs: "{LABEL} vs. {POKEMON}",
		or: " or ",
	},
