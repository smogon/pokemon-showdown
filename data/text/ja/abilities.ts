...
export const AbilitiesText: { [id: IDEntry]: AbilityText } = {
	...
	auraguard: {
		name: "はどうのぼうご",
		shortDesc: "直接攻撃で受けるダメージが半減する。", // NEEDS QC
	},
	baddreams: {
		name: "ナイトメア",
	...
	contrary: {
		...
		gen7: {
			desc: "このポケモンは能力ランクが上がるとき代わりに下がり、下がるとき代わりに上がる。Ｚワザを使う前のＺパワーによる能力上昇には適用されない。", // NEEDS QC
		},
		gen6: {
	...
	healer: {
		...
		champions: {
			desc: "毎ターン終了時、50%の確率で味方の状態異常を治す。", // NEEDS QC
			shortDesc: "毎ターン50%の確率で味方の状態異常を治す。", // NEEDS QC
		},
		gen6: {
	...
	multitype: {
		...
		gen7: {
			shortDesc: "アルセウスなら、持っているプレートかＺクリスタルでタイプが変わる。", // NEEDS QC
		},
		gen6: {
			shortDesc: "アルセウスのタイプが持っているプレートに応じて変わる。", // NEEDS QC
		},
	...
	parentalbond: {
		...
		gen7: {
			desc: "このポケモンの攻撃技は2回攻撃する連続技になる。2回目の攻撃のダメージは1/4になる。はめつのねがい・がむしゃら・だいばくはつ・いのちがけ・なげつける・みらいよち・アイスボール・ころがる・じばく、連続攻撃技、複数を対象とする技、2ターン技、Ｚワザには発動しない。", // NEEDS QC
		},
		gen6: {
	...
	shielddust: {
		...
		desc: "このポケモンは、他のポケモンの技の追加効果を受けない。防がれる追加効果には、まひ・ねむり・こおり・やけど・どく・こんらん・ひるみ・能力ランクの低下（確率が100%のものを含む）のほか、アンカーショット、ぶきみなじゅもん、なげつける、サイコノイズ、しおづけ、かげぬい、みずあめボム、じごくづきの効果が含まれる。うたかたのアリアの効果は、このポケモンが唯一の対象である場合に防がれる。おうじゃのしるし・するどいキバや、特性のどくしゅ・あくしゅう・どくのくさりによって付与される追加効果も防ぐ。", // NEEDS QC
	...
	simple: {
		...
		gen7: {
			desc: "このポケモンの能力ランクが上がるときと下がるとき、その幅が2倍になる。変化技のＺワザを使う前のＺパワーによる能力上昇には適用されない。", // NEEDS QC
		},
		gen6: {
	...
	slowstart: {
		...
		gen7: {
			desc: "場に出ると、5ターンの間、攻撃と素早さが半分になる。効果中、特殊技に基づく汎用Ｚワザを使うと、ダメージ計算時に特攻が半分になる。", // NEEDS QC
		},
		gen6: {
