...
export class RandomGen3MegaTeams extends RandomGen3Teams {
	...
	constructor(format: string | Format, prng: PRNG | PRNGSeed | null) {
		...
		for (const formeid of this.megaFormes) {
			...
			if (!forme.exists || !this.randomSets[formeid] || !forme.requiredItems?.length) {
				throw new Error(`Missing Gen 3 Mega Random Battle data for ${formeid}`);
			}
		}
	}
...
export default RandomGen3MegaTeams;
