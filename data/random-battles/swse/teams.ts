...
const RECOVERY_MOVES = [
	'cidercannon', 'healorder', 'juicedrink', 'milkdrink', 'moonlight', 'morningsun', 'recover', 'roost', 'shoreup', 'slackoff', 'softboiled', 'strengthsap', 'synthesis',
];
// Moves that drop stats:
const CONTRARY_MOVES = [
	'armorcannon', 'closecombat', 'leafstorm', 'makeitrain', 'overheat', 'shreddingriff', 'spinout', 'superpower', 'vcreate',
];
...
const MIXED_SETUP = [
	'clangoroussoul', 'evoboost', 'growth', 'happyhour', 'holdhands', 'noretreat', 'shellsmash', 'workup',
];
...
const SETUP = [
	'acidarmor', 'agility', 'autotomize', 'bellydrum', 'bulkup', 'calmmind', 'clangoroussoul', 'coil', 'cosmicpower', 'curse', 'dragondance', 'evoboost',
	'flamecharge', 'fluffbuff', 'growth', 'honeclaws', 'howl', 'irondefense', 'meditate', 'nastyplot', 'noretreat', 'poweruppunch', 'quiverdance',
	'rockpolish', 'shellsmash', 'shiftgear', 'swordsdance', 'tailglow', 'takeheart', 'tidyup', 'trailblaze', 'workup', 'victorydance',
];
const SPEED_CONTROL = [
	'electroweb', 'glare', 'icywind', 'lowsweep', 'mockery', 'nuzzle', 'quash', 'tailwind', 'thunderwave', 'trickroom',
];
...
const NO_STAB = [
	...
	'grassyglide', 'iceshard', 'icywind', 'incinerate', 'infestation', 'lavajet', 'machpunch', 'meteorbeam', 'mockery', 'mortalspin', 'nuzzle', 'pluck', 'pursuit',
	'quickattack', 'rapidspin', 'reversal', 'selfdestruct', 'shadowsneak', 'skydrop', 'snarl', 'strugglebug', 'suckerpunch', 'trailblaze',
	'uturn', 'vacuumwave', 'voltswitch', 'watershuriken', 'waterspout', 'windtunnel',
];
// Hazard-setting moves
const HAZARDS = [
	'spikes', 'stealthrock', 'steelbarbs', 'stickyweb', 'toxicspikes',
];
// Protect and its variants
const PROTECT_MOVES = [
	'banefulbunker', 'burningbulwark', 'frostification', 'protect', 'shockshelter', 'silktrap', 'spikyshield',
];
// Moves that switch the user out
const PIVOT_MOVES = [
	'chillyreception', 'escaperoot', 'flipturn', 'partingshot', 'shedtail', 'teleport', 'uturn', 'voltswitch',
];
...
const PRIORITY_POKEMON = [
	'breloom', 'honchkrow', 'mimikyu', 'scizor',
];
...
export class RandomSwSeTeams {
	...
	getMoveType(move: Move, species: Species, abilities: string[]): string {
	...
	getAbility(
		...
		// If all abilities are rejected, prioritize weathergy abilities over non-weathergy abilities
		if (!abilityAllowed.length) {
			const weatherAbilities = abilities.filter(
				a => [
					'Absolute Zero', 'Bloomspring', 'Bubble Helm', 'Carbon Capture', 'Chlorophyll', 'Consecration', 'Corrosion', 'Droughtproof', 'Druidry', 'Dust Gather', 'Earth Force', 'Energizer', 'Evergreen', 'Flame Body', 'Forked', 'Gale Wings', 'Glacial Armor', 'Grass Pelt', 'Hydration', 'Hydrophobic', 'Indomitable', 'Leaf Guard', 'Machine Precision', 'Magma Armor', 'Magnapult', 'Malice', 'Master Instinct', 'Nanomachines', 'Neutralize', 'Nullify', 'Overcoat', 'Petrichor', 'Powder Cure', 'Power Above', 'Power Plumage', 'Power Within', 'Rage State', 'Sand Rush', 'Shadow Step', 'Slush Rush', 'Smoke and Mirrors', 'Solar Power', 'Soul Drain', 'Surge Surfer', 'Swarming', 'Sweet Dreams', 'Swift Swim', 'Trained Eye', 'Warp Mist', 'Wet and Dry', 'Wind Power', 'Wind Rider',
				].includes(a)
			);
	...
	getPokemonCompatibility(
		...
		const sunSetters = ['ninetales', 'flareon', 'torkoal', 'revylon'];
		const rainSetters = ['masquerain', 'azumarillkaskade', 'vaporeon', 'exeggutoralola'];
		const hailSetters = ['ninetalesalola', 'abomasnow', 'glaceon', 'aurorus'];
		const bloodmoonSetters = ['umbreon', 'mandibuzz', 'umbralu', 'spiritomb', 'leocease'];
		const fogSetters = ['enigmeon', 'wyrdeer', 'drampa'];

		const sandSetters = ['palossandkaskade', 'stalagen', 'hippowdon', 'gigalith'];
		...
		const fairydustSetters = ['sylveon', 'shiinotic', 'excadrillkaskade', 'gardevoir'];
		...
		const incompatiblePokemon = [
			...
			// Prevent conflicting weathergy abilities from generating together
			[sunSetters, [...rainSetters, ...hailSetters, ...bloodmoonSetters, ...fogSetters]],
			[rainSetters, [...hailSetters, ...bloodmoonSetters, ...fogSetters]],
			[hailSetters, [...bloodmoonSetters, ...fogSetters]],
			...
			[battleauraSetters, [
				...paranormalactivitySetters, ...dreamscapeSetters, ...dragonforceSetters,
				...thunderstormSetters, ...magnetosphereSetters,
			]],
			[paranormalactivitySetters, [
	...
	randomBSSFactorySet(
		...
		const climateWeatherAbilities = ['drought', 'drizzle', 'snowwarning', 'eventide', 'condensation'];
		const irritantWeatherAbilities = ['sandstream', 'dustdevil', 'hayfever', 'secretion', 'pollution', 'incantation'];
		const energyWeatherAbilities = ['standoff', 'seance', 'dreamer', 'arcanum', 'stormfront', 'ferroflux'];
		const clearingWeatherAbilities = ['galeforce'];
		const terrainItemsRequire: { [k: string]: string } = {
			electricseed: "stormfront",
			psychicseed: "dreamer",
			grassyseed: "hayfever",
			mistyseed: "incantation",
		};
		...
		for (const curSet of setList) {
			...
			// reject 2+ climate weathergy setters
			if (teamData.climateWeather && climateWeatherAbilities.includes(curSet.ability)) {
				continue;
			}
			// reject 2+ irritant weathergy setters
			if (teamData.irritantWeather && irritantWeatherAbilities.includes(curSet.ability)) {
				continue;
			}
			// reject 2+ energy weathergy setters
			if (teamData.energyWeather && energyWeatherAbilities.includes(curSet.ability)) {
				continue;
			}
			// reject 2+ clearing weathergy setters
			if (teamData.clearingWeather && clearingWeatherAbilities.includes(curSet.ability)) {
				continue;
			}

			for (const item of curSet.item) {
				if (terrainItemsRequire[item] && !teamData.terrain?.includes(terrainItemsRequire[item]) && !teamData.clearingWeather) {
	...
	randomBSSFactoryTeam(side: PlayerOptions, depth = 0): RandomTeamsTypes.RandomFactorySet[] {
		...
		const climateWeatherAbilitiesSet: { [k: string]: string } = {
			drought: "sunnyday",
			drizzle: "raindance",
			snowwarning: "hail",
			eventide: "bloodmoon",
		...
		const irritantWeatherAbilitiesSet: { [k: string]: string } = {
			sandstream: "sandstorm",
			dustdevil: "duststorm",
			hayfever: "pollinate",
		...
		const energyWeatherAbilitiesSet: { [k: string]: string } = {
		...
		const clearingWeatherAbilitiesSet: { [k: string]: string } = {
		...
		while (shuffledSpecies.length && pokemon.length < this.maxTeamSize) {
			...
			if (abilityState.id in climateWeatherAbilitiesSet) {
				teamData.climateWeather = climateWeatherAbilitiesSet[abilityState.id];
			}
			if (abilityState.id in irritantWeatherAbilitiesSet) {
				teamData.irritantWeather = irritantWeatherAbilitiesSet[abilityState.id];
			}
			if (abilityState.id in energyWeatherAbilitiesSet) {
				teamData.energyWeather = energyWeatherAbilitiesSet[abilityState.id];
			}
			if (abilityState.id in clearingWeatherAbilitiesSet) {
				teamData.clearingWeather = clearingWeatherAbilitiesSet[abilityState.id];
			}
...
export default RandomSwSeTeams;
