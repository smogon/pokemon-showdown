...
export const Rulesets: {[k: string]: FormatData} = {
	...
	statboostpassmod: {
		...
		onBegin() {
			this.add('rule', 'Stat Boost Pass Mod: Positive stat boosts are reset upon using Baton Pass');
		},
		onHit(source, target, move) {
			if (source.positiveBoosts() && move.id === 'batonpass') {
				this.add('-clearpositiveboost', source);
				this.hint("Stat Boost Pass Mod activated: Stat Boosts cannot be passed");
			}
		},
		onValidateTeam(team) {
			for (const set of team) {
