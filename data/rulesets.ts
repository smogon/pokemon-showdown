...
export const Formats: {[k: string]: FormatData} = {
	...
	isleofarmorpokedex: {
		...
		desc: "Only allows Pok&eacute;mon native to the Crown Tundra in the Galar Region (Sw/Sh DLC1)",
		onValidateSet(set, format) {
			const ioaDex = [
	...
	crownedtundrapokedex: {
		effectType: 'ValidatorRule',
		name: 'Crowned Tundra Pokedex',
		desc: "Only allows Pok&eacute;mon native to the Crowned Tundra in the Galar Region (Sw/Sh DLC2)",
		onValidateSet(set, format) {
			const ioaDex = [
			...
			if (!ioaDex.includes(species.baseSpecies) && !ioaDex.includes(species.name) &&
				!this.ruleTable.has('+' + species.id)) {
				return [`${species.baseSpecies} is not in the Crowned Tundra Pokédex.`];
			}
		},
	},
	potd: {
