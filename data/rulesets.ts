...
export const Rulesets: import('../sim/dex-formats').FormatDataTable = {
	...
	// Persistent Battle Rules
	///////////////////////////////////////////////////////////////////

	persistentbattles: {
		effectType: 'Rule',
		name: 'Persistent Battles',
		desc: "Pokemon HP, PP, and status conditions persist between consecutive battles. Useful for gauntlet-style challenges.",
		onBegin() {
			// Access the persistence manager from Battle class
			const manager = (this.constructor as any).persistenceManager;
			if (!manager) return;

			// Try to restore state for each side
			for (const side of this.sides) {
				...
				if (state) {
					this.add('raw', `Restoring previous battle state for ${side.name}...`);
					manager.restoreBattleState(side, state);

					// Log restored Pokemon states
					for (const pokemon of side.pokemon) {
						if (pokemon.hp < pokemon.maxhp || pokemon.status) {
							const hpPercent = Math.floor((pokemon.hp / pokemon.maxhp) * 100);
							const statusMsg = pokemon.status ? ` (${pokemon.status.toUpperCase()})` : '';
							this.add('raw', `${pokemon.name}: ${hpPercent}% HP${statusMsg}`);
						}
					}
				}
			}

			// Save state when battle ends (hook into the win method)
			const originalWin = this.win.bind(this);
			this.win = function (side?: any) {
				// Save state for each side before declaring winner
	...
	persistentbattlesp1: {
		effectType: 'Rule',
		name: 'Persistent Battles P1',
		desc: "Like Persistent Battles, but only Player 1's Pokemon state persists. Useful for gauntlet challenges against fresh opponents.",
		onBegin() {
			// Access the persistence manager from Battle class
			const manager = (this.constructor as any).persistenceManager;
			if (!manager) return;

			// Only restore state for player 1
			...
			if (state) {
				this.add('raw', `Restoring previous battle state for ${p1.name}...`);
				manager.restoreBattleState(p1, state);

				for (const pokemon of p1.pokemon) {
					if (pokemon.hp < pokemon.maxhp || pokemon.status) {
						const hpPercent = Math.floor((pokemon.hp / pokemon.maxhp) * 100);
						const statusMsg = pokemon.status ? ` (${pokemon.status.toUpperCase()})` : '';
						this.add('raw', `${pokemon.name}: ${hpPercent}% HP${statusMsg}`);
			...
			const originalWin = this.win.bind(this);
			this.win = function (side?: any) {
				// Only save state for player 1
				const p1Side = this.sides[0];
				manager.saveBattleState(this, p1Side);
				return originalWin(side);
			};
		},
	},
	...
	clearpersistentstate: {
		effectType: 'Rule',
		name: 'Clear Persistent State',
		desc: "Clears all persistent battle state at the start of this battle. Use this to reset a gauntlet challenge.",
		onBegin() {
			// Access the persistence manager from Battle class
			const manager = (this.constructor as any).persistenceManager;
			if (!manager) return;
			...
			manager.clearAll();
			this.add('raw', 'All persistent battle state has been cleared.');
		},
	},
};
