...
export const Abilities: import('../sim/dex-abilities').AbilityDataTable = {
	...
	quickdraw: {
		...
		onFractionalPriority(priority, pokemon, target, move) {
			if (pokemon.volatiles['quickdraw']?.turns >= 3 && pokemon.item !== 'galaricacuff') {
				...
				return 0.1;
			} else if (pokemon.volatiles['quickdraw']?.turns >= 2 && pokemon.item === 'galaricacuff' && pokemon.species.name === 'Slowbro-Galar') {
				pokemon.volatiles['quickdraw'].turns = 0;
				this.add('-activate', pokemon, 'ability: Quick Draw');
				return 0.1;
			}
	...
	snowcloak: {
		onSourceModifyDamage(damage, source, target, move) {
			let mod = 1;
			if (target.hp >= target.maxhp / 2 || (target.hp >= target.maxhp / 4 && this.field.isWeather(['hail', 'snow']))) mod *= 0.75;
			return this.chainModify(mod);
		},
		flags: {breakable: 1},
		name: "Snow Cloak",
