	...
	pokemon: {
		...
		onChangeSet: function (set, format) {
			...
			if (battleForme) {
				...
				if (template.requiredItem && !template.requiredItem.includes(item.name)) {
					problems.push("" + template.species + " transforms in-battle with " + Chat.plural(template.requiredItem.length, "either ") + template.requiredItem.join(" or ") + '.'); // Mega or Primal
				}
				...
				if (template.requiredItem && !template.requiredItem.includes(item.name)) {
					problems.push("" + (set.name || set.species) + " needs to hold " + Chat.plural(template.requiredItem.length, "either ") + template.requiredItem.join(" or ") + '.'); // Memory/Drive/Griseous Orb/Plate/Z-Crystal - Forme mismatch
				}
				if (template.requiredMove && set.moves.indexOf(toId(template.requiredMove)) < 0) {
