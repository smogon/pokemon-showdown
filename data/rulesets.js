...
let BattleFormats = {
	...
	hoennpokedex: {
		...
		onValidateSet(set, format) {
			...
			if (!hoennDex.includes(template.baseSpecies) && !this.ruleTable.has('+' + template.id)) {
	...
	sinnohpokedex: {
		...
		onValidateSet(set, format) {
			...
			let template = this.dex.getTemplate(set.species || set.name);
			if ((!sinnohDex.includes(template.baseSpecies) || template.gen > 4) && !this.ruleTable.has('+' + template.id)) {
	...
	kalospokedex: {
		...
		onValidateSet(set, format) {
			...
			let template = this.dex.getTemplate(set.species || set.name);
			if ((!kalosDex.includes(template.baseSpecies) || template.gen > 6) && !this.ruleTable.has('+' + template.id)) {
	...
	alolapokedex: {
		...
		onValidateSet(set, format) {
			...
			let template = this.dex.getTemplate(set.species || set.name);
			if (!alolaDex.includes(template.baseSpecies) && !alolaDex.includes(template.name) && !this.ruleTable.has('+' + template.id)) {
	...
	galarpokedex: {
		...
		onValidateSet(set, format) {
			...
			let template = this.dex.getTemplate(set.species || set.name);
			if (!galarDex.includes(template.baseSpecies) && !galarDex.includes(template.name) && !this.ruleTable.has('+' + template.id)) {
	...
	formeclause: {
		...
		onValidateTeam(team) {
			...
			for (const set of team) {
				let template = this.dex.getTemplate(set.species);
				if (template.name !== template.baseSpecies) {
