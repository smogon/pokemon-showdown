...
let BattleFormats = {
	...
	hoennpokedex: {
		...
		onValidateSet(set, format) {
			...
			if (!hoennDex.includes(template.baseName) && !this.ruleTable.has('+' + template.id)) {
				return [template.baseName + " is not in the Hoenn Pokédex."];
			}
		},
	},
	sinnohpokedex: {
		...
		onValidateSet(set, format) {
			...
			let template = this.dex.getTemplate(set.species || set.name);
			if ((!sinnohDex.includes(template.baseName) || template.gen > 4) && !this.ruleTable.has('+' + template.id)) {
				return [`${template.name} is not in the Sinnoh Pokédex.`];
			}
		},
	},
	kalospokedex: {
		...
		onValidateSet(set, format) {
			...
			let template = this.dex.getTemplate(set.species || set.name);
			if ((!kalosDex.includes(template.baseName) || template.gen > 6) && !this.ruleTable.has('+' + template.id)) {
				return [`${template.name} is not in the Kalos Pokédex.`];
			}
		},
	},
	alolapokedex: {
		...
		onValidateSet(set, format) {
			...
			let template = this.dex.getTemplate(set.species || set.name);
			if (!alolaDex.includes(template.baseName) && !alolaDex.includes(template.name) && !this.ruleTable.has('+' + template.id)) {
				return [`${template.baseName} is not in the Alola Pokédex.`];
			}
		},
	},
	galarpokedex: {
		...
		onValidateSet(set, format) {
			...
			let template = this.dex.getTemplate(set.species || set.name);
			if (!galarDex.includes(template.baseName) && !galarDex.includes(template.name) && !this.ruleTable.has('+' + template.id)) {
				return [`${template.baseName} is not in the Galar Pokédex.`];
			}
		},
	},
	potd: {
		...
		onValidateTeam(team, format) {
			...
			for (const set of team) {
				...
				if (speciesTable[template.num]) {
					return ["You are limited to one of each Pokémon by Species Clause.", "(You have more than one " + template.baseName + ")"];
				}
	...
	nicknameclause: {
		...
		onValidateTeam(team, format) {
			...
			for (const set of team) {
				let name = set.name;
				if (name) {
					if (name === this.dex.getTemplate(set.species).baseName) continue;
					if (nameTable[name]) {
		...
		onValidateTeam(team) {
			...
			for (const [i, set] of team.entries()) {
				...
				if (this.gen >= 7) {
					...
					if (item.id === "ultranecroziumz" && template.baseName === "Necrozma") {
	...
	stabmonsmovelegality: {
		...
		checkLearnset(move, template, setSources, set) {
			...
			if (!restrictedMoves.includes(move.name) && !move.isNonstandard && !move.isMax) {
				...
				let baseTemplate = dex.getTemplate(template.baseName);
				...
				if (baseTemplate.otherFormes) {
					for (const formeid of baseTemplate.otherFormes) {
						...
						if (!forme.battleOnly) {
							if (!forme.forme.includes('Alola') && forme.forme !== 'Galar' && forme.baseName !== 'Wormadam') {
	...
	formeclause: {
		...
		onValidateTeam(team) {
			...
			for (const set of team) {
				let template = this.dex.getTemplate(set.species);
				if (template.name !== template.baseName) {
					let baseSpecies = this.dex.getTemplate(template.baseName);
				...
				if (formeTable.has(template.name)) {
					return [`You are limited to one of each forme of a Pokémon by Forme Clause.`, `(You have more than one of ${template.name})`];
				}
				formeTable.add(template.name);
			}
		},
	},
	scalemonsmod: {
