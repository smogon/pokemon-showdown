...
export const Scripts: ModdedBattleScriptsData = {
	...
	init() {
		...
		// Gen 1 has no per-move category: physical/special is decided by the move's type.
		// gen2 inherits gen3's remap, but our own init() overrides it, so redo it here with
		// the Gen 1 special-type list (no Dark) for the later-gen moves we added.
		const specialTypes = ['Fire', 'Water', 'Grass', 'Ice', 'Electric', 'Psychic', 'Dragon'];
