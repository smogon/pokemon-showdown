...
export const Scripts: ModdedBattleScriptsData = {
	...
	actions: {
		...
		runSwitch(pokemon: Pokemon) {
			...
			for (const poke of switchersIn) {
				...
				poke.isStarted = true;
				poke.draggedIn = null;
				if (poke.species.name === 'Iron Valiant' && !pokemon.battle.ruleTable.tagRules.includes("+tag:cap"))
					pokemon.m.usedMoves = [];
				if (poke.species.name === 'Sudowoodo' && !pokemon.battle.ruleTable.tagRules.includes("+tag:cap"))
					poke.m.grassMoves = 0;
			}
			return true;
		},
		useMove(
			...
			if (success && pokemon.species.name === 'Iron Valiant' &&
				!pokemon.battle.ruleTable.tagRules.includes("+tag:cap")) {
	...
	faintMessages(lastFirst = false, forceCheck = false, checkWin = true) {
		...
		while (this.faintQueue.length) {
			...
			if (!pokemon.fainted && this.runEvent('BeforeFaint', pokemon, faintData.source, faintData.effect)) {
				if (pokemon.species.name === 'Kecleon' && !this.ruleTable.tagRules.includes("+tag:cap")) {
					let forme = 'None';
	...
	field: {
		setWeather(status: string | Condition, source: Pokemon | 'debug' | null = null, sourceEffect: Effect | null = null) {
			...
			if (!this.battle.singleEvent('FieldStart', status, this.weatherState, this, source, sourceEffect)) {
				...
				return false;
			} else if (prevWeather === 'snowscape' && !this.battle.ruleTable.tagRules.includes("+tag:cap")) {
	...
	pokemon: {
		...
		cureStatus(this: Pokemon, silent?: boolean, source: Pokemon | null = null) {
			...
			if (source?.species.name === 'Zarude' && !source.battle.ruleTable.tagRules.includes("+tag:cap")) {
		...
		eatItem(this: Pokemon, force?: boolean, source?: Pokemon, sourceEffect?: Effect) {
			...
			if (
				...
				this.ateBerry = true;
				if (this.species.baseSpecies === 'Alcremie' && !this.battle.ruleTable.tagRules.includes("+tag:cap")) {
		...
		calculateStat(statName: StatIDExceptHP, boost: number, modifier?: number, statUser?: Pokemon) {
			...
			if (!this.battle.ruleTable.tagRules.includes("+tag:cap")) {
