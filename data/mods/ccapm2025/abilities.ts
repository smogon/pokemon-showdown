export const Abilities: import('../../../sim/dex-abilities').ModdedAbilityDataTable = {
	...
	heartofcold: {
		...
		onModifyDef(pokemon) {
			if (/* pokemon.hasType('Ice') && */ !this.field.isClimateWeather(['hail', 'snowscape'])) {
				return this.chainModify(1.5);
			}
		},
		flags: { breakable: 1 },
	...
	snowface: {
		onSwitchInPriority: -2,
		onStart(pokemon) {
			if (this.field.isClimateWeather(['hail', 'snowscape']) && pokemon.species.id === 'eiscuesnowconenosnow') {
	...
	gingerstream: {
		onStart(source) {
			this.field.setIrritantWeather('gingerstorm');
		},
		flags: {},
		name: "Ginger Stream",
		rating: 4,
