export const Moves: import('../../../sim/dex-moves').ModdedMoveDataTable = {
	...
	moneytrees: {
		...
		onBasePower(basePower, source) {
			if ((this.field.isTerrain('grassyterrain') && source.isGrounded()) ||
				['sunnyday', 'desolateland'].includes(source.effectiveClimateWeather())
			) {
	...
	auroraveil: {
		inherit: true,
		onTry(source) {
			if (!this.field.isClimateWeather(['hail', 'snowscape']) && !source.hasAbility('heartofcold')) return false;
		},
	},
	blizzard: {
		inherit: true,
		onModifyMove(move, pokemon) {
			if (this.field.isClimateWeather(['hail', 'snowscape']) || pokemon.hasAbility('heartofcold')) move.accuracy = true;
		},
	},
	weatherball: {
		inherit: true,
		onModifyType(move, pokemon) {
			...
			if (!this.field.isClimateWeather(['hail', 'snowscape']) && pokemon.hasAbility('heartofcold')) {
				move.type = 'Ice';
			}
		},
		onModifyMove(move, pokemon) {
