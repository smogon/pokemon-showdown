...
export const Abilities: {[k: string]: ModdedAbilityData} = {
	...
	*/
	...
	ragequit: {
		...
		onAfterMove(pokemon, target, move) {
			if (pokemon.moveThisTurnResult === false) {
				...
				pokemon.faint();
				if (pokemon.side.getFoeActive()[0]) {
					this.boost({atk: -2, spa: -2}, pokemon.side.getFoeActive()[0], pokemon, null, true);
				}
			}
		},
		isNonstandard: "Custom",
	...
	degenerator: {
		...
		onStart(pokemon) {
			const foe = Array.isArray(pokemon.side.foe) ? pokemon.side.foe[0] : pokemon.side.foe;
			foe.addSideCondition('degenerator', pokemon);
			const data = foe.getSideConditionData('degenerator');
			if (!data.sources) {
				data.sources = [];
			}
		...
		onEnd(pokemon) {
			const foe = Array.isArray(pokemon.side.foe) ? pokemon.side.foe[0] : pokemon.side.foe;
			foe.removeSideCondition('degenerator');
		},
		condition: {
	...
	stubbornness: {
		...
		onFoeAfterBoost(boost, target, source, effect) {
			const pokemon = target.side.getFoeActive()[0];
			let success = false;
			let i: BoostName;
			for (i in boost) {
				if (boost[i]! > 0) {
