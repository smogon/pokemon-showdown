...
export const Conditions: {[k: string]: ModdedConditionData & {innateName?: string}} = {
	...
	*/
	...
	billo: {
		noCopy: true,
		onStart(source) {
			let activeMon = source.side.getFoeActive()[0].species.name;
	...
	brandon: {
		...
		onFaint(pokemon) {
			...
			const foeName = foe.illusion ? foe.illusion.name : pokemon.side.getFoeActive()[0].name;
	...
	fart: {
		...
		onStart(source) {
			let activeMon;
			activeMon = source.side.getFoeActive()[0];
		...
		onFaint(pokemon) {
			let activeMon;
			activeMon = pokemon.side.getFoeActive()[0];
	...
	kris: {
		...
		onStart(source) {
			...
			const foeName = foe.illusion ? foe.illusion.name : source.side.getFoeActive()[0].name;
			if (!Array.isArray(source.side.foe) && (foeName === 'Aeonic' || source.side.foe.name === 'Aeonic')) {
	...
	lamp: {
		noCopy: true,
		onStart(pokemon) {
			this.add(`c|${getName('Lamp')}|DUDE HI ${Array.isArray(pokemon.side.foe) ? pokemon.side.foe[0].name : pokemon.side.foe.name} (:`);
		},
		onSwitchOut(pokemon) {
			this.add(`c|${getName('Lamp')}|bye ${Array.isArray(pokemon.side.foe) ? pokemon.side.foe[0].name : pokemon.side.foe.name} :)`);
		},
		onFaint() {
	...
	mitsuki: {
		...
		onSwitchOut(source) {
			this.add(`c|${getName('Mitsuki')}|You're weak, ${Array.isArray(source.side.foe) ? source.side.foe[0].name : source.side.foe.name}. Why? Because you lack... hatred.`);
		},
		onFaint() {
	...
	overneat: {
		noCopy: true,
		onStart(source) {
			this.add(`c|${getName('Overneat')}|Lets end this ${Array.isArray(source.side.foe) ? source.side.foe[0].name : source.side.foe.name}!!`);
	...
	pirateprincess: {
		...
		onStart(source) {
			...
			const activeMon = this.toID(
				foe.illusion ? foe.illusion.name : source.side.getFoeActive()[0].name
			);
	...
	segmr: {
		...
		onFaint(pokemon) {
			const foe = pokemon.foes()[0];
			const foeName = foe.illusion ? foe.illusion.name : pokemon.side.getFoeActive()[0].name;
			this.add(`c|${getName('Segmr')}|I'm sorry ${foeName} but could you please stop talking to me?`);
		},
	},
	sejesensei: {
		noCopy: true,
	...
	theimmortal: {
		noCopy: true,
		onStart(source) {
			const foe = source.side.getFoeActive()[0];
	...
	zodiax: {
		...
		onFaint(pokemon) {
			const name = Array.isArray(pokemon.side.foe) ? pokemon.side.foe[0].name : pokemon.side.foe;
	...
	minior: {
		...
		onSwitchIn(pokemon) {
			...
			if (pokemon.species.id === 'miniorindigo') {
				this.boost({atk: 1, spa: 1}, pokemon.side.getFoeActive()[0]);
