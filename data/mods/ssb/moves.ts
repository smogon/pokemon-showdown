	...
	*/
	...
	thetownoutplay: {
		...
		onHit(target, source, move) {
			if (this.randomChance(1, 10)) {
				for (const foe of source.side.getFoeActive()) {
	...
	waveterrain: {
		...
		condition: {
			...
			onStart(battle, source, effect) {
				...
				const foe = Array.isArray(source.side.foe) ? source.side.foe[0] : source.side.foe;
				for (const sideCondition of removeAll) {
					if (foe.removeSideCondition(sideCondition)) {
						if (!silentRemove.includes(sideCondition)) {
							this.add('-sideend', foe, this.dex.getEffect(sideCondition).name, '[from] move: Wave Terrain', '[of] ' + source);
						}
					}
					if (source.side.removeSideCondition(sideCondition)) {
						if (!silentRemove.includes(sideCondition)) {
	...
	gacha: {
		...
		onHit(target, source, move) {
			...
			if (forme === 'Minior-Indigo') {
				this.boost({atk: 1, spa: 1}, target.side.getFoeActive()[0]);
			} else if (forme === 'Minior') {
				target.side.getFoeActive()[0].addVolatile('taunt');
			} else if (forme === 'Minior-Yellow') {
				target.side.getFoeActive()[0].trySetStatus('par', target);
	...
	tsukuyomi: {
		...
		onTryHit(target, source, move) {
			if (!source.hasType('Ghost')) {
				...
				move.onHit = function (t, s, m) {
					s.side.getFoeActive()[0].addVolatile('trapped', s, m, 'trapper');
				};
		...
		onHit(target, source, move) {
			...
			source.side.getFoeActive()[0].addVolatile('trapped', source, move, 'trapper');
	...
	rightoncue: {
		...
		onHit(target, source) {
			...
			if (effects <= 0) {
				...
				this.add(`c|${getName('tiki')}|whos ${Array.isArray(source.side.foe) ? source.side.foe[0].name : source.side.foe.name}?`);
			}
		},
		secondary: null,
		target: "normal",
