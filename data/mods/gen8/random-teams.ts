...
export class RandomGen8Teams {
	...
	constructor(format: Format | string, prng: PRNG | PRNGSeed | null) {
		...
		this.moveEnforcementCheckers = {
			...
			Water: (movePool, moves, abilities, types, counter, species) => {
				...
				if (
					["hypervoice", "liquidation", "surgingstrikes"].some((m) =>
						movePool.includes(m)
					)
				)
					return true;
			...
			if (forme === "Smeargle") {
				...
				for (let i = 0; i < 2 && learnsetSpecies.prevo; i++) {
					...
					for (const moveid in learnset) {
						if (
							(!pool.includes(moveid) && !evoRegion) ||
							learnset[moveid].some(
								(source) =>
									!source.startsWith(`${learnsetSpecies.gen}`) ||
									source.charAt(1) === "E"
							)
						) {
							pool.push(moveid);
		...
		for (const moveid of moves) {
			...
			if (
				...
				if (types.includes(moveType)) {
					...
					(moveType === "Normal" &&
						["Aerilate", "Galvanize", "Pixilate", "Refrigerate"].some(
	...
	getAbility(
		...
		if (isDoubles) {
			if (
				abilities.has("Competitive") &&
				!abilities.has("Shadow Tag") &&
				!abilities.has("Strong Jaw")
			)
				return "Competitive";
			...
			if (
				abilities.has("Telepathy") &&
				(abilities.has("Pressure") || abilities.has("Analytic"))
			)
				return "Telepathy";
		}
	...
	getHighPriorityItem(
		...
		if (species.name === "Shedinja") {
			const noSash =
				!teamDetails.defog && !teamDetails.rapidSpin && !isDoubles;
		...
		if (
			...
			["Cheek Pouch", "Harvest", "Ripen"].includes(ability)
		)
			return "Sitrus Berry";
		if (ability === "Gluttony")
			return (
		...
		if (ability === "Sheer Force" && counter.get("sheerforce"))
			return "Life Orb";
		if (ability === "Unburden")
			return moves.has("closecombat") || moves.has("curse")
				? "White Herb"
				: "Sitrus Berry";
	...
	getLowPriorityItem(
		...
		const spinnerCase =
			moves.has("rapidspin") &&
			(ability === "Regenerator" || !!counter.get("recovery"));
	...
	randomCAP1v1Team() {
		...
		while (pokemonPool.length && pokemon.length < this.maxTeamSize) {
			...
			const setData: AnyObject = this.sample(
				this.randomCAP1v1Sets[species.name]
			);
			const set = {
				name: species.baseSpecies,
				species: species.name,
	...
	randomBSSFactoryTeam(
		...
		for (const speciesName of pokemonPool) {
			const sortObject = {
				...
				score: Math.pow(
					this.prng.next(),
					1 / this.randomBSSFactorySets[speciesName].usage
				),
			};
		...
		while (shuffledSpecies.length && pokemon.length < this.maxTeamSize) {
			...
			if (
				!this.forceMonotype &&
				teamData.typeComboCount[typeCombo] >= limitFactor
			)
				continue;
