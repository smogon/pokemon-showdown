...
export const Rulesets: { [k: string]: FormatData } = {
	...
	limittworestricted: {
		...
		onValidateTeam(team) {
			...
			if (restrictedSpecies.length > 2) {
				return [
					`You can only use up to two restricted Pok\u00E9mon (you have: ${restrictedSpecies.join(
						", "
					)})`,
				];
			}
	...
	limitonerestricted: {
		...
		onValidateTeam(team) {
			...
			if (restrictedSpecies.length > 1) {
				return [
					`You can only use one restricted Pok\u00E9mon (you have: ${restrictedSpecies.join(
						", "
					)})`,
				];
			}
	...
	standardnatdex: {
		...
		onValidateSet(set) {
			...
			if (species.natDexTier === "Illegal") {
				...
				return [
					`${set.name || set.species} does not exist in the National Dex.`,
				];
			}
			...
			if (requireObtainable) {
				if (species.natDexTier === "Unreleased") {
					...
					return [
						...
						} does not exist in the National Dex.`,
					];
				}
				for (const moveid of set.moves) {
					...
					if (
						...
						if (this.ruleTable.has("omunobtainablemoves")) {
							const outOfBattleSpecies =
								this.getValidationSpecies(set)[0];
							if (
								!this.omCheckCanLearn(
									move,
									outOfBattleSpecies,
									this.allSources(outOfBattleSpecies),
									set,
									problem
								)
							)
			...
			if (requireObtainable && item.isNonstandard) {
				...
				return [
					`${set.name}'s item ${item.name} does not exist in Gen ${this.dex.gen}.`,
				];
			}
		},
		onBegin() {
	...
	obtainable: {
		...
		onValidateTeam(team, format) {
			...
			for (const set of team) {
				if (
					set.species === "Kyurem-White" ||
					set.species === "Kyurem-Black"
				) {
					if (kyuremCount > 0) {
						return [
				...
				if (
					set.species === "Calyrex-Ice" ||
					set.species === "Calyrex-Shadow"
				) {
					if (calyrexCount > 0) {
						return [
		...
		onChangeSet(set) {
			...
			if (set.moves) {
				...
				for (const moveId of set.moves) {
					...
					if (hasMove[moveid])
						return [
							`${species.baseSpecies} has multiple copies of ${move.name}.`,
						];
					hasMove[moveid] = true;
				}
			}
	...
	hoennpokedex: {
		...
		onValidateSet(set, format) {
			...
			if (
				!hoennDex.includes(species.baseSpecies) &&
	...
	sinnohpokedex: {
		...
		onValidateSet(set, format) {
			...
			const species = this.dex.species.get(set.species || set.name);
			if (
				(!sinnohDex.includes(species.baseSpecies) || species.gen > 4) &&
				!this.ruleTable.has("+" + species.id)
			) {
	...
	oldunovapokedex: {
		...
		onValidateSet(set, format) {
			...
			const isUnova =
				species.num >= 494 &&
				species.num <= 649 &&
				!["Black", "White", "Therian", "Resolute"].includes(
					species.forme
				) &&
				species.gen <= 5;
	...
	newunovapokedex: {
		...
		onValidateSet(set, format) {
			...
			const species = this.dex.species.get(set.species || set.name);
			const isUnova =
				unovaDex.includes(species.baseSpecies) && species.gen <= 5;
			if (!isUnova && !this.ruleTable.has("+" + species.id)) {
	...
	kalospokedex: {
		...
		onValidateSet(set, format) {
			...
			const species = this.dex.species.get(set.species || set.name);
			if (
				(!kalosDex.includes(species.baseSpecies) || species.gen > 6) &&
				!this.ruleTable.has("+" + species.id)
			) {
	...
	oldalolapokedex: {
		...
		onValidateSet(set, format) {
			...
			const species = this.dex.species.get(set.species || set.name);
			if (
				!alolaDex.includes(species.baseSpecies) &&
				!alolaDex.includes(species.name) &&
				!this.ruleTable.has("+" + species.id)
			) {
	...
	galarpokedex: {
		...
		onValidateSet(set, format) {
			...
			const species = this.dex.species.get(set.species || set.name);
			if (
				!galarDex.includes(species.baseSpecies) &&
				!galarDex.includes(species.name) &&
				!this.ruleTable.has("+" + species.id)
			) {
	...
	isleofarmorpokedex: {
		...
		onValidateSet(set, format) {
			...
			const species = this.dex.species.get(set.species || set.name);
			if (
				!ioaDex.includes(species.baseSpecies) &&
				!ioaDex.includes(species.name) &&
				!this.ruleTable.has("+" + species.id)
			) {
				return [
					`${species.baseSpecies} is not in the Isle of Armor Pokédex.`,
				];
			}
		},
	},
	crowntundrapokedex: {
		...
		onValidateSet(set, format) {
			...
			const species = this.dex.species.get(set.species || set.name);
			if (
				!tundraDex.includes(species.baseSpecies) &&
				!tundraDex.includes(species.name)
			) {
				return [
					`${species.baseSpecies} is not in the Crown Tundra Pokédex.`,
				];
			}
		},
	},
	galarexpansionpokedex: {
		...
		onValidateSet(set, format) {
			...
			if (
				...
				return [
					`${species.baseSpecies} is not in the Galar, Isle of Armor, or Crown Tundra Pokédexes.`,
				];
			}
		},
	},
	paldeapokedex: {
		...
		onValidateSet(set, format) {
			...
			const species = this.dex.species.get(set.species || set.name);
			if (
				!paldeaDex.includes(species.baseSpecies) &&
				!paldeaDex.includes(species.name) &&
				!this.ruleTable.has("+" + species.id)
			) {
	...
	kitakamipokedex: {
		...
		onValidateSet(set, format) {
			...
			const species = this.dex.species.get(set.species || set.name);
			if (
				!kitakamiDex.includes(species.baseSpecies) &&
				!kitakamiDex.includes(species.name) &&
				!this.ruleTable.has("+" + species.id)
			) {
	...
	forcemonotype: {
		...
		onValidateRule(value) {
			...
			if (
				...
				throw new Error(
					`Invalid type "${type.name}" in Generation ${this.dex.gen}`
				);
			}
		},
		onValidateSet(set) {
			const species = this.dex.species.get(set.species);
			const type = this.dex.types.get(
				this.ruleTable.valueRules.get("forcemonotype")!
			);
	...
	forceselect: {
		...
		onValidateTeam(team) {
			...
			const species = this.dex.species.get(
				this.ruleTable.valueRules.get("forceselect")
			);
			for (const set of team) {
	...
	evlimits: {
			...
			return slashedParts
				.map((slashedPart) => {
					...
					if (
						...
						!UINT_REGEX.test(low) ||
						hyphen !== "-" ||
						!UINT_REGEX.test(high)
					) {
						throw new Error(
							`EV limits should be in the format "EV Limits = Atk 0-124 / Def 100-252"`
						);
					}
					const statid = this.dex.toID(stat) as StatID;
					if (!this.dex.stats.ids().includes(statid)) {
						throw new Error(
							`Unrecognized stat name "${stat}" in "${value}"`
						);
					}
		...
		onValidateSet(set) {
			...
			for (const limit of limits.split(" / ")) {
				...
				if (ev < low || ev > high) {
					problems.push(
						`${set.name || set.species}'s ${
							this.dex.stats.names[statid]
						} EV (${ev}) must be ${low}-${high}`
					);
				}
			}
			return problems;
		...
		onTeamPreview() {
			...
			for (const pokemon of this.getAllPokemon()) {
				...
				if (!this.ruleTable.has("speciesrevealclause")) {
					details = details
						.replace(
							/(Greninja|Gourgeist|Pumpkaboo|Xerneas|Silvally|Urshifu|Dudunsparce)(-[a-zA-Z?-]+)?/g,
							"$1-*"
						)
	...
	teratypepreview: {
		...
		onValidateRule() {
			if (!this.ruleTable.has("teampreview")) {
				throw new Error(
					`The "Tera Type Preview" rule${this.ruleTable.blame(
						"teratypepreview"
	...
	littlecup: {
		...
		onValidateSet(set) {
			...
			if (
				species.prevo &&
				this.dex.species.get(species.prevo).gen <= this.gen
			) {
	...
	abilityclause: {
		...
		onBegin() {
			...
			this.add(
				"rule",
				`${num} Ability Clause: Limit ${num} of each ability`
			);
		},
		onValidateRule(value) {
			...
			for (const set of team) {
				...
				if ((abilityTable.get(ability) || 0) >= num) {
					return [
						...
						`(You have more than ${num} ${
							this.dex.abilities.get(ability).name
						} variant${num === 1 ? "" : "s"})`,
					];
				}
			...
			if (set.moves) {
				for (const id of set.moves) {
					...
					if (move.status && move.status === "slp")
						problems.push(
							move.name + " is banned by Sleep Moves Clause."
						);
				}
			}
			return problems;
		...
		onValidateTeam(team) {
			...
			for (const set of team) {
				...
				for (const moveid of set.moves) {
					...
					const hasMissChanceOrNeverMisses =
						move.accuracy === true || move.accuracy < 100;

					if (
						move.status &&
						move.status === "slp" &&
						hasMissChanceOrNeverMisses
					) {
						hasSleepMove = true;
					}
				}
			...
			if (hasOrbeetle && hasSleepMove) {
				return [
					`The combination of Gravity and Gigantamax Orbeetle on the same team is banned.`,
				];
			}
		},
		onBegin() {
			this.add(
				"rule",
				"Gravity Sleep Clause: The combination of sleep-inducing moves with imperfect accuracy and Gravity or Gigantamax Orbeetle are banned"
			);
		},
	},
	endlessbattleclause: {
	...
	batonpassclause: {
		...
		onValidateSet(set, format, setHas) {
			...
			for (const moveId of set.moves) {
				...
				if (
					nonSpeedBoostedMoves.includes(move.id) ||
					(move.boosts &&
				...
				if (
					...
					if (
						(boosts.atk && boosts.atk > 0) ||
						(boosts.def && boosts.def > 0) ||
						(boosts.spa && boosts.spa > 0) ||
						(boosts.spd && boosts.spd > 0)
					) {
			...
			if (
				speedBoostedAbilities.includes(ability) ||
				speedBoostedItems.includes(item.id)
			) {
				speedBoosted = true;
			}
			...
			if (
				nonSpeedBoostedAbilities.includes(ability) ||
				nonSpeedBoostedItems.includes(item.id)
			) {
				nonSpeedBoosted = true;
			}
			...
			if (
				speedBoosted !== nonSpeedBoosted &&
				typeof speedBoosted === "string" &&
			...
			return [
				`${
					set.name || set.species
				} can Baton Pass both Speed and a different stat, which is banned by Baton Pass Clause.`,
			];
		},
	},
	onebatonpassclause: {
			...
			for (const set of team) {
				...
				if (passableBoosts > 1) {
					return [
						`${
							set.name || set.species
						} has Baton Pass and multiple ways to boost its stats, which is banned by One Boost Passer Clause.`,
					];
				}
				if (passers > 1) {
					return [
	...
	batonpassstatclause: {
		...
		onBegin() {
			this.add(
				"rule",
				"Baton Pass Stat Clause: No Baton Passer may have a way to boost its stats"
			);
		},
		onValidateTeam(team) {
			const boostingEffects = [
			...
			for (const set of team) {
				...
				if (passableBoosts) {
					return [
						`${
							set.name || set.species
						} has Baton Pass and a way to boost its stats, which is banned by Baton Pass Stat Clause.`,
					];
				}
			}
		},
	...
	zmoveclause: {
		...
		onValidateSet(set) {
			...
			if (item.zMove)
				return [
					`${set.name || set.species}'s item ${
						item.name
					} is banned by Z-Move Clause.`,
				];
		},
		onBegin() {
			...
			if (status.id === "slp") {
				for (const pokemon of target.side.pokemon) {
					if (pokemon.hp && pokemon.status === "slp") {
						this.add(
							"-message",
							"Sleep Clause activated. (In Nintendo formats, Sleep Clause activates if any of the opponent's Pokemon are asleep, even if self-inflicted from Rest)"
						);
						return false;
					}
				}
	...
	deoxyscamouflageclause: {
		...
		onBegin() {
			this.add(
				"rule",
				"Deoxys Camouflage Clause: Reveals the Deoxys forme when it is sent in battle."
			);
		},
	},
	freezeclausemod: {
		...
		onValidateTeam(team) {
			...
			for (const [i, set] of team.entries()) {
				...
				if (i === 0) {
					...
					typeTable = typeTable.filter((type) =>
			...
			for (const set of team) {
				if (
					this.gen === 9 &&
					set.teraType &&
					!typeTable.includes(set.teraType) &&
					this.ruleTable.has(`enforcesameteratype`)
				) {
					return [
						`${set.species}'s Tera Type must match the team's type.`,
					];
				}
			}
		},
	...
	minsourcegen: {
		...
		onValidateRule(value) {
			...
			if (minSourceGen > this.dex.gen) {
				...
				throw new Error(
					`Invalid generation ${minSourceGen}${this.ruleTable.blame(
						"minsourcegen"
					)} for a Gen ${this.dex.gen} format (${this.format.name})`
				);
			}
		},
	},
	...
	omunobtainablemoves: {
		...
		onValidateRule() {
			if (!this.ruleTable.checkCanLearn?.[0]) {
				throw new Error(
					`A format with the "OM Unobtainable Moves"${this.ruleTable.blame(
						"omunobtainablemoves"
					)} rule must also have a special move legality rule.`
				);
			}
		},
	},
	stabmonsmovelegality: {
		...
		checkCanLearn(move, species, setSources, set) {
			const nonstandard =
				...
				!this.ruleTable.has("standardnatdex");
			if (
				!nonstandard &&
				!move.isZ &&
				!move.isMax &&
				...
				const minObtainableSpeciesGen =
					this.dex.currentMod === "gen8bdsp" || this.dex.gen === 9
						? this.dex.gen
						: species.gen;
				for (
					let i = this.dex.gen;
					i >= minObtainableSpeciesGen && i >= move.gen;
					i--
				) {
					...
					if (pokemon.forme || pokemon.otherFormes) {
						...
						const originalForme = dex.species.get(
							pokemon.changesFrom || pokemon.name
						);
						...
						if (baseSpecies.otherFormes) {
							for (const formeName of baseSpecies.otherFormes) {
								...
								if (
									forme.changesFrom === originalForme.name &&
									!forme.battleOnly
								) {
	...
	sketchmonsmovelegality: {
		...
		onValidateTeam(team) {
			...
			const overSketched = [...sketches.entries()].filter(
				([moveName, count]) => count > 1
			);
			if (overSketched.length) {
				return overSketched.map(
					([moveName, count]) =>
	...
	camomonsmod: {
			...
			const types = [
				...new Set(
					target.baseMoveSlots
						.slice(0, 2)
						.map((move) => this.dex.moves.get(move.id).type)
				),
			];
		...
		onSwitchIn(pokemon) {
			this.add(
				...
				pokemon,
				"typechange",
				(pokemon.illusion || pokemon).getTypes(true).join("/"),
				"[silent]"
			);
		},
		onAfterMega(pokemon) {
			...
			for (const set of team) {
				...
				if (species.name !== species.baseSpecies) {
					...
					if (
						...
						Object.values(species.baseStats).join("/") ===
							Object.values(baseSpecies.baseStats).join("/")
					) {
						species = baseSpecies;
					}
				}
	...
	"350cupmod": {
		...
		onBegin() {
			this.add(
				"rule",
				"350 Cup Mod: If a Pokemon's BST is 350 or lower, all of its stats get doubled."
			);
		},
		onModifySpeciesPriority: 2,
		onModifySpecies(species) {
			...
			if (newSpecies.bst <= 350) {
				...
				for (const stat in newSpecies.baseStats) {
					newSpecies.baseStats[stat] = this.clampIntRange(
	...
	flippedmod: {
		...
		onBegin() {
			this.add(
				"rule",
				"Flipped Mod: Pokemon have their stats flipped (HP becomes Spe, vice versa)."
			);
		},
		onModifySpeciesPriority: 2,
		onModifySpecies(species) {
			...
			for (const [i, statName] of Object.keys(
	...
	scalemonsmod: {
		...
		onModifySpecies(species) {
			...
			for (const stat in newSpecies.baseStats) {
				if (stat === "hp") continue;
				newSpecies.baseStats[stat] = this.clampIntRange(
					(newSpecies.baseStats[stat] * scale) / bstWithoutHp,
					1,
					255
				);
				newSpecies.bst += newSpecies.baseStats[stat];
			...
			for (const side of this.sides) {
				for (const pokemon of side.pokemon) {
					const details = pokemon.details
						.replace(", shiny", "")
						.replace(
							/(Arceus|Greninja|Gourgeist|Pumpkaboo|Silvally|Urshifu)(-[a-zA-Z?-]+)?/g,
							"$1-*"
						);
					this.add("poke", pokemon.side.id, details, "");
				}
	...
	openteamsheets: {
		...
		onValidateRule() {
			if (
				!(
					this.ruleTable.has("teampreview") ||
					this.ruleTable.has("teamtypepreview")
				)
			) {
				throw new Error(
					`The "Open Team Sheets" rule${this.ruleTable.blame(
						"openteamsheets"
					)} requires Team Preview.`
				);
			}
	...
	forceopenteamsheets: {
		...
		onValidateRule() {
			if (
				...
				throw new Error(
					`The "Force Open Team Sheets" rule${this.ruleTable.blame(
						"forceopenteamsheets"
					)} requires Team Preview.`
				);
			}
	...
	eventmovesclause: {
		...
		onValidateSet(set) {
			...
			while (prevo) {
				const prevoSpecies = this.dex.species.get(prevo);
				const prevoLsetData =
					this.dex.data.Learnsets[prevoSpecies.id]?.learnset || {};
			...
			if (set.moves?.length) {
				for (const move of set.moves) {
					if (
						learnsetData[this.toID(move)] &&
						!learnsetData[this.toID(move)].filter((v) => !v.includes("S"))
							.length
					) {
						problems.push(
							`${species.name}'s move ${move} is obtainable only through events.`
						);
					}
				}
			}
	...
	pickedteamsize: {
		...
		onValidateRule() {
			if (
				...
				throw new Error(
					`The "Picked Team Size" rule${this.ruleTable.blame(
						"pickedteamsize"
					)} requires Team Preview.`
				);
			}
	...
	maxteamsize: {
		...
		onValidateRule(value) {
			if (this.format.id.endsWith("computergeneratedteams")) {
				throw new Error(
					`${this.format.name} does not support Max Team Size.`
				);
			}
		},
	},
	...
	maxtotallevel: {
		...
		onValidateTeam(team) {
			...
			if (totalLowestLevels > maxTotalLevel) {
				const thePokemon =
					pickedTeamSize === team.length
						? `all ${team.length} Pokémon`
						: `the ${pickedTeamSize} lowest-leveled Pokémon`;
				return [
					`The combined levels of ${thePokemon} of your team is ${totalLowestLevels}, above the format's total level limit of ${maxTotalLevel}${this.ruleTable.blame(
						"maxtotallevel"
					)}.`,
				];
			}
			...
			if (minTotalWithHighestLevel > maxTotalLevel) {
				return [
					`Your highest level Pokémon is unusable, because there's no way to create a team with it whose total level is less than the format's total level limit of ${maxTotalLevel}${this.ruleTable.blame(
						"maxtotallevel"
					)}.`,
				];
			}
		...
		onValidateRule(value) {
			...
			const maxTeamSizeBlame = ruleTable.pickedTeamSize
				? ruleTable.blame("pickedteamsize")
				: ruleTable.blame("maxteamsize");
			if (maxTotalLevel >= ruleTable.maxLevel * maxTeamSize) {
				throw new Error(
					`A Max Total Level of ${maxTotalLevel}${ruleTable.blame(
						"maxtotallevel"
					)} is too high (and will have no effect) with ${maxTeamSize}${maxTeamSizeBlame} Pokémon at max level ${
						ruleTable.maxLevel
			...
			if (maxTotalLevel <= ruleTable.minLevel * maxTeamSize) {
				throw new Error(
					`A Max Total Level of ${maxTotalLevel}${ruleTable.blame(
						"maxtotallevel"
					)} is too low with ${maxTeamSize}${maxTeamSizeBlame} Pokémon at min level ${
						ruleTable.minLevel
	...
	crazyhouserule: {
		...
		onValidateRule(value) {
			if (
				this.format.gameType === "doubles" ||
				this.format.gameType === "triples"
			) {
				throw new Error(
					`Crazyhouse Rule currently does not support ${this.format.gameType} battles.`
				);
			}
			const ruleTable = this.ruleTable;
			const maxTeamSize = ruleTable.pickedTeamSize || ruleTable.maxTeamSize;
			const numPlayers =
				this.format.gameType === "freeforall" ||
			...
			if (potentialMaxTeamSize > 24) {
				throw new Error(
					`Crazyhouse Rule cannot be added because a team can potentially have ${potentialMaxTeamSize} Pokemon on one team, which is more than the server limit of 24.`
				);
			}
		},
		...
		onBeforeSwitchIn(pokemon) {
			...
			pokemon.side.pokemon = pokemon.side.pokemon.filter(
				(x) => !(x.fainted && !x.m.outofplay)
			);
		...
		onFaint(target, source, effect) {
			...
			if (
				...
				source.side.pokemon.length < 24 &&
				source.side !== target.side &&
				target.m.numSwaps < 4
			) {
				const hpCost = this.clampIntRange(
					Math.floor((target.baseMaxhp * target.m.numSwaps) / 4),
					1
				);
				...
				this.add(
					"poke",
					source.side.pokemon[newPos].side.id,
					source.side.pokemon[newPos].details,
					""
				);
				this.add(
					"-message",
					`${target.name} was captured by ${newPoke.side.name}!`
				);
			} else {
				target.m.outofplay = true;
			}
	...
	chimera1v1rule: {
		...
		onBeforeSwitchIn(pokemon) {
			...
			pokemon.moveSlots = (pokemon as any).baseMoveSlots = [
				...
				(move, index, moveSlots) =>
					moveSlots.find((othermove) => othermove.id === move.id) === move
			);
	...
	bonustypemod: {
		...
		onBegin() {
			this.add(
				"rule",
				"Bonus Type Mod: Pok\u00e9mon have their Tera Type added onto their current ones."
			);
		},
		onModifySpeciesPriority: 1,
		onModifySpecies(species, target, source, effect) {
	...
	firstbloodrule: {
		...
		onBegin() {
			this.add(
				"rule",
				"First Blood Rule: The first team to have a Pok\u00e9mon faint loses."
			);
		},
		onFaint(target) {
	...
	tiershiftmod: {
		...
		onModifySpecies(species, target, source, effect) {
			...
			let tier: string = this.toID(
				isNatDex ? species.natDexTier : species.tier
			);
			...
			for (statName in pokemon.baseStats as StatsTable) {
				...
				pokemon.baseStats[statName] = this.clampIntRange(
					pokemon.baseStats[statName] + boost,
					1,
					255
				);
	...
	revelationmonsmod: {
		...
		onBegin() {
			this.add(
				"rule",
				"Revelationmons Mod: The first moveslots have their types changed to match the Pok\u00e9mon's types"
			);
		},
		onValidateSet(set) {
			const species = this.dex.species.get(set.species);
			...
			for (const [i, moveid] of set.moves.entries()) {
				...
				if (i <= slotIndex) {
					problems.push(
						`${move.name} can't be in moveslot ${
							i + 1
						} because it's restricted from being in the first ${
							slotIndex + 1 > 1 ? `${slotIndex + 1} slots` : "slot"
						}.`
					);
				}
			}
	...
	reevolutionmod: {
		...
		onBegin() {
			this.add(
				"rule",
				"Re-Evolution Mod: Pok\u00e9mon gain the boosts they would gain from evolving again"
			);
		},
		onModifySpecies(species, target) {
			...
			if (!newSpecies.prevo) {
				...
				for (statid in prevoSpecies.baseStats) {
					const change =
						baseSpecies.baseStats[statid] -
						prevoSpecies.baseStats[statid];
					const formeChange =
						newSpecies.baseStats[statid] - baseSpecies.baseStats[statid];
					newSpecies.baseStats[statid] = this.clampIntRange(
						baseSpecies.baseStats[statid] + change,
						1,
						255
					);
					newSpecies.baseStats[statid] = this.clampIntRange(
						newSpecies.baseStats[statid] + formeChange,
						1,
						255
					);
			...
			for (statid in prevoSpecies.baseStats) {
				const change =
					newSpecies.baseStats[statid] - prevoSpecies.baseStats[statid];
				newSpecies.baseStats[statid] = this.clampIntRange(
					newSpecies.baseStats[statid] + change,
					1,
					255
				);
				newSpecies.bst += newSpecies.baseStats[statid];
	...
	forceofthefallenmod: {
		...
		onValidateSet(set, format, setHas, teamHas) {
			const lastMoveslot = this.dex.moves.get(
				set.moves[set.moves.length - 1]
			);
			if (this.ruleTable.isRestricted(`move:${lastMoveslot.id}`)) {
				return [
					`${set.species}'s move ${lastMoveslot.name} cannot be placed in the last moveslot.`,
				];
			}
		},
		onBegin() {
			this.add(
				"rule",
				"Force of the Fallen Mod: Pok&\u00e9mon pass the move in their last moveslot to their allies when they're KOed"
			);
			for (const pokemon of this.getAllPokemon()) {
				pokemon.m.trueLastMoveSlot =
					pokemon.baseMoveSlots[pokemon.baseMoveSlots.length - 1];
			}
		},
		onFaint(target) {
			const allies = target.side.pokemon.filter(
				(ally) => ally && target !== ally
			);
			for (const ally of allies) {
				ally.moveSlots = (ally as any).baseMoveSlots = [
					...ally.baseMoveSlots,
					target.m.trueLastMoveSlot,
				];
			}
		},
	},
	categoryswapmod: {
		...
		onBegin() {
			this.add(
				"rule",
				"Category Swap Mod: All physical moves become special, and vice versa"
			);
		},
		onModifyMove(move, pokemon, target) {
				...
				case "doomdesire": {
					move.onTry = function (source, subtarget) {
						if (!subtarget.side.addSlotCondition(subtarget, "futuremove"))
							return false;
						Object.assign(
							subtarget.side.slotConditions[subtarget.position][
								"futuremove"
							],
							{
								move: "doomdesire",
	...
	godlygiftmod: {
		...
		onValidateTeam(team) {
			...
			for (const set of team) {
				...
				if (
					(species.baseSpecies === "Zamazenta" &&
						this.toID(set.item) === "rustedshield") ||
					(species.baseSpecies === "Zacian" &&
						this.toID(set.item) === "rustedsword")
				) {
		...
		onModifySpecies(species, target, source) {
			...
			const god =
				target.side.team.find((set) => {
					...
					if (set.item) {
						...
						if (
							["Zacian", "Zamazenta"].includes(godSpecies.baseSpecies) &&
							item.id.startsWith("rusted")
						) {
							godSpecies = this.dex.species.get(
								set.species + "-Crowned"
							);
						}
					}
			...
			if (this.gen === 1 && (stat === "spa" || stat === "spd")) {
				newSpecies.baseStats["spa"] = newSpecies.baseStats["spd"] =
					godSpecies.baseStats[stat];
			}
			newSpecies.bst += newSpecies.baseStats[stat];
			return newSpecies;
		},
	...
	voltturnmayhemmod: {
		...
		onBegin() {
			this.add(
				"rule",
				"VoltTurn Mayhem Mod: Every move that targets a foe causes the user to switch out after use"
			);
		},
	...
	convergencelegality: {
		...
		onValidateSet(set, format) {
			...
			const matchingSpecies = this.dex.species
				.all()
				.filter(
					(species) =>
						...
						species.types.every((type) =>
							curSpecies.types.includes(type)
						) &&
			...
			if (!obtainableAbilityPool.has(this.toID(set.ability))) {
				return [
					`${curSpecies.name} doesn't have access to ${
						this.dex.abilities.get(set.ability).name
					}.`,
				];
			}
		},
			...
			const matchingSpecies = this.dex.species
				...
				.filter(
					(s) =>
						(!s.isNonstandard ||
							this.ruleTable.has(
								`+pokemontag:${this.toID(s.isNonstandard)}`
							)) &&
						s.types.every((type) => species.types.includes(type)) &&
						s.types.length === species.types.length &&
						!this.ruleTable.isBannedSpecies(s)
				);
			const someCanLearn = matchingSpecies.some(
				(s) => this.checkCanLearn(move, s, setSources, set) === null
			);
		...
		onChangeSet(set, format, setHas, teamHas) {
			...
			if (
				(species.natDexTier === "Illegal" ||
					species.forme.includes("Totem")) &&
				!["Floette-Eternal", "Greninja-Ash", "Xerneas-Neutral"].includes(
					species.name
				) &&
				!this.ruleTable.has(`+pokemon:${species.id}`)
			) {
			...
			const problemPokemon = this.dex.species
				.all()
				.filter(
					(s) =>
						(s.name === "Xerneas" ||
						...
						!(
							s.isMega ||
							s.isPrimal ||
							["Greninja-Ash", "Necrozma-Ultra"].includes(s.name)
						) &&
						!(
							this.ruleTable.has(`+pokemon:${s.id}`) ||
							this.ruleTable.has(
								`+basepokemon:${this.toID(s.baseSpecies)}`
							)
						)
				);
			if (problemPokemon.includes(species)) {
				if (
					...
					this.toID(set.item) !== this.toID(species.requiredItem)
				) {
					return [
						`${
							set.name ? `${set.name} (${species.name})` : species.name
						} is required to hold ${species.requiredItem}.`,
					];
				}
				if (
					...
					!set.moves
						.map(this.toID)
						.includes(this.toID(species.requiredMove))
				) {
					return [
						`${
							set.name ? `${set.name} (${species.name})` : species.name
						} is required to have ${species.requiredMove}.`,
					];
				}
				set.species = (
					species.id === "xerneas"
						? "Xerneas-Neutral"
						: species.id === "zygardecomplete"
						? "Zygarde"
						: species.battleOnly
				) as string;
			...
			if (
				...
				!this.ruleTable.has(`+item:${item.id}`)
			) {
				return [`${item.name} is illegal.`];
			}
			if (
				species.baseSpecies === "Xerneas" &&
				this.toID(set.ability) !== "fairyaura"
			) {
				return [
					`${
						set.name ? `${set.name} (${species.name})` : species.name
					} is ability-locked into Fairy Aura.`,
				];
			}
		},
	},
	speciesrevealclause: {
		...
		onBegin() {
			this.add(
				"rule",
				"Species Reveal Clause: Reveals a Pok\u00e9mon's true species in hackmons-based metagames."
			);
		},
	},
	franticfusionsmod: {
		...
		onBegin() {
			this.add(
				"rule",
				"Frantic Fusions Mod: Pok\u00e9mon nicknamed after another Pok\u00e9mon get buffed stats and more abilities."
			);
		},
		onValidateSet(set) {
			const species = this.dex.species.get(set.species);
			...
			if (!abilityPool.has(ability.name)) {
				return [
					`${
						species.name
					} only has access to the following abilities: ${Array.from(
						abilityPool
					).join(", ")}.`,
				];
			}
		...
		onValidateTeam(team, format) {
			...
			for (const [fusionName, number] of donors) {
				if (number > 1) {
					return [
						`You can only fuse with any Pok\u00e9 once.`,
				...
				if (this.ruleTable.isBannedSpecies(fusion) || fusion.battleOnly) {
					return [
						`Pok\u00e9mon can't fuse with banned Pok\u00e9mon.`,
						`(${fusionName} is banned.)`,
					];
				}
				if (
					fusion.isNonstandard &&
					!(
						this.ruleTable.has(
							`+pokemontag:${this.toID(fusion.isNonstandard)}`
						) ||
						this.ruleTable.has(`+pokemon:${fusion.id}`) ||
						this.ruleTable.has(
							`+basepokemon:${this.toID(fusion.baseSpecies)}`
						)
					)
				) {
					return [
						`${fusion.name} is marked as ${fusion.isNonstandard}, which is banned.`,
					];
				}
			}
		},
		onModifySpecies(species, target, source, effect) {
			...
			const fusionSpecies = this.dex.deepClone(
				this.dex.species.get(fusionName)
			);
			newSpecies.bst = newSpecies.baseStats.hp;
			for (const stat in newSpecies.baseStats) {
				if (stat === "hp") continue;
	...
	proteanpalacemod: {
		...
		onBegin() {
			this.add(
				"rule",
				"Protean Palace Mod: Every Pok\u00e9mon innately has Protean."
			);
		},
		onSwitchIn(pokemon) {
