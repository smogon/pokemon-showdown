...
export default class TeamGenerator {
	...
	constructor(format: Format | string, seed: PRNG | PRNGSeed | null) {
		...
		this.itemPool = this.dex.items
			.all()
			.filter(
				(i) => i.exists && i.isNonstandard !== "Past" && !i.isPokeball
			);
	...
	getTeam(): PokemonSet[] {
		...
		while (team.length < this.teamSize && speciesPool.length) {
			...
			const haveRoomToReject =
				speciesPool.length >= this.teamSize - team.length;
			...
			speciesPool = speciesPool.filter(
				(s) => s.baseSpecies !== species.baseSpecies
			);
	...
	protected makeSet(species: Species, teamStats: TeamStats): PokemonSet {
		...
		const abilityWeights = abilityPool.map((a) =>
			this.getAbilityWeight(this.dex.abilities.get(a))
		);
		...
		if (learnset) {
			movePool = Object.keys(learnset).filter((moveid) =>
				learnset![moveid].find((learned) => learned.startsWith("9"))
			);
		}
			...
			for (const moveid in learnset) {
				if (
					...
					learnset[moveid].some(
						(source) => source.startsWith("9") && !evoRegion
					)
				) {
					movePool.push(moveid);
				}
		...
		const numberOfMovesToConsider = Math.min(
			movePool.length,
			Math.max(15, Math.trunc(movePool.length * 0.3))
		);
		...
		while (moves.length < 4 && movePool.length) {
			...
			if (!movePoolIsTrimmed) {
				...
				for (const move of movePool) {
					const weight = this.getMoveWeight(
			...
			const moveID = this.weightedRandomPick(movePool, weights, {
				remove: true,
			});
		...
		if (species.requiredItem) {
			...
			item = this.prng.sample(
				species.requiredItems.filter(
					(i) => !this.dex.items.get(i).isNonstandard
				)
			);
		} else if (moves.every((m) => m.id !== "acrobatics")) {
			// Don't assign an item if the set includes Acrobatics...
			const weights = [];
			const items = [];
		...
		const ivs: PokemonSet["ivs"] = {
			hp: 31,
			atk: moves.some(
				(move) => this.dex.moves.get(move).category === "Physical"
			)
				? 31
				: 0,
			def: 31,
		...
		let teraType;
		const nonStatusMoves = moves.filter(
			(move) => this.dex.moves.get(move).category !== "Status"
		);
		if (!moves.some((m) => m.id === "terablast") && nonStatusMoves.length) {
			teraType = this.prng.sample(
				nonStatusMoves.map((move) => this.dex.moves.get(move).type)
			);
		} else {
	...
	protected speciesIsGoodFit(species: Species, stats: TeamStats): boolean {
		...
		for (const type of this.dex.types.all()) {
			const effectiveness = this.dex.getEffectiveness(
				type.name,
				species.types
			);
			if (effectiveness === 1) {
	...
	protected getMoveWeight(
		...
		if (move.category === "Status") {
			...
			const isHazard = (m: Move) =>
				m.sideCondition && m.target === "foeSide";
			...
			if (
				species.baseStats.def >= 100 ||
				species.baseStats.spd >= 100 ||
				species.baseStats.hp >= 100
			) {
			...
			if (move.id in HARDCODED_MOVE_WEIGHTS)
				weight *= HARDCODED_MOVE_WEIGHTS[move.id];
			...
			const goodAttacker =
				species.baseStats.atk > 80 || species.baseStats.spa > 80;
			if (
				goodAttacker &&
				movesSoFar.filter((m) => m.category !== "Status").length < 2
			) {
				weight *= 0.3;
			}
		...
		if (SPEED_BASED_MOVES.includes(move.id))
		...
		const hasSpecialSetup = movesSoFar.some(
			(m) => m.boosts?.spa || m.self?.boosts?.spa || m.selfBoost?.boosts?.spa
		);
		const hasPhysicalSetup = movesSoFar.some(
			(m) => m.boosts?.atk || m.self?.boosts?.atk || m.selfBoost?.boosts?.atk
		);
		...
		if (move.secondary?.status) {
			weight *= TeamGenerator.statusWeight(
				move.secondary.status,
				(move.secondary.chance || 100) / 100
			);
		}
		...
		for (const { chance, boosts } of [
			...
			if (boosts.def)
				weight +=
					...
					0.5 *
					boosts.def *
					(species.baseStats.def > 75 ? 1 : 0.5);
			if (boosts.spd)
				weight +=
					(chance || 1) *
					0.5 *
					boosts.spd *
					(species.baseStats.spd > 75 ? 1 : 0.5);
			...
			if (boosts.spe)
				weight +=
					...
					0.5 *
					boosts.spe *
					(species.baseStats.spe > 120 ? 0.5 : 1);
		}
	...
	protected opponentDebuffWeight(move: Move): number {
		if (
			!["allAdjacentFoes", "allAdjacent", "foeSide", "normal"].includes(
				move.target
			)
		)
			return 1;
	...
	protected getItemWeight(
		...
		switch (item.id) {
			...
			case "focussash":
				...
				// frail
				if (
					species.baseStats.hp < 80 &&
					species.baseStats.def < 80 &&
					species.baseStats.spd < 80
				)
					return 35;
				return 10;
			case "heavydutyboots":
			...
			case "flameorb":
				weight =
					ability === "Guts" && !species.types.includes("Fire") ? 30 : 0;
			...
			case "throatspray":
				if (
					moves.some((m) => m.flags.sound) &&
					moves.some((m) => m.category === "Special")
				)
					return 30;
				return 0;
