...
export const Moves: {[k: string]: ModdedMoveData} = {
		...
		condition: {
			...
			onBeforeMove(pokemon, target, move) {
				if (this.effectData.duration === 1) {
					...
					this.move.tryMoveHit(target, pokemon, moveData);
					return false;
				}
	...
	highjumpkick: {
		...
		onMoveFail(target, source, move) {
			if (target.runImmunity('Fighting')) {
				const damage = this.move.getDamage(source, target, move, true);
	...
	mirrormove: {
		...
		onHit(pokemon) {
			...
			this.move.useMove(lastMove, pokemon);
		},
		noSketch: true,
	},
	moonlight: {
	...
	sleeptalk: {
		...
		onHit(pokemon) {
			...
			this.move.useMove(randomMove, pokemon);
		},
		noSketch: true,
	},
	solarbeam: {
