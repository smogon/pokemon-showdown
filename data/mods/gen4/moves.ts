export const Moves: {[k: string]: ModdedMoveData} = {
	...
	doomdesire: {
		...
		onTry(source, target) {
			...
			const damage = this.move.getDamage(source, target, moveData, true);
	...
	highjumpkick: {
		...
		onMoveFail(target, source, move) {
			...
			let damage = this.move.getDamage(source, target, move, true);
	...
	magiccoat: {
		...
		condition: {
			...
			onTryHit(target, source, move) {
				...
				this.move.useMove(newMove, target, source);
				return null;
			},
		},
	},
	...
	mirrormove: {
		...
		onHit(pokemon) {
			...
			this.move.useMove(lastAttackedBy.move, pokemon);
		},
		target: "self",
	},
	moonlight: {
	...
	naturepower: {
		inherit: true,
		onHit(pokemon) {
			this.move.useMove('triattack', pokemon);
		},
	},
	odorsleuth: {
		inherit: true,
