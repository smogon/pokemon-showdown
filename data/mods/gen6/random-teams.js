...
class RandomGen6Teams extends RandomTeams {
	...
	randomSet(template, slot, teamDetails = {}) {
		...
		do {
			...
			for (const [i, setMoveid] of moves.entries()) {
				...
				case 'protect':
					...
					break;
				case 'pursuit':
					if (counter.setupType || hasMove['nightslash'] || (hasMove['knockoff'] && !hasType['Dark']) || (hasMove['rest'] && hasMove['sleeptalk'])) rejected = true;
					break;
				case 'rapidspin':
		...
		} else if (slot === 0 && ability !== 'Regenerator' && ability !== 'Sturdy' && !counter['recoil'] && !counter['recovery'] && template.baseStats.hp + template.baseStats.def + template.baseStats.spd < 250) {
			item = 'Focus Sash';
