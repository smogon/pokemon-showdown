...
export const Moves: {[k: string]: ModdedMoveData} = {
		...
		condition: {
			...
			onBeforeMove(pokemon, t, move) {
				if (this.effectData.duration === 1) {
					...
					this.move.moveHit(target, pokemon, move, {damage: this.effectData.totalDamage * 2} as ActiveMove);
					return false;
				}
	...
	mirrormove: {
		...
		onHit(pokemon) {
			...
			this.move.useMove(foe.lastMove.id, pokemon);
		},
	},
	mist: {
		inherit: true,
	...
	substitute: {
		...
		condition: {
			...
			onTryHit(target, source, move) {
				...
				let uncappedDamage = move.hit > 1 ? source.lastDamage : this.move.getDamage(source, target, move);
