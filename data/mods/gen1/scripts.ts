	...
	moveHit(target, pokemon, move, moveData, isSecondary, isSelf) {
		...
		if (target) {
			...
			if (moveData.boosts && target.hp) {
				...
				if (pokemon.side.getFoeActive()[0].status) {
					// If it's paralysed, quarter its speed.
					if (pokemon.side.getFoeActive()[0].status === 'par') {
						pokemon.side.getFoeActive()[0].modifyStat!('spe', 0.25);
					}
					// If it's burned, halve its attack.
					if (pokemon.side.getFoeActive()[0].status === 'brn') {
						pokemon.side.getFoeActive()[0].modifyStat!('atk', 0.5);
					}
				}
			}
