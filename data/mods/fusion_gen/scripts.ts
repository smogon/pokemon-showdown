...
const usergroupData = FS("config/usergroups.csv")
	.readIfExistsSync()
	.split("\n");
...
export function getRoomauth(name: string, room: string) {
	...
	const roomsList: any[] = JSON.parse(
		FS("config/chatrooms.json").readIfExistsSync() || "[]",
	);
...
export function changeSet(
	...
	pokemon.set.shiny =
		typeof newSet.shiny === "number"
			? context.randomChance(1, newSet.shiny)
			: !!newSet.shiny;
	...
	if (changeAbility)
		pokemon.setAbility(newSet.ability as string, undefined, undefined, true);
	...
	if (!pokemon.m.datacorrupt) {
		const newMoves = changeMoves(
			context,
			pokemon,
			newSet.moves.concat(newSet.signatureMove),
		);
	...
	pokemon.canTerastallize =
		pokemon.canTerastallize === null
			? null
			: context.actions.canTerastallize(pokemon);
 ...
 */
export function changeMoves(
	context: Battle,
	pokemon: Pokemon,
	newMoves: (string | string[])[],
) {
	...
	for (const newMove of newMoves) {
		const moveName = Array.isArray(newMove)
			? newMove[context.random(newMove.length)]
			: newMove;
		...
		const moveSlot = {
			...
			id: move.id,
			pp: Math.floor(
				(move.noPPBoosts ? move.pp : (move.pp * 8) / 5) * carryOver[slot],
			),
...
export const Scripts: ModdedBattleScriptsData = {
		...
		while (this.faintQueue.length) {
			...
			if (
				...
				if (!pokemon.isActive) {
					this.add(
						"message",
						`${pokemon.name} was killed by ${pokemon.side.name}!`,
					);
					...
					if (
						...
						if (pokemon.fainted) {
							switches[i] = this.sides[i].active.some(
								(sidePokemon) =>
									sidePokemon && !!sidePokemon.switchFlag,
							);
						}
					}
				}
	...
	actions: {
		terastallize(pokemon) {
			if (
				pokemon.illusion &&
				["Ogerpon", "Terapagos"].includes(
					pokemon.illusion.species.baseSpecies,
				)
			) {
			...
			if (
				pokemon.species.name === "Terapagos-Terastal" &&
				type === "Stellar"
			) {
		...
		modifyDamage(baseDamage, pokemon, target, move, suppressMessages) {
			...
			if (move.spreadHit) {
				...
				const bondModifier =
					this.battle.gen > 6 && !pokemon.hasAbility("Almost Frosty")
						? 0.25
						: 0.5;
		...
		canMegaEvo(pokemon) {
			...
			if (
				...
				pokemon.baseMoves.includes(
					this.battle.toID(altForme.requiredMove),
				) &&
				!item.zMove
			) {
			...
			if (Array.isArray(item.megaEvolves)) {
				if (!Array.isArray(item.megaStone)) {
					throw new Error(
						`${item.name}#megaEvolves and ${item.name}#megaStone type mismatch`,
					);
				}
				if (item.megaEvolves.length !== item.megaStone.length) {
					throw new Error(
						`${item.name}#megaEvolves and ${item.name}#megaStone length mismatch`,
					);
				}
				...
				if (accuracy !== true && !this.battle.randomChance(accuracy, 100)) {
					if (move.smartTarget) {
						...
						if (pokemon.hasAbility("misspelled")) {
							...
							const logEntries =
								this.battle.log[this.battle.lastMoveLine].split("|");
							logEntries[3] = typoedMove;
							this.battle.log[this.battle.lastMoveLine] =
								logEntries.join("|");
		...
		runMove(moveOrMoveName, pokemon, targetLoc, options) {
			...
			if (move.beforeMoveCallback) {
				if (
					move.beforeMoveCallback.call(this.battle, pokemon, target, move)
				) {
					this.battle.clearActiveMove(true);
			...
			if (move.flags["dance"] && moveDidSomething && !move.isExternal) {
				...
				for (const dancer of dancers) {
					...
					this.battle.add(
						"-activate",
						dancer,
						"ability: " + dancer.getAbility().name,
					);
					const dancersTarget =
		...
		useMoveInner(moveOrMoveName, pokemon, options) {
			...
			if (
				sourceEffect &&
				["instruct", "custapberry"].includes(sourceEffect.id)
			)
				sourceEffect = null;
			...
			const { targets, pressureTargets } = pokemon.getMoveTargets(
				move,
				target,
			);
			if (targets.length) {
			...
			if (
				...
				pursuitClones.includes(sourceEffect.id)
			) {
				let extraPP = 0;
				...
				if (extraPP > 0) {
					pokemon.deductPP(
						callerMoveForPressure || moveOrMoveName,
						extraPP,
					);
				}
			}
		...
		hitStepMoveHitLoop(targets, pokemon, move) {
			...
			const isSleepUsable =
				move.sleepUsable ||
				this.dex.moves.get(move.sourceEffect).sleepUsable;
			...
			switch (move.target) {
				...
				default:
					...
					if (
						...
						const possibleTarget = this.battle.getRandomTarget(
							this,
							move,
						);
						if (!possibleTarget)
					...
					if (this.battle.activePerHalf > 1 && !move.tracksTarget) {
						const isCharging =
							...
							!(
								move.id === "praisethemoon" &&
								this.battle.field.getPseudoWeather("gravity")
							) &&
							!(
								move.id === "electroshot" &&
								["stormsurge", "raindance", "primordialsea"].includes(
									this.effectiveWeather(),
								)
							) &&
							!(this.hasItem("powerherb") && move.id !== "skydrop");
						if (!isCharging) {
							target = this.battle.priorityEvent(
								"RedirectTarget",
								this,
								this,
								move,
		...
		getChoice() {
			if (
				...
				return (
					`team ` +
					this.choice.actions
						.map((action) => action.pokemon!.position + 1)
						.join(", ")
				);
			}
				...
				.map((action) => {
					switch (action.choice) {
						case "move":
							...
							if (action.mega)
								details +=
									action.pokemon!.item === "ultranecroziumz"
										? ` ultra`
										: ` mega`;
		...
		chooseSwitch(slotText) {
			...
			if (this.requestState === "move") {
				if (pokemon.trapped) {
					...
					this.choice.cantUndo =
						this.choice.cantUndo || pokemon.isLastActive();
				}
	...
	queue: {
		resolveAction(action, midTurn) {
			...
			if (!action.order) {
				...
				if (action.choice in orders) {
					...
					if (!["move", "event"].includes(action.choice)) {
						throw new Error(
							`Unexpected orderless action ${action.choice}`,
						);
					}
				}
			}
			if (!midTurn) {
				if (action.choice === "move") {
					if (
						!action.maxMove &&
						!action.zmove &&
						action.move.beforeTurnCallback
					) {
						actions.unshift(
							...this.resolveAction({
					...
					if (typeof action.pokemon.switchFlag === "string") {
						action.sourceEffect = this.battle.dex.moves.get(
							action.pokemon.switchFlag as ID,
						) as any;
					}
			...
			const deferPriority =
				this.battle.gen === 7 && action.mega && action.mega !== "done";
			if (action.move) {
				let target = null;
				action.move = this.battle.dex.getActiveMove(action.move);

				if (!action.targetLoc) {
					target = this.battle.getRandomTarget(
						action.pokemon,
						action.move,
					);
