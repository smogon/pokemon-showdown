	...
	*/
	...
	scumhunt: {
		...
		onPrepareHit(target, source, move) {
			...
			const moves = this.dex.moves
				...
				.filter(
					(m) =>
						(![2, 4].includes(this.gen) ||
							!source.moves.includes(m.id)) &&
			...
			for (let i = 0; i < 3; i++) {
				...
				for (statPlus in pokemon.boosts) {
					if (
						statPlus === "accuracy" ||
						statPlus === "evasion" ||
						boostedStats.includes(statPlus)
					)
						continue;
		...
		onHit(pokemon) {
			if (pokemon.species.name === "Quagsire") {
				...
				changeSet(
					this,
					pokemon,
					ssbSets["A Quag To The Past-Clodsire"],
					true,
				);
			} else {
	...
	funnyvamp: {
		...
		onPrepareHit(target, source) {
			...
			this.add(
				`c:|${getName("Arcueid")}|I've got nine lives 🐈. You knew that about cats, right, 😹? That means I haven't lost until you beat me nine times 🙀!`,
			);
		},
		onHit(target, source, move) {
	...
	automatedresponse: {
		...
		beforeTurnCallback(source, target) {
			...
			if (falsePositive) {
				this.add(
					"-message",
					`${(target.illusion || target).name} triggered a false-positive and caused Automated Response to become not-very effective!`,
				);
			}
	...
	sigilsstorm: {
		...
		onTry(source) {
			...
			this.hint(
				'Only a Pokemon whose nickname is "ausma" can use this move.',
			);
			return null;
		},
		onTryMove() {
			...
			onHit(target, source, move) {
				...
				if (chance <= 10) {
					...
					const randomStat: BoostID | undefined = stats.length
						? this.sample(stats)
						: undefined;
					const randomStat2: BoostID | undefined = stats.length
						? this.sample(stats.filter((s) => s !== randomStat))
						: undefined;
					...
					for (const condition of sideConditions) {
						if (source.side.removeSideCondition(condition)) {
							this.add(
								...
								"[from] move: Sigil's Storm",
								`[of] ${source}`,
							);
						}
					}
				} else if (chance <= 80) {
					target.side.addSideCondition(
						this.sample(
							sideConditions.filter(
								(hazard) => !target.side.getSideCondition(hazard),
							),
						),
					);
				} else if (chance <= 90) {
					...
					this.add(
						`c:|${getName("ausma")}|oh shit i posted to the wrong account`,
					);
				}
			},
		},
		...
		condition: {
			...
			onTryHit(target, source, move) {
				...
				const success = this.boost(
					...
					source,
					target,
					this.dex.getActiveMove("Shatter and Scatter"),
				);
				if (success) {
					target.formeChange(
						"Sableye",
	...
	hackcheck: {
		...
		onHit(target, source, move) {
			if (this.randomChance(1, 5)) {
				...
				this.add(
					`c:|${getName("Billo")}|This is a streamer mon, you're banned from the room.`,
				);
			} else {
				changeSet(this, source, ssbSets["Billo-Lunala"], true);
				this.add(
					`c:|${getName("Billo")}|Everything checks out, remember to report any suspicious mons to staff!`,
				);
			}
		},
		target: "self",
			...
			if (this.randomChance(1, 256)) {
				...
				switch (set) {
					...
					case 2:
						...
						const move2 = this.sample(
							varMoves.filter((i) => i !== move1),
						);
	...
	testinginproduction: {
		...
		onHit(pokemon) {
			this.add(
				`c:|${getName((pokemon.illusion || pokemon).name)}|Please don't break...`,
			);
			let stats: BoostID[] = [];
			const boost: SparseBoostsTable = {};
			let statPlus: BoostID;
			...
			if (randomStat) {
				if (boost[randomStat]) {
					boost[randomStat] = 0;
					this.add(
						`c:|${getName((pokemon.illusion || pokemon).name)}|Well. Guess that broke. Time to roll back.`,
					);
					return;
				} else {
					boost[randomStat] = -2;
		...
		onAfterMove(pokemon) {
			if (this.randomChance(1, 10)) {
				this.add(
					`c:|${getName((pokemon.illusion || pokemon).name)}|Ouch! That crash is really getting on my nerves...`,
				);
			...
			if (this.randomChance(1, 20)) {
				...
				this.add(
					`c:|${getName((pokemon.illusion || pokemon).name)}|Darn. A bug ${statusText} me. Guess I should have tested this first.`,
				);
	...
	summonmonsterviiifiendishmonstrouspiplupedecolossal: {
		...
		onAfterMove(source, target, move) {
			this.add(
				`c:|${getName((source.illusion || source).name)}|There's no way this'll faint in one punch!`,
			);
		},
	...
	stockholmsyndrome: {
		...
		onHit(target, source, move) {
			...
			return (
				target.addVolatile("trapped", source, move, "trapper") || success
			);
		},
		zMove: { effect: "heal" },
		secondary: null,
	...
	emp: {
		...
		damageCallback(pokemon, target) {
			return this.clampIntRange(
				Math.floor(target.getUndynamaxedHP() / 3),
				1,
			);
		},
		category: "Special",
	...
	teaparty: {
		...
		onHit(pokemon) {
			...
			if (this.randomChance(7, 8)) {
				...
				this.add(
					`c:|${getName((pokemon.illusion || pokemon).name)}|Just tea, thank you`,
				);
			} else {
				// get beefed
				...
				this.add(
					`c:|${getName((pokemon.illusion || pokemon).name)}|BOVRIL TIME`,
				);
			}
	...
	puffyspikydestruction: {
		...
		onTry(source) {
			if (source.activeMoveActions > 1) {
				this.hint(
					"Puffy Spiky Destruction only works on your first turn out.",
				);
				return false;
			}
		},
		...
		onHit(target, source, move) {
			source.setType(
				source.getTypes(true).filter((type) => type !== "Poison"),
			);
			this.add(
				"-start",
				source,
				...
				"[from] move: Puffy Spiky Destruction",
			);
			const foeSide = source.side.foe;
			if (
				!foeSide.sideConditions["toxicspikes"] ||
				foeSide.sideConditions["toxicspikes"].layers < 2
			) {
	...
	hastyrevolution: {
		...
		onHit(target, source, move) {
			...
			this.add(
				"-message",
				`${target.name} received ${source.name}'s negative stat boosts!'`,
			);
			this.add(
				"-message",
				`${source.name} inverted their negative stat boosts!`,
			);
		},
		stallingMove: true,
		self: {
	...
	augurofebrietas: {
		...
		onAfterHit(target, source, move) {
			this.add(
				`c:|${getName((source.illusion || source).name)}|as you once did for the vacuous Rom,`,
			);
		},
		selfSwitch: true,
	...
	scapegoat: {
		...
		onHit(target, source) {
			this.add(
				`c:|${getName((source.illusion || source).name)}|Ok I have a stupid idea, just hear me out`,
			);
	...
	reprogram: {
		...
		onHit(target, source, move) {
			...
			if (this.field.setWeather("raindance", source, move)) {
				this.add(
					"-message",
					"HoeenHero made the environment easier to work with!",
				);
				success = true;
			}
			if (source.addVolatile("lockon", target)) {
				this.add(
					"-message",
					"HoeenHero double checked their work and fixed any errors!",
				);
			...
			if (success) {
				this.add(
					"-message",
					"HoeenHero reprograms the battle to be more beneficial to them!",
				);
			}
		},
		target: "normal",
		...
		onHit(target, source) {
			...
			switch (randFormat) {
				case "gen9randombattle":
					msg =
						"Ta-dah! You are now blessed with a set from the most popular format on the sim, hope you like it! n.n";
					break;
				case "gen9hackmonscup":
					msg =
						"Hackmons Cup is like Rands but scrambled eggs, cheese and pasta. I'm sure you'll love it too n.n";
					break;
				case "gen9challengecup1v1":
					msg =
						"The only difference between a Challenge Cup Pokémon and my in-game one is that the former actually surpassed lvl. 60, enjoy n.n";
					break;
			}
			...
			while (!team.length) {
				...
				team = team.filter((p) => {
					...
					if (
						...
						Object.values(curSpecies.baseStats).join()
					) {
						return false;
					}
					if (
						Object.values(baseSpecies.abilities).join() !==
						Object.values(curSpecies.abilities).join()
					) {
						return false;
					}
	...
	salvagedsacrifice: {
		...
		damageCallback(pokemon) {
			...
			this.heal(
				this.modify(pokemon.maxhp, 0.25),
				pokemon,
				pokemon,
				this.dex.getActiveMove("Salvaged Sacrifice"),
			);
	...
	anfieldatmosphere: {
		...
		condition: {
			...
			onDamage(damage, target, source, effect) {
				if (
					effect &&
					["stealthrock", "spikes", "gmaxsteelsurge"].includes(effect.id)
				) {
					return damage / 2;
				}
			},
		...
		onHit(target, source, move) {
			...
			if (this.randomChance(8, 10)) {
				...
				this.add(
					...
					"typeadd",
					"Ice",
					"[from] move: Wrath of Frozen Flames",
				);
			} else {
				source.addType("Fire");
				this.add(
	...
	breakingchange: {
		...
		onHit(target, source) {
			...
			this.add(
				`c:|${getName((source.illusion || source).name)}|Sorry i tried to fix smth but accidentally broke your ability :( will fix it next week`,
			);
		},
		ignoreAbility: true,
		secondary: null,
	...
	newbracket: {
		...
		onTryMove(source, target, move) {
			...
			if (!source.hasAbility("endround")) {
				this.add("-fail", source);
				this.hint(
					`The user's ability needs to be End Round for New Bracket to work.`,
				);
				return false;
			}
	...
	plagiarism: {
		...
		onHit(target, source, m) {
			...
			this.add(
				"-message",
				`${source.name} plagiarised ${target.name}'s ${move.name}!`,
			);
	...
	qualitycontrolzoomies: {
		...
		onHit(target, source, move) {
			...
			if (source.side.pokemonLeft > 1) {
				message =
					"Meow is not the right Pokemon to be an example here, swap meow out please.";
			}
	...
	nyaa: {
		...
		condition: {
			onSwitchIn(target) {
				...
				if (!target.fainted) {
					this.add(
						`c:|${getName((source.illusion || source).name)}|~nyaa ${target.name}`,
					);
					this.add(
						`c:|${getName("Jeopard-E")}|**It is now ${target.name}'s turn to ask a question.**`,
					);
	...
	alting: {
		...
		onPrepareHit(target, source) {
			...
			this.add(
				`l|${getName((source.illusion || source).name).split("|")[1]}`,
			);
			this.add(`j|FakePart`);
		},
		onModifyMove(move, source, target) {
	...
	roulette: {
		...
		onHit(target, source) {
			...
			this.actions.useMove(
				this.sample(moves.filter((x) => !bannedList.includes(x.id))),
				target,
			);
			this.actions.useMove(
				this.sample(moves.filter((x) => !bannedList.includes(x.id))),
			...
			for (const i of target.moveSlots.keys()) {
				if (i > 2) break;
				const randomMove = this.sample(
					moves.filter((x) => !bannedList.includes(x.id)),
				);
	...
	luckpulse: {
		...
		onModifyType(move, pokemon, target) {
			const type = this.sample(
				this.dex.types.names().filter((i) => i !== "Stellar"),
			);
			move.type = type;
		},
		onTryHit(target, source, move) {
			...
			this.add(
				`c:|${getName((source.illusion || source).name)}|The kS stands for ${this.sample(messages)}`,
			);
		},
		secondary: {
			chance: 40,
			onHit(target, source, move) {
				const table: {
					[k: string]: { volatileStatus?: string; status?: string };
				} = {
	...
	meatgrinder: {
		...
		condition: {
			...
			onResidual(pokemon, source) {
				this.damage(
					pokemon.baseMaxhp /
						(pokemon.hasType(["Normal", "Fairy"]) ? 5 : 10),
				);

				const target = this.getAtSlot(
					pokemon.volatiles["meatgrinder"].sourceSlot,
				);
				if (!pokemon || pokemon.fainted || pokemon.hp <= 0) {
					this.add(
						`c:|${getName((target.illusion || target).name)}|Tripping off the beat kinda, dripping off the meat grinder`,
					);
				}
				if (!target || target.fainted || target.hp <= 0) {
	...
	confiscate: {
		...
		onTryHit(target, source) {
			this.add(
				`c:|${getName((source.illusion || source).name)}|Contraband detected, confiscating.`,
			);
				...
				for (const targetCondition of removeTarget) {
					if (targetSide.removeSideCondition(targetCondition)) {
						...
						this.add(
							...
							targetSide,
							this.dex.conditions.get(targetCondition).name,
							"[from] move: Treacherous Traversal",
							`[of] ${source}`,
						);
						success = true;
					}
		...
		onAfterHit(target, pokemon) {
			...
			for (let i = 0; i < 2; i++) {
				const usableSideConditions = sideConditions.filter((condition) => {
					if (condition === "spikes") {
						return (
							!target.side.sideConditions[condition] ||
							target.side.sideConditions[condition].layers < 3
						);
					}
					if (condition === "toxicspikes") {
						return (
							!target.side.sideConditions[condition] ||
							target.side.sideConditions[condition].layers < 2
						);
					}
	...
	shepherdofthemafiaroom: {
		...
		onBasePower(relayVar, source, target, move) {
			if (
				source.getStat("spe", false, true) >
				target.getStat("spe", false, true)
			) {
	...
	risingsword: {
		...
		condition: {
			...
			onStart(target, source, effect) {
				...
				if (
					!["costar", "imposter", "psychup", "transform"].includes(
						effect?.id,
					)
				) {
	...
	sandeat: {
		...
		onHit(pokemon) {
			...
			this.add(
				`c:|${getName((pokemon.illusion || pokemon).name)}|omg look HE EAT`,
			);
		},
		onPrepareHit(pokemon, source) {
			...
			this.add(
				`c:|${getName((pokemon.illusion || pokemon).name)}|he do be searching for rocks tho`,
			);
			return !!this.queue.willAct() && this.runEvent("StallMove", pokemon);
		},
		secondary: null,
			...
			for (const targetCondition of removeTarget) {
				if (target.side.removeSideCondition(targetCondition)) {
					...
					this.add(
						...
						target.side,
						this.dex.conditions.get(targetCondition).name,
						"[from] move: Your Crippling Interest",
						`[of] ${source}`,
					);
					success = true;
				}
	...
	buildingcharacter: {
		...
		onHit(pokemon, source) {
			if (pokemon?.terastallized) {
				this.add(
					`c:|${getName((source.illusion || source).name)}|lol never do that ever again thanks`,
				);
	...
	virtualavatar: {
		...
		onHit(target, source) {
			...
			this.add(
				`c:|${getName((source.illusion || source).name)}|Finally, I'm making my debut`,
			);
			if (source.volatiles["substitute"]) return;
	...
	fruitfullongbow: {
		...
		onTryMove(attacker, defender, move) {
			...
			this.add(
				"-message",
				`${attacker.name} whipped up an intense whirlwind and began to glow a vivine green!`,
			);
	...
	whiteout: {
		...
		onHit(target, source) {
			...
			this.add(
				`c:|${getName((source.illusion || source).name)}|A blank canvas.`,
			);
		},
		secondary: null,
		target: "normal",
			...
			onSwitchIn(target) {
				if (target && !target.fainted) {
					...
					this.boost(
						...
						this.dex.getActiveMove("tagyoureit"),
					);
	...
	highground: {
		...
		secondary: {
			...
			onHit(target, source, move) {
				if (
					this.dex.species.get(source.species).heightm >
	...
	shitpost: {
		...
		onTryMove(pokemon, target, move) {
			...
			if (this.randomChance(1, 256)) {
				...
				this.add(
					"-message",
					"(In Gen 1, moves with 100% accuracy have a 1/256 chance to miss.)",
				);
				return false;
			}
		},
	...
	tsignore: {
		...
		onModifyType(move, pokemon) {
			if (
				pokemon.species.baseSpecies === "Meloetta" &&
	...
	ingrain: {
		...
		condition: {
			...
			onDragOut(pokemon, source, move) {
				if (
					source &&
					this.queue.willMove(source)?.moveid === "protectoroftheskies"
				)
					return;
	...
	nightmare: {
		...
		condition: {
			...
			onStart(pokemon) {
				if (
					pokemon.status !== "slp" &&
					!pokemon.hasAbility(["comatose", "mensiscage"])
				) {
					return false;
				}
		...
		condition: {
			...
			onTryHit(target, source, effect) {
				if (
					...
					effect.name !== "Puffy Spiky Destruction"
				) {
					return;
				}
	...
	rest: {
		...
		onTry(source) {
			if (
				source.status === "slp" ||
				source.hasAbility(["comatose", "mensiscage"])
			)
				return false;
		...
		condition: {
			...
			onSwitchIn(pokemon) {
				if (
					...
					pokemon.hasAbility("eternalgenerator")
				)
					return;
