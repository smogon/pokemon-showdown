...
export const Abilities: import("../../../sim/dex-abilities").ModdedAbilityDataTable =
	...
	*/
		...
		fortifiedmetal: {
			shortDesc:
				"This Pokemon's weight is doubled and Attack is 1.5x when statused.",
		...
		theeminenceintheshadow: {
			...
			onBasePower(basePower, attacker, defender, move) {
				if (this.effectState.fallen) {
					...
					this.debug(
						`Supreme Overlord boost: ${powMod[this.effectState.fallen]}/25`,
					);
		...
		orchardsgift: {
			shortDesc:
				"Summons Grassy Terrain. 1.5x Sp. Atk and Sp. Def during Grassy Terrain.",
			name: "Orchard's Gift",
			onStart(pokemon) {
		...
		servedcold: {
			shortDesc:
				"This Pokemon's Defense is raised 2 stages if hit by an Ice move; Ice immunity.",
			name: "Served Cold",
		...
		quagofruin: {
			shortDesc:
				"Active Pokemon without this Ability have 0.85x Defense. Ignores abilities.",
		...
		clodofruin: {
			shortDesc:
				"Active Pokemon without this Ability have 0.85x Attack. Ignores stat changes.",
		...
		saintlybullet: {
			shortDesc:
				"Snipe Shot always has STAB and heals the user by 1/5 (or 1/4 on a crit) of its max HP.",
			name: "Saintly Bullet",
			onModifyMove(move) {
		...
		marblephantasm: {
			shortDesc:
				"Deoxys-Defense is immune to status moves/effects. Deoxys-Attack gains Fairy type.",
			...
			onStart(source) {
				if (
					source.species.name === "Deoxys-Attack" &&
					source.setType(["Psychic", "Fairy"])
				) {
					this.add(
						"-start",
						source,
						"typechange",
						source.getTypes(true).join("/"),
						"[from] ability: Marble Phantasm",
					);
				} else if (
					source.species.name === "Deoxys-Defense" &&
			...
			onTryHit(target, source, move) {
				if (
					...
					target !== source &&
					target.species.name === "Deoxys-Defense"
				) {
		...
		absorbphys: {
			shortDesc:
				"This Pokemon heals 1/4 of its max HP when hit by Normal moves; Normal immunity.",
			name: "Absorb Phys",
			onTryHit(target, source, move) {
			...
			onTryBoost(boost, target, source, effect) {
				...
				if (
					...
					this.add(
						...
						target,
						"unboost",
						"[from] ability: Supervised Learning",
						`[of] ${target}`,
					);
				}
			},
		...
		cascade: {
			shortDesc:
				"At 25% HP, transforms into a Mismagius. Sigil's Storm becomes Ghost type and doesn't charge.",
			name: "Cascade",
			onUpdate(pokemon) {
				if (
					pokemon.baseSpecies.baseSpecies !== "Hatterene" ||
					pokemon.transformed ||
					!pokemon.hp
				)
					return;
				if (
					pokemon.species.id === "mismagius" ||
					pokemon.hp > pokemon.maxhp / 4
				)
					return;
				this.add(
					`c:|${getName("ausma")}|that's it, yall mfs are about to face the wrath of Big Stall™`,
				);
				this.add(
					`c:|${getName("ausma")}|or i guess moreso Big Pult. pick your poison`,
				);
		...
		pesteringassault: {
			shortDesc:
				"Uses Knock Off, Taunt, Torment, Soak, and Confuse Ray with 33% accuracy at turn end.",
		...
		blitzofruin: {
			...
			onStart(pokemon) {
				...
				this.add(
					"-message",
					`${pokemon.name}'s Blitz of Ruin lowered the Speed of all surrounding Pokémon!`,
				);
			},
		...
		painfulexit: {
			...
			onBeforeSwitchOut(pokemon) {
				this.add(
					`c:|${getName("Breadey")}|Just kidding!! Take this KNUCKLE SANDWICH`,
				);
		...
		hacking: {
			name: "Hacking",
			shortDesc:
				"Hacks into PS and finds out if the enemy has any super effective moves.",
			onStart(pokemon) {
				const name = (pokemon.illusion || pokemon).name;
				this.add(
					`c:|${getName(name)}|One moment, please. One does not simply go into battle blind.`,
				);
				...
				if (!warnMoves.length) {
					this.add(
						`c:|${getName(name)}|Fascinating. None of your sets have any moves of interest.`,
					);
					return;
				}
				...
				this.add(
					`c:|${getName(name)}|Interesting. With that in mind, bring it!`,
				);
			},
			flags: {},
		},
		...
		acetosa: {
			shortDesc:
				"This Pokemon's moves are changed to be Grass type and have 1.2x power.",
			name: "Acetosa",
			onModifyTypePriority: 1,
			onModifyType(move, pokemon) {
				const noModifyType = [
		...
		thatshacked: {
			shortDesc:
				"Tries to inflict the foe with Torment at the end of each turn.",
			name: "That's Hacked",
			onResidual(target, source, effect) {
				...
				this.add(
					`c:|${getName((target.illusion || target).name)}|${this.sample(abilMessages)}`,
				);
		...
		meltingpoint: {
			shortDesc:
				"+2 Speed. Fire moves change user to Water type. Fire immunity.",
			name: "Melting Point",
			onTryHit(target, source, move) {
				if (target !== source && move.type === "Fire") {
					this.boost(
						...
						target,
						source,
						this.dex.abilities.get("meltingpoint"),
					);
		...
		masquerade: {
			shortDesc:
				"(Mimikyu only) The first hit is blocked: instead, takes 1/8 damage and gets +1 Atk/Spe.",
		...
		jetstream: {
			...
			onStart(source) {
				...
				this.add(
					"message",
					`Strong air currents keep Flying-types ahead of the chase!`,
				);
			},
			onAnySetWeather(target, source, weather) {
				if (
					this.field.isWeather("deltastream") &&
					!STRONG_WEATHERS.includes(weather.id)
				)
					return false;
			},
			onEnd(pokemon) {
		...
		firewall: {
			shortDesc:
				"Burns foes that attempt to use status moves on this Pokemon; Status move immunity.",
			name: "Firewall",
			onTryHit(target, source, move) {
		...
		grassyemperor: {
			shortDesc:
				"On switch-in, summons Grassy Terrain. During Grassy Terrain, Attack is 1.33x.",
			name: "Grassy Emperor",
			onStart(pokemon) {
				if (this.field.setTerrain("grassyterrain")) {
		...
		formchange: {
			shortDesc:
				">50% HP Necrozma, else Necrozma-Ultra. SpA boosts become Atk boosts and vice versa.",
			...
			onResidual(pokemon) {
				if (
					pokemon.baseSpecies.baseSpecies !== "Necrozma" ||
					pokemon.transformed ||
					!pokemon.hp
				)
					return;
				...
				if (pokemon.hp > pokemon.maxhp / 2) {
					...
					this.add(
						`c:|${getName("Dawn of Artemis")}|Good, I'm healthy again, time to swap back.`,
					);
				} else {
					if (pokemon.species.id === "necrozmaultra") return;
					this.add(
						`c:|${getName("Dawn of Artemis")}|Time for me to transform and you to witness the power of Ares now!`,
					);
					newSet += "-Ultra";
				}
				this.add("-activate", pokemon, "ability: Form Change");
				changeSet(this, pokemon, ssbSets[newSet]);
				[pokemon.boosts["atk"], pokemon.boosts["spa"]] = [
					pokemon.boosts["spa"],
					pokemon.boosts["atk"],
				];
				this.add(
				...
				this.add(
					"-message",
					`${pokemon.name} swapped its Attack and Special Attack boosts!`,
				);
			},
			flags: {},
		},
		...
		shadowartifice: {
			shortDesc:
				"Traps adjacent foes. If KOed with a move, that move's user loses an equal amount of HP.",
			name: "Shadow Artifice",
			onFoeTrapPokemon(pokemon) {
				if (
					!pokemon.hasAbility(["shadowtag", "shadowartifice"]) &&
		...
		virus: {
			shortDesc:
				"Causes hacked enemies to catch a virus that deals 145 damage over 3 turns.",
			name: "Virus",
		...
		stormsurge: {
			shortDesc:
				"On switch-in, summons rain that causes wind moves to have perfect accuracy and 1.1x Base Power.",
		...
		hogwash: {
			shortDesc:
				"Reckless; on STAB moves, also add Rock Head. On non-STAB moves, recoil is recovery.",
		...
		almostfrosty: {
			shortDesc:
				"This Pokemon's damaging moves hit twice. The second hit has its damage halved.",
			name: "Almost Frosty",
			onPrepareHit(source, target, move) {
				if (
					move.category === "Status" ||
		...
		snowballer: {
			shortDesc:
				"This Pokemon's Attack is raised 2 stages if hit by an Ice move; Ice immunity.",
			name: "Snowballer",
			onTryHitPriority: 1,
			onTryHit(target, source, move) {
				if (target !== source && move.type === "Ice") {
		...
		gamblingaddiction: {
			shortDesc:
				"When under 1/4 max HP: +1 Spe, heal to full HP, and all moves become Final Gambit.",
			name: "Gambling Addiction",
			onResidualOrder: 29,
			onResidual(pokemon) {
				if (
					!this.effectState.gamblingAddiction &&
					pokemon.hp &&
					pokemon.hp < pokemon.maxhp / 4
				) {
					this.boost({ spe: 1 });
		...
		mensiscage: {
			shortDesc:
				"Immune to status and is considered to be asleep. 30% chance to Disable when hit.",
			name: "Mensis Cage",
			onDamagingHit(damage, target, source, move) {
		...
		misspelled: {
			shortDesc:
				"Swift Swim + Special Attack 1.5x, Accuracy 0.8x. Never misses, only misspells.",
			name: "Misspelled",
		...
		hydrostaticpositivity: {
			shortDesc:
				"Sturdy + Storm Drain + Motor Drive + 1.3x accuracy of Water & Electric moves",
				...
				if (target !== source && move.type === "Water") {
					if (!this.boost({ spa: 1 })) {
						this.add(
							"-immune",
							target,
							"[from] ability: Hydrostatic Positivity",
						);
					}
					return null;
				}
		...
		illiterit: {
			...
			onTryHit(target, source, move) {
				if (target !== source && move.id.length >= 12) {
					...
					this.add(
						`c:|${getName("in the hills")}|Gee ${source.name}, maybe I should get a dictionary so I can understand what move you just used.`,
					);
					return null;
				}
			},
		...
		mimeknowsbest: {
			shortDesc:
				"When this Pokemon switches in, it uses a random screen or protect move.",
		...
		fortifyingfrost: {
			shortDesc:
				"If Snow is active, this Pokemon's Sp. Atk and Sp. Def are 1.5x.",
		...
		deserteddunes: {
			shortDesc:
				"Summons Deserted Dunes until switch-out; Sandstorm + Rock weaknesses removed.",
			...
			onAnySetWeather(target, source, weather) {
				if (
					this.field.getWeather().id === "deserteddunes" &&
					!STRONG_WEATHERS.includes(weather.id)
				)
					return false;
			},
		...
		anfield: {
			shortDesc:
				"Clears terrain/hazards/pseudo weathers. Summons Anfield Atmosphere.",
			name: "Anfield",
			onStart(target) {
				let success = false;
		...
		youllneverwalkalone: {
			shortDesc:
				"Boosts Atk, Def, SpD, and Spe by 25% under Anfield Atmosphere.",
		...
		peerpressure: {
			shortDesc:
				"All moves used while this Pokemon is on the field consume 4 PP. Sets Trump Card to 1 PP.",
			name: "Peer Pressure",
			onStart(pokemon) {
				this.add("-ability", pokemon, "Peer Pressure");
				const trumpCard = pokemon.moveSlots.find(
					(moveSlot) => moveSlot.id === "trumpcard",
				);
				if (trumpCard) {
					trumpCard.pp = 1;
				}
			...
			onResidual(source, target, effect) {
				const trumpCard = source.moveSlots.find(
					(moveSlot) => moveSlot.id === "trumpcard",
				);
				if (trumpCard) {
					trumpCard.pp = 1;
		...
		flashfreeze: {
			shortDesc:
				"Heatproof + If attacker's used offensive stat has positive stat changes, take 0.75x damage.",
			name: "Flash Freeze",
		...
		idealizedworld: {
			...
			onStart(pokemon) {
				...
				for (const targetCondition of Object.keys(target.sideConditions)) {
					if (
						target.removeSideCondition(targetCondition) &&
						displayText.includes(targetCondition)
					) {
						this.add(
							"-sideend",
							target,
							this.dex.conditions.get(targetCondition).name,
							"[from] ability: Idealized World",
							`[of] ${pokemon}`,
						);
					}
				}
				for (const sideCondition of Object.keys(
					pokemon.side.sideConditions,
				)) {
					if (
						pokemon.side.removeSideCondition(sideCondition) &&
						displayText.includes(sideCondition)
					) {
						this.add(
							"-sideend",
							pokemon.side,
		...
		lowtidehightide: {
			shortDesc:
				"Switch-in sets Gravity, immune to Water, traps Water-type foes.",
			...
			onTryHit(target, source, move) {
				if (target !== source && move.type === "Water") {
					this.add(
						"-immune",
						target,
						"[from] ability: Low Tide, High Tide",
					);
					return null;
				}
			},
		...
		powerabuse: {
			shortDesc:
				"Drought + 60% damage reduction + 20% burn after physical move.",
			name: "Power Abuse",
			onStart() {
		...
		endround: {
			...
			onStart(pokemon) {
				...
				this.add(
					"-message",
					"A new round is starting! Resetting the field...",
				);
				this.field.clearWeather();
				this.field.clearTerrain();
				for (const pseudoWeather of PSEUDO_WEATHERS) {
		...
		tatsuglare: {
			shortDesc:
				"Fur Coat + All of the user's moves use the Special Attack stat.",
			name: "TatsuGlare",
			onModifyMove(move, pokemon, target) {
		...
		therollingspheal: {
			shortDesc:
				"1.5x dmg boost for every repeated move use. Up to 5 uses. +1 Spe when use contact.",
		...
		eoncall: {
			shortDesc:
				"Changes into Latios after status move, Latias after special move.",
			...
			onAfterMove(source, target, move) {
				if (
					move.category === "Status" &&
					source.species.baseSpecies === "Latias"
				) {
					changeSet(this, source, ssbSets["MyPearl"], true);
				} else if (
					move.category === "Special" &&
					source.species.baseSpecies === "Latios"
				) {
		...
		weatherproof: {
			shortDesc:
				"Water-/Fire-type moves against this Pokemon deal damage with a halved offensive stat.",
			name: "Weatherproof",
			onSourceModifyAtkPriority: 6,
			onSourceModifyAtk(atk, attacker, defender, move) {
		...
		pranksterplus: {
			shortDesc:
				"This Pokemon's Status moves have priority raised by 1. Dark types are not immune.",
			name: "Prankster Plus",
			onModifyPriority(priority, pokemon, target, move) {
				if (move?.category === "Status") {
					return priority + 1;
		...
		adorablegrace: {
			shortDesc:
				"This Pokemon's secondary effects and certain items have their activation chance doubled.",
		...
		drifting: {
			...
			onDamagingHit(damage, target, source, move) {
				if (
					source.getAbility().flags["failskillswap"] ||
		...
		ctiershitposter: {
			shortDesc:
				"-1 Atk/SpA, +1 Def/SpD. +1 Atk/SpA/Spe, -1 Def/SpD, Mold Breaker if 420+ dmg taken.",
			...
			onDamage(damage, target, source, effect) {
				...
				if (target.set && !target.set.shiny) {
					if (target.m.damageTaken >= 420) {
						...
						if (!target.hp) {
							return this.add(
								`c:|${getName("PartMan")}|MWAHAHA NOW YOU - oh I'm dead`,
							);
						}
						this.add(
							`c:|${getName("PartMan")}|That's it. Get ready to be rapid-fire hugged.`,
						);
						target.clearBoosts();
		...
		godsmercy: {
			shortDesc:
				"Summons Grassy Terrain and cures the team's status conditions on switch-in.",
			name: "God's Mercy",
			onStart(source) {
				this.field.setTerrain("grassyterrain");
				const allies = [
		...
		hardcorehustle: {
			shortDesc:
				"Moves have 15% more power and -5% Acc for each fainted ally, up to 5 allies.",
			...
			onBasePower(basePower, attacker, defender, move) {
				if (this.effectState.fallen) {
					...
					this.debug(
						`Hardcore Hustle boost: ${powMod[this.effectState.fallen]}`,
					);
			...
			onSourceModifyAccuracy(accuracy, target, source, move) {
				if (this.effectState.fallen) {
					...
					this.debug(
						`Hardcore Hustle debuff: ${accMod[this.effectState.fallen]}`,
					);
		...
		highperformancecomputing: {
			...
			onResidual(source) {
				const type = this.sample(
					this.dex.types
						.names()
						.filter((i) => i !== "Stellar" && i !== source.getTypes()[0]),
				);
				if (source.setType(type)) {
					this.add(
						...
						"typechange",
						type,
						"[from] ability: High Performance Computing",
					);
				}
			},
		},
		...
		antipelau: {
			...
			onStart(target) {
				...
				this.actions.useMove(this.dex.getActiveMove("wish"), target, {
					target,
					sourceEffect: this.dex.abilities.get("antipelau"),
				});
			},
			flags: {},
		},
		...
		rainysaura: {
			shortDesc:
				"On switch-in, this Pokemon summons rain. Boosts all Psychic-type damage by 33%.",
			name: "Rainy's Aura",
			onStart(source) {
		...
		ultramystik: {
			shortDesc:
				"Stats 1.3x + Magic Guard + Leftovers until hit super effectively.",
		...
		monkeseemonkedo: {
			shortDesc:
				"Boosts Atk or SpA by 1 based on foe's defenses, then copies foe's Ability.",
			...
			onUpdate(pokemon) {
				...
				const possibleTargets = pokemon
					...
					.filter(
						(target) =>
							!target.getAbility().flags["notrace"] &&
							target.ability !== "noability",
					);
				...
				if (pokemon.setAbility(ability)) {
					this.add(
						...
						pokemon,
						ability,
						"[from] ability: Monke See Monke Do",
						`[of] ${target}`,
					);
				}
			},
		...
		hotpursuit: {
			...
			condition: {
				...
				onBeforeSwitchOut(pokemon) {
					...
					const moveName =
						move && move.moveid ? move.moveid.toString() : "";
		...
		youkaiofthedusk: {
			shortDesc:
				"This Pokemon's Defense is doubled and its status moves gain +1 priority.",
		...
		ancestryritual: {
			shortDesc:
				"Recoil heals. While below 50% HP, changes to Typhlosion-Hisui.",
			...
			onResidual(pokemon) {
				if (
					pokemon.baseSpecies.baseSpecies !== "Typhlosion" ||
					pokemon.transformed
				) {
					return;
				}
				if (
					...
					pokemon.species.id !== "typhlosionhisui"
				) {
		...
		magicalmysterycharge: {
			shortDesc:
				"Summons Electric Terrain upon switch-in, +1 boost to Sp. Def during Electric Terrain.",
		...
		perfectlyimperfect: {
			...
			shortDesc:
				"Fire-/Ice-type moves against this Pokemon deal damage with a halved offensive stat.",
		...
		spikesofwrath: {
			shortDesc:
				"Cheek Pouch + sets Spikes and Toxic Spikes upon getting KOed.",
			name: "Spikes of Wrath",
			onDamagingHit(damage, target, source, effect) {
				if (!target.hp) {
					const side = source.isAlly(target)
						? source.side.foe
						: source.side;
		...
		adaptiveengineering: {
			shortDesc:
				"Every turn, raises a random stat by 1 stage if the foe has more raised stats.",
		...
		icanheartheheartbeatingasone: {
			shortDesc:
				"Pixilate + Sharpness. -1 Atk upon KOing an opposing Pokemon.",
		...
		overaskedclause: {
			shortDesc:
				"Moves used by opposing Pokemon on the previous turn will always fail.",
			...
			onFoeTryMove(target, source, move) {
				if (target.volatiles["foresight"]) {
					...
					if (
						move.target === "foeSide" ||
						(move.target === "all" &&
							!targetAllExceptions.includes(move.id))
					) {
						return;
					}
		...
		thegraceofjesuschrist: {
			...
			onResidual(pokemon) {
				const plates = this.dex.items
					.all()
					.filter((item) => item.onPlate && !item.zMove);
				const item = this.sample(
					plates.filter(
						(plate) => this.toID(plate) !== this.toID(pokemon.item),
					),
				);
				pokemon.item = "";
				this.add(
					...
					pokemon,
					item,
					"[from] ability: The Grace Of Jesus Christ",
				);
				pokemon.setItem(item);
				pokemon.formeChange(
					"Arceus-" + item.onPlate!,
					this.dex.abilities.get("thegraceofjesuschrist"),
					true,
				);
			},
			flags: {},
		...
		eyesofeternity: {
			shortDesc:
				"Moves used by/against this Pokemon always hit; only damaged by attacks.",
		...
		aswesee: {
			shortDesc:
				"1x per turn: Stat gets boosted -> 75% chance to copy, 40% to raise another.",
			...
			onFoeAfterBoost(boost, target, source, effect) {
				...
				if (this.randomChance(3, 4)) {
					if (
						effect &&
						["As We See", "Mirror Herb", "Opportunist"].includes(
							effect.name,
						)
					)
						return;
			...
			onResidual(target, source, effect) {
				if (this.randomChance(4, 10) && this.effectState.triggered) {
					...
					const randomStat: BoostID | undefined = stats.length
						? this.sample(stats)
						: undefined;
		...
		concreteoverwater: {
			shortDesc:
				"Gains +1 Defense and Sp. Def before getting hit by a super effective move.",
		...
		seenoevilhearnoevilspeaknoevil: {
			shortDesc:
				"Dark immune; Cornerstone: Sound immune. Wellspring: Moves never miss. Hearthflame: 1.3x BP vs male.",
			...
			onTryHit(target, source, move) {
				if (
					target !== source &&
					move.flags["sound"] &&
					target.species.id.startsWith("ogerponcornerstone")
				) {
					if (!this.heal(target.baseMaxhp / 4)) {
						this.add(
							"-immune",
							target,
							"[from] ability: See No Evil, Hear No Evil, Speak No Evil",
						);
					}
					return null;
				}
		...
		primevalharvest: {
			shortDesc:
				"Sun: Heal 1/8 max HP, random berry if no item. Else 50% random berry if no item.",
		...
		yellowmagic: {
			shortDesc:
				"+25% HP, +1 SpA, +1 Spe, Charge, or paralyzes attacker when hit by an Electric move; Electric immunity.",
		...
		treasurebag: {
			shortDesc:
				"At the end of the turn and when top kek is used, use one Treasure Bag item in the cycle.",
				...
				onResidual(pokemon, source, effect) {
					...
					switch (currentItem) {
						case "Blast Seed":
							...
							this.add(
								"-message",
								`${pokemon.name} dug through its Treasure Bag and found a ${currentItem}!`,
							);
							if (foe) {
						...
						case "Oran Berry":
							...
							this.add(
								"-message",
								`${pokemon.name} dug through its Treasure Bag and found an ${currentItem}!`,
							);
							this.heal(
								100,
								pokemon,
								pokemon,
								this.dex.items.get("Oran Berry"),
							);
							break;
						case "Petrify Orb":
							this.add("-activate", pokemon, "ability: Treasure Bag");
							...
							if (foe?.trySetStatus("par", pokemon, this.effect)) {
								this.add(
									"-message",
									`${pokemon.name} petrified ${foe.name}`,
								);
							} else if (!foe) {
								this.add("-message", `But there was no target!`);
							} else {
						...
						case "Reviver Seed":
							...
							this.add(
								"-message",
								`${pokemon.name} dug through its Treasure Bag and found a Reviver Seed!`,
							);
				...
				onDamage(damage, pokemon, source, effect) {
					if (damage >= pokemon.hp && pokemon.m.seedActive) {
						if (!pokemon.m.reviverSeedTriggered) {
							...
							this.add(
								"-message",
								`Not a Reviver Seed, but a Reviser Seed!`,
							);
							this.add(`c:|${getName("yeet dab xd")}|An "s"?`);
							this.add("-message", `that wasn't a "v", but an "s"!`);
							this.add(
								"-message",
								`yeet dab xd burst into spontaneous laughter and fainted!`,
							);
							return damage;
						}
					}
		...
		partyup: {
			shortDesc:
				"On switch-in, this Pokemon's ability is replaced with a random teammate's ability.",
			name: "Party Up",
			onStart(target) {
				const abilities = target.side.pokemon
					.map((x) => x.getAbility())
					.filter((x) => !x.flags["notrace"]);
		...
		heightadvantage: {
			shortDesc:
				"If this Pokemon's height is more than that of the foe, -1 to foe's Attack/Sp. Atk.",
			...
			onStart(pokemon) {
				...
				for (const target of pokemon.adjacentFoes()) {
					...
					if (target.volatiles["substitute"]) {
						...
						if (
							this.dex.species.get(pokemon.species).heightm >
							this.dex.species.get(target.species).heightm
						) {
							this.boost(
								{ atk: -1, spa: -1 },
								target,
		...
		troll: {
			shortDesc:
				"Using moves that can flinch makes user move first in their priority bracket.",
			name: "Troll",
			onFractionalPriority(priority, pokemon, target, move) {
		...
		tempochange: {
			shortDesc:
				"Switches Meloetta's forme between Aria and Pirouette at the end of each turn.",
			name: "Tempo Change",
			onResidualOrder: 29,
			onResidual(pokemon) {
		...
		ninelives: {
			shortDesc:
				"Twice per battle, this Pokemon will survive a lethal hit with 1 HP remaining, regardless of HP.",
			name: "Nine Lives",
			onTryHit(pokemon, target, move) {
				if (move.ohko) {
		...
		baddreams: {
			...
			onResidual(pokemon) {
				...
				for (const target of pokemon.foes()) {
					if (
						target.status === "slp" ||
						target.hasAbility(["comatose", "mensiscage"])
					) {
		...
		dryskin: {
			...
			onWeather(target, source, effect) {
				...
				if (
					effect.id === "raindance" ||
					effect.id === "primordialsea" ||
					effect.id === "stormsurge"
				) {
					this.heal(target.baseMaxhp / 8);
				} else if (
					effect.id === "sunnyday" ||
					effect.id === "desolateland"
				) {
