	...
	*/
	...
	Beowulf: {
		...
		moves: [
			"Poison Jab",
			"X-Scissor",
			["Earthquake", "Volt Tackle", "Glacial Lance"],
		],
	...
	ciran: {
		...
		signatureMove:
			"Summon Monster VIII: Fiendish monstrous Piplupede, Colossal",
		evs: { atk: 252, spd: 4, spe: 252 },
		nature: "Jolly",
		teraType: "Poison",
		shiny: true,
	...
	Felucia: {
		...
		moves: [
			"Strength Sap",
			["Oblivion Wing", "Night Shade"],
			["Thief", "Calm Mind", "Toxic"],
		],
	...
	Frostyicelad: {
		...
		moves: [
			"Darkest Lariat",
			"Recover",
			["Dire Claw", "Meteor Mash", "Bitter Malice"],
		],
	...
	Mex: {
		...
		moves: [
			"Dragon Pulse",
			"Flash Cannon",
			["Aura Sphere", "Volt Switch", "Meteor Beam"],
		],
	...
	WarriorGallade: {
		...
		moves: [
			"Sunny Day",
			"Natural Gift",
			["Bitter Blade", "Sappy Seed", "Stored Power", "Counter"],
		],
...
export class RandomStaffBrosTeams extends RandomTeams {
	randomStaffBrosTeam(options: { inBattle?: boolean } = {}) {
		...
		if (monotype && !debug.length && !afd && !meme) {
			pool = pool.filter((x) =>
				this.dex.species.get(ssbSets[x].species).types.includes(monotype),
			);
		}
		if (global.Config?.disabledssbsets?.length) {
			pool = pool.filter(
				(x) => !global.Config.disabledssbsets.includes(this.dex.toID(x)),
			);
		}
		...
		while (pool.length && team.length < this.maxTeamSize) {
			if (depth >= 200)
				throw new Error(
					`Infinite loop in Super Staff Bros team generation.`,
				);
			depth++;
			const name = meme
				? this.sample(pool)
			...
			if (
				!moves
					.map((x) => this.dex.moves.get(x))
					.some((x) => x.category === "Physical")
			) {
				ivs.atk = 0;
			}
			...
			const set: PokemonSet = {
				...
				happiness:
					typeof ssbSet.happiness === "number" ? ssbSet.happiness : 255,
				shiny:
					typeof ssbSet.shiny === "number"
						? this.randomChance(1, ssbSet.shiny)
						: !!ssbSet.shiny,
			};
