...
export const Conditions: {
	[id: IDEntry]: ModdedConditionData & { innateName?: string };
} = {
	/*
	// Example:
	userid: {
	...
	*/
	...
	aegii: {
		...
		onStart() {
			this.add(
				`c:|${getName("aegii")}|**It is now aegii's turn to beat you down.**`,
			);
		},
		onSwitchOut(pokemon) {
			if (this.randomChance(2, 100)) {
				this.add(
					`c:|${getName("aegii")}|...right, I was saying in SSB4 to "stan loona", but this has to be changed now that we've found out that the company managing loona is shady af. I would like to amend that to "stan the individual members of loona" (or if you want, you can choose to stan any other group of your choice!)`,
				);
			} else {
	...
	aelita: {
		...
		onStart() {
			this.add(
				`c:|${getName("Aelita")}|You know, no one appreciates the work that goes into making weapons and towers.`,
			);
		},
		onSwitchOut() {
			this.add(
				`c:|${getName("Aelita")}|Gotta use this tower to change sectors, BRB.`,
			);
		},
		onFaint() {
			this.add(
				`c:|${getName("Aelita")}|Well, I hope the Lyoko Warriors are at least well equipped.`,
			);
		},
	},
	aethernum: {
		...
		onStart() {
			this.add(
				`c:|${getName("Aethernum")}|We are the Shadow Garden, and your time has come. Prepare yourself`,
			);
		},
		onSwitchOut() {
			this.add(
				`c:|${getName("Aethernum")}|Better play the side character for now, i'll wait a more favorable opportunity`,
			);
		},
		onFaint() {
			this.add(
				`c:|${getName("Aethernum")}|There are important things that i have to attend, i don't have any more time for you`,
			);
		},
	},
	akir: {
	...
	apple: {
		...
		onStart() {
			this.add(
				`c:|${getName("Apple")}|An Apple a day keeps the Opplesite mon away!`,
			);
		},
		onSwitchOut() {
	...
	appletunalamode: {
		...
		innateName: "Ripen",
		shortDesc:
			"When this Pokemon eats certain Berries, the effects are doubled.",
	...
	april: {
		...
		onStart(pokemon) {
			if (this.ruleTable.has("zmoveclause")) {
				...
				this.add(
					`c:|${getName("April")}|I said, "Do you have something against dogs?"`,
				);
			}
		},
		onSwitchOut() {
		...
		onFaint() {
			if (this.ruleTable.has("zmoveclause")) {
				...
				this.add(
					`c:|${getName("April")}|Don't get too impressed, you might lose your breath...`,
				);
			}
		},
		onTryHit() {
	...
	aqrator: {
		...
		onSwitchOut() {
			this.add(
				`c:|${getName("aQrator")}|A few Water Guns and Force Palms later, Tori and Riolu- Wait where are you going?`,
			);
		},
		onFaint() {
	...
	aquagtothepast: {
		...
		onStart() {
			this.add(
				`c:|${getName("A Quag To The Past")}|I'm coming out of my cage and I've been doing just fine`,
			);
		},
		onSwitchOut() {
	...
	archas: {
		...
		onStart() {
			this.add(
				`c:|${getName("Archas")}|We'll get over this barrier together!`,
			);
		},
		onSwitchOut() {
		...
		onFaint() {
			this.add(
				`c:|${getName("Archas")}|What would Grandfather... think of me now...`,
			);
		},
	},
	archfaewild: {
		...
		onStart() {
			this.add(
				`c:|${getName("Archfaewild")}|Ready for Initiative? Cause I'm gonna Clickity Clackity, Roll to Attackity!`,
			);
		},
		onSwitchOut() {
		...
		onFaint() {
			this.add(
				`c:|${getName("Archfaewild")}|Guess I didn't roll high enough`,
			);
		},
	},
	arcueid: {
		...
		onFaint() {
			this.add(
				`c:|${getName("Arcueid")}|change da world,,, my final message. Goodb ye`,
			);
		},
	},
	arsenal: {
	...
	artemis: {
		...
		onFoeAfterFaint(target, source, effect) {
			this.add(
				"message",
				`${source.name} was banned from Pok\u00e9mon Showdown!`,
			);
		},
	},
	arya: {
		...
		onFaint() {
			this.add(
				`c:|${getName("Arya")}|Nevermind, happy tuesday and let's pray for the 33.`,
			);
		},
		onAfterMega() {
	...
	audiino: {
		...
		onStart() {
			this.add(
				`c:|${getName("Audiino")}|anyone up for othello, scrabble, connect 4, splendor, codenames, catan, actually that's a long enough list already so don't actually take me up on all of those simultaneously`,
			);
		},
		onSwitchOut() {
	...
	ausma: {
		...
		onStart(pokemon) {
			...
			switch (this.toID(enemyStaff(pokemon))) {
				...
				case "lumari":
					this.add(
						`c:|${getName("ausma")}|we should watch the next ladybug ep after this tbh`,
					);
					break;
			}
		},
		onSwitchOut() {
			...
			this.add(
				`c:|${getName("ausma")}|oh shit i forgot to do this ${this.sample(phrases)} hang on`,
			);
		},
		onFaint() {
		...
		onFoeSwitchOut(pokemon) {
			if (
				this.effectState.foeMemory &&
				pokemon.species.name === "Fennekin"
			) {
	...
	auzbat: {
		...
		onSwitchOut() {
			this.add(
				`c:|${getName("AuzBat")}|I believe what doesn't kill you simply makes you, stranger`,
			);
		},
		onFaint() {
	...
	berry: {
		...
		onFaint() {
			this.add(
				`c:|${getName("berry")}|and all I got was this lousy t-shirt`,
			);
		},
	},
	bert122: {
		...
		onStart() {
			this.add(
				`c:|${getName("Bert122")}|*cackling laughter and gem crunching noises*`,
			);
		},
		onSwitchOut() {
			this.add(
				`c:|${getName("Bert122")}|Off to collect more shiny rocks! Hehehe!`,
			);
		},
		onFaint() {
	...
	billo: {
		...
		onStart(pokemon) {
			this.add(
				`c:|${getName("Billo")}|So where did you say you got this mon from again?`,
			);
		},
		onFaint(pokemon) {
			if (
				pokemon.species.name === "Solgaleo" &&
				!pokemon.getVolatile("perishsong")
			) {
				this.add(
					`c:|${getName("Billo")}|Bruh this is the worst hack I've ever seen...`,
				);
			} else if (pokemon.species.name === "Solgaleo") {
				this.add(
					`c:|${getName("Billo")}|@Room Owner this user needs blacklisting but I have to head to bed.`,
				);
			} else if (pokemon.species.name === "Lunala") {
				this.add(
					`c:|${getName("Billo")}|Someone take me to the hozzy please.`,
				);
			}
		},
	...
	blazeofvictory: {
		...
		onStart(pokemon) {
			this.add(
				`c:|${getName("blazeofvictory")}|blazeofvictorys in ur puter, askin u trivia questinz`,
			);
		},
		onSwitchOut() {
			this.add(
				`c:|${getName("blazeofvictory")}|I'll let you have bp... for now...`,
			);
		},
		onFaint() {
	...
	blitzuser: {
		...
		onFaint() {
			this.add(
				`c:|${getName("Blitz")}|https://www.youtube.com/watch?v=lPGipwoJiOM`,
			);
		},
	},
	breadey: {
		...
		onFoeFaint(target, source, effect) {
			if (
				...
				effect?.name === "Painful Exit"
			) {
				this.add(
					`c:|${getName("Breadey")}|Ashes to ashes, crust to crust.`,
				);
			} else {
				this.add(
					`c:|${getName("Breadey")}|Ope, someone's swallowing fishes.`,
				);
			}
		},
	...
	calmvibes: {
		...
		onSwitchOut() {
			this.add(
				`c:|${getName("calmvibes ♫")}|Here for a good time, not a long time!`,
			);
		},
		onFaint() {
	...
	cassiopeia: {
		...
		onFaint() {
			this.add(
				`c:|${getName("Cassiopeia")}|git checkout --detach HEAD && git commit -m "war crimes"`,
			);
		},
	},
	chaos: {
	...
	chris: {
		...
		onSwitchOut() {
			this.add(
				`c:|${getName("Chris")}|She thought it was the ocean, it's just the pool!`,
			);
		},
		onFaint() {
			this.add(
				`c:|${getName("Chris")}|Had me out like a light (like a light)`,
			);
		},
	},
	ciran: {
		...
		onStart() {
			this.add(
				`c:|${getName("ciran")}|Nobody expects the Spanish Inquisition!`,
			);
		},
		onSwitchOut() {
	...
	clefableuser: {
		...
		onFaint() {
			this.add(
				`c:|${getName("Clefable")}|I needed a VISA to be in Paldea, Wasn't even worth it. Bloody Brexit.`,
			);
		},
	...
	clem: {
		...
		onStart() {
			this.add(
				`c:|${getName("Clem")}|Je suis peut-être con comme une table`,
			);
		},
		onSwitchOut(pokemon) {
	...
	clouds: {
		onStart() {
			this.add(
				`c:|${getName("Clouds")}|i can feel it coming in the air tonight...`,
			);
		},
		onSwitchOut() {
		...
		onFaint() {
			this.add(
				`c:|${getName("Clouds")}|and i've been waiting for this moment for all my life`,
			);
		},
	},
	coolcodename: {
		...
		onSwitchOut() {
			this.add(
				`c:|${getName("Coolcodename")}|right, i forgot i have a skill issue`,
			);
		},
		onFaint() {
	...
	corthius: {
		...
		onFaint() {
			this.add(
				`c:|${getName("Corthius")}|Maurice, I can't "move it move it" anymore.`,
			);
		},
	},
	dawnofartemis: {
		...
		onStart(pokemon) {
			const god =
				pokemon.species.id === "necrozmaultra" ? "Ares" : "Artemis";
			this.add(
				`c:|${getName("Dawn of Artemis")}|Time for you to witness the power of ${god}!`,
			);
		},
		onSwitchOut() {
			this.add(
				`c:|${getName("Dawn of Artemis")}|You'll witness it again later.`,
			);
		},
		onFaint() {
	...
	dawoblefet: {
		...
		onStart() {
			this.add(
				`c:|${getName("DaWoblefet")}|What's going on guys? This is DaWoblefet, and welcome to Mechanics Monday.`,
			);
		},
		onSwitchOut() {
			this.add(
				`c:|${getName("DaWoblefet")}|Until next time, have a good one.`,
			);
		},
		onFaint() {
	...
	deftinwolf: {
		...
		onSwitchOut() {
			this.add(
				`c:|${getName("deftinwolf")}|I'll give you a moment to say your prayers.`,
			);
		},
		onFaint() {
	...
	dhelmiseuser: {
		...
		onStart(pokemon) {
			...
			this.add(
				"-message",
				`${pokemon.name}'s Stealth made it harder to hit!`,
			);
		...
		innateName: "Stealth",
		shortDesc:
			"Until using EMP, targeted moves are 25% less accurate, this Pokemon's moves do half damage, and this Pokemon only takes 3/4 damage.",
	...
	easyonthehills: {
		...
		onSwitchOut() {
			this.add(
				`c:|${getName("EasyOnTheHills")}|Would you rather have unlimited bacon, but no more video games, or would you rather have games, unlimited games, but no more games.`,
			);
		},
		onFaint() {
			this.add(
				`c:|${getName("EasyOnTheHills")}|__loud Dorito bag crinkling noises__`,
			);
		},
	},
	elliot: {
	...
	elly: {
		...
		onFaint(pokemon) {
			this.add(
				`c:|${getName("Elly")}|that wasn't very nice, ${enemyStaff(pokemon)}.`,
			);
		},
	},
	emboar02: {
		...
		onStart() {
			this.add(
				`c:|${getName("Emboar02")}|I'm the best fire-fighting starter!`,
			);
		},
		onSwitchOut() {
	...
	fame: {
		...
		onFaint(pokemon) {
			this.add(
				`c:|${getName("Fame")}|NOOOOOOOOOOOO! I'M A STAR! PLEASE, IM A STAR!`,
			);
		},
	},
	felucia: {
		...
		onStart() {
			this.add(
				`c:|${getName("Felucia")}|Good morning gamers! Just here to fix a few things`,
			);
		},
		onSwitchOut(pokemon) {
			this.add(
				`c:|${getName("Felucia")}|I have bots to make and chatrooms to manage...`,
			);
			if (pokemon.illusion) return;
	...
	froggeh: {
		...
		onStart(pokemon) {
			this.add(
				`c:|${getName("Froggeh")}|Hello. Froggeh the dad here. And welcome to The Happy Place!`,
			);
			switch (this.toID(enemyStaff(pokemon))) {
				case "valerian":
					this.add(
						`c:|${getName("Froggeh")}|See that frog, she is green, diggin the froggy queen!`,
					);
					break;
				case "queeni":
					this.add(
						`c:|${getName("Froggeh")}|Imagine if you will- a frog with a smol crown on her head.`,
					);
					break;
			}
		},
	...
	frostyicelad: {
		...
		onSwitchOut() {
			this.add(
				`c:|${getName("Frostyicelad")}|time to bring in the Ice types`,
			);
		},
		onFaint(pokemon) {
	...
	frozoid: {
		...
		onSwitchOut() {
			this.add(
				`c:|${getName("Frozoid")}|Wait let me finish what i was doi-`,
			);
		},
		onFaint() {
	...
	ganjafin: {
		...
		onStart() {
			this.add(
				`c:|${getName("Ganjafin")}|How's it going guys, Ganjafin here`,
			);
		},
		onSwitchOut() {
			this.add(
				`c:|${getName("Ganjafin")}|And I'll see you guys, in the next one`,
			);
		},
		onFaint() {
			this.add(
				`c:|${getName("Ganjafin")}|I knew I'd die before Silksong came out`,
			);
		},
	},
	hasteinky: {
		...
		onStart() {
			this.add(
				`c:|${getName("Haste Inky")}|Wanna see whatever weird thing I can do?`,
			);
		},
		onSwitchOut() {
			this.add(
				`c:|${getName("Haste Inky")}|Good call! I wasn't liking this situation either.`,
			);
		},
		onFaint() {
			this.add(
				`c:|${getName("Haste Inky")}| I am NOT feeling full of beans rn…`,
			);
		},
	},
	havi: {
		...
		onFaint() {
			this.add(
				`c:|${getName("havi")}|the nightmare swirls and churns unending n_n`,
			);
		},
	},
	hizo: {
		...
		onStart() {
			...
			this.add(
				`c:|${getName("HiZo")}|Why am I needed here, I was busy playing ${tier} with friends`,
			);
			this.add(`c:|${getName("HiZo")}|Did I break something again`);
		},
		onSwitchOut() {
			this.add(
				`c:|${getName("HiZo")}|This isn't my fault this time I swear`,
			);
			this.add(
				`c:|${getName("HiZo")}|Ok maybe it is but that doesn't mean you should blame me automatically`,
			);
		},
		onFaint() {
			this.add(`c:|${getName("HiZo")}|What did I do to deserve this`);
			this.add(
				`c:|${getName("HiZo")}|Actually on second thought don't answer that question`,
			);
		},
	},
	hoeenhero: {
		...
		onStart() {
			this.add(
				`c:|${getName("HoeenHero")}|Ok what did Hippopotas break now?`,
			);
		},
		onSwitchOut() {
			this.add(
				`c:|${getName("HoeenHero")}|TODO think of a switch out message later.`,
			);
		},
		onFaint() {
			this.add(
				`c:|${getName("HoeenHero")}|I should of reprogrammed the RNG to be in my favor too...`,
			);
		},
	},
	hsy: {
		...
		onFaint() {
			this.add(
				`c:|${getName("hsy")}|https://www.youtube.com/watch?v=g104OJIh9hs`,
			);
		},
	},
	hydrostaticsuser: {
		...
		onStart(pokemon) {
			this.add(
				`c:|${getName("Hydrostatics")}|Straighten your backs and get some hydration :]`,
			);
			this.add(
				"-start",
				pokemon,
		...
		onFaint(pokemon) {
			this.add(
				`c:|${getName("Hydrostatics")}|Seems like you were more hydrated than me :c`,
			);
	...
	inthehills: {
		...
		onFaint() {
			this.add(
				`c:|${getName("in the hills")}|im starting to feel kinda stupid can i please leave`,
			);
		},
	},
	ironwater: {
		...
		onFaint() {
			this.add(
				`c:|${getName("ironwater")}|I'll ban you in the next game...`,
			);
		},
	},
	irpachuza: {
		...
		onStart() {
			this.add(
				`c:|${getName("Irpachuza!")}|Hf. I never say gl because I sincerely don't want my oppo to have better luck than me in rands n.n`,
			);
		},
		onSwitchOut() {
		...
		onFaint(pokemon) {
			this.add(
				`c:|${getName("Irpachuza!")}|how DARE YOU ${pokemon.side.foe.name} ;-; n.n`,
			);
		},
	...
	j0rdy004: {
		...
		onStart(pokemon) {
			this.add(
				`c:|${getName("J0rdy004 ♫")}|Get-get-get-get, got-got-got-got`,
			);
		},
		onSwitchOut() {
			this.add(
				`c:|${getName("J0rdy004 ♫")}|I've seen footage, I stay noided`,
			);
		},
		onFaint() {
	...
	karthik: {
		...
		onStart(pokemon) {
			this.add(
				`c:|${getName("Karthik")}|>>> const staraptor = battle.player('${pokemon.side.name}').active[0]`,
			);
		},
		onSwitchOut(pokemon) {
			this.add(
				`c:|${getName("Karthik")}|>>> staraptor.heal(staraptor.baseMaxhp / 3)`,
			);
	...
	kennedy: {
		...
		onStart(target, source, effect) {
			...
			if (target.species.id === "cinderacegmax" && !target.terastallized) {
				this.add(
					...
					target,
					"typechange",
					target.getTypes(true, true).join("/"),
					"[silent]",
				);
			}
		},
		onSwitchOut() {
			this.add(
				`c:|${getName("Kennedy")}|Stream some Taylor Swift whilst I'm gone!`,
			); // TODO replace
		},
		onAnySwitchIn(pokemon) {
		...
		onSourceAfterFaint(length, target, source, effect) {
			...
			this.add(`c:|${getName("Kennedy")}|${message}`);
			if (
				source.species.id === "cinderace" &&
				this.field.pseudoWeather["anfieldatmosphere"] &&
				!source.transformed &&
				...
				source.baseMaxhp = Math.floor(
					(Math.floor(
						2 * source.species.baseStats["hp"] +
							source.set.ivs["hp"] +
							Math.floor(source.set.evs["hp"] / 4) +
							100,
					) *
						source.level) /
						100 +
				...
				const newMaxHP = source.volatiles["dynamax"]
					? 2 * source.baseMaxhp
					: source.baseMaxhp;
		...
		onUpdate(pokemon) {
			if (pokemon.volatiles["attract"]) {
				this.add(
					`c:|${getName("Kennedy")}|NAAA FUCK OFF, I'd rather be dead`,
				);
				pokemon.faint();
				this.add(
					"message",
					"Kennedy would have been infatuated but fainted mysteriously",
				);
			}
		},
	...
	kingbaruk: {
		...
		onFaint() {
			this.add(
				`c:|${getName("kingbaruk")}|Why can't we give love that one more chance?`,
			);
		},
	...
	kiwi: {
		...
		onStart() {
			this.add(
				`c:|${getName("Kiwi")}|Hey, are you a goldfish or a shark? I guess it depends on how quickly you get flushed down`,
			);
		},
		onSwitchOut() {
			this.add(
				`c:|${getName("Kiwi")}|You're lively, but I'm not done peeling off your scales`,
			);
		},
		onFaint() {
			this.add(
				`c:|${getName("Kiwi")}|Too late, the manifestation has completed. You'll be reduced to a fillet one day unexpectedly...`,
			);
		},
	},
	klmondo: {
	...
	kolohe: {
		...
		onStart(pokemon) {
			...
			if (foe === "Rumia") {
				...
				this.add(
					`c:|${getName("kolohe ✮彡")}|The weekly Smogon furry convention starts NOW`,
				);
			} else if (foe === "Peary") {
		...
		onFaint() {
			this.add(
				`c:|${getName("kolohe ✮彡")}|change da world... my final message. goodbye`,
			);
		},
	},
	kry: {
		...
		onFaint() {
			this.add(
				`c:|${getName("Kry")}|Guys whatever you do don't say Farigiraf backwards`,
			);
		},
	},
	lasen: {
		...
		onStart() {
			this.add(
				`c:|${getName("Lasen")}|That's a Hungarian yield sign, easy Budapest guess.`,
			);
		},
		onSwitchOut() {
			this.add(
				`c:|${getName("Lasen")}|Will give QC 2/2 after implementation.`,
			);
		},
		onFaint() {
	...
	letsgoshuckles: {
		...
		onStart() {
			this.add(
				`c:|${getName("Lets go shuckles")}|Behold the magnificence of Shuckle.`,
			);
		},
		onSwitchOut() {
			this.add(
				`c:|${getName("Lets go shuckles")}|Wise men don't fight battles they cannot win.`,
			);
		},
		onFaint() {
			this.add(
				`c:|${getName("Lets go shuckles")}|He who lives by the Shuckle shall die by the Shuckle.`,
			);
		},
	},
	loethalion: {
		...
		onStart(pokemon) {
			...
			if (foe === "WigglyTree") {
				this.add(
					`c:|${getName("Loethalion")}|No, I'm not drawing Dialga on a bike again.`,
				);
			} else if (foe === "Swiffix") {
				...
				this.add(
					`c:|${getName("Loethalion")}|So your saying I can't ban myself?`,
				);
			} else if (foe === "Clefable") {
				this.add(
					`c:|${getName("Loethalion")}|But what if I hack a tiny bit?`,
				);
			} else if (foe === "Lunell") {
				this.add(`c:|${getName("Loethalion")}|We bean posting?`);
			} else if (foe === "Ciran") {
				this.add(
					`c:|${getName("Loethalion")}|THE FR*CK WORD STAYS BANNED.`,
				);
			} else if (foe === "Appletun a la Mode") {
				this.add(`c:|${getName("Loethalion")}|OOOUUGGHHHH OOOUUUWWAAAA.`);
			} else if (foe === "SexyMalasada") {
				this.add(
					`c:|${getName("Loethalion")}|I forgot how to rng this mon again, what singular step did I miss?`,
				);
			} else {
		...
		onSwitchOut(pokemon) {
			this.add(
				`c:|${getName("Loethalion")}| I don't remember why I'm even here __walks out the room__`,
			);
		},
		onFaint() {
	...
	lunell: {
		...
		onSwitchOut() {
			this.add(
				`c:|${getName("Lunell")}|brb looking for bean images, don't disturb`,
			);
		},
		onFaint() {
	...
	mathy: {
		...
		onSwitchOut(pokemon) {
			this.add(
				`c:|${getName("Mathy")}|whatever i'll make ${enemyStaff(pokemon)} fix it`,
			);
		},
		onFaint() {
	...
	merritty: {
		...
		onSwitchOut() {
			this.add(
				`c:|${getName("Merritty")}|If you believe there's been a mistake, please let me know ASAP.`,
			);
		},
		onFaint() {
		...
		innateName: "Tourban",
		shortDesc:
			"Takes half damage from Ghost moves, deals double damage to Ghost-types.",
		onSourceModifyDamage(damage, source, target, move) {
			if (source.illusion) return;
			if (move.type === "Ghost") {
	...
	miojo: {
		...
		onFaint() {
			this.add(
				`c:|${getName("Miojo")}|Em caso de investigação policial, eu declaro que não tenho envolvimento com este grupo e não sei como estou no mesmo, provavelmente fui inserido por terceiros, declaro que estou disposto a colaborar com as investigações e estou disposto a me apresentar a depoimento se necessário`,
			);
		},
	},
	monkey: {
		...
		onSwitchOut() {
			this.add(
				`c:|${getName("Monkey")}|Don't mind me I was just monkeying around`,
			);
		},
		onFaint() {
			this.add(
				`c:|${getName("Monkey")}|I'm a seeker too. But my dreams aren't like yours. I can't help thinking that somewhere in the universe there has to be something better than man. Has to be.`,
			);
		},
	},
	mypearl: {
	...
	neko: {
		...
		onFaint() {
			this.add(
				`c:|${getName("Neko")}|Chien-Meow is cute when it doesn't scratch the ground, between it and Flutter Mane its dangerous to go out and ladder. You have been warned ;w;`,
			);
		},
	},
	ney: {
	...
	notater517: {
		...
		onSwitchOut() {
			this.add(
				`c:|${getName("Notater517")}|brb, dealing with mobile connection, here's a song to listen to before I return: https://www.youtube.com/watch?v=dQw4w9WgXcQ`,
			);
		},
		onFaint() {
	...
	nya: {
		...
		shortDesc:
			"This Pokemon's moves have a 30% chance to be doubled in power.",
	...
	partman: {
		...
		onStart(pokemon) {
			...
			switch (this.toID(enemyStaff(pokemon))) {
				...
				case "aqrator":
					message =
						"Speaking of cafes - this Pokemon is so popular, it has an entire cafe dedicated to it in the Pokemon world! Alongside the cafe, there's also stuff like a bus tour where you can sit one-on-one with the Pokemon and admire its beauty.";
					break;
				case "beowulf":
					message = "BEE";
					break;
				...
				case "trace":
					this.add(
						"-message",
						`PartMan's Neutralizing Gas filled the area! (but not really)`,
					);
					message = null;
					break;
				case "warriorgallade":
		...
		shortDesc:
			"Any move that does damage equal to this Pokemon's max HP fails.",
		// onDamagePriority: 1,
	...
	pastorgigas: {
		...
		onSwitchOut() {
			this.add(
				`c:|${getName("Pastor Gigas")}|I'll leave, but God stays forever`,
			);
		},
		onFaint() {
	...
	pissog: {
		...
		onFaint() {
			this.add(
				`c:|${getName("Pissog")}|Yes, there are two paths you can go by, but in the long run`,
			);
		},
	},
	pokemonvortex: {
	...
	princessautumn: {
		...
		onStart() {
			this.add(
				`c:|${getName("Princess Autumn")}|good meowning, here's why you're wrong.`,
			);
		},
		onSwitchOut(pokemon) {
			this.add(
				`c:|${getName("Princess Autumn")}|good nyight, I'm always right.`,
			);
		...
		onFaint() {
			this.add(
				`c:|${getName("Princess Autumn")}|We let TPP cook too hard...`,
			);
		},
	...
	ptoad: {
		...
		onFaint() {
			this.add(
				`c:|${getName("ptoad")}|Who told you you're allowed to rain on my parade?`,
			);
		},
	},
	pulseks: {
		...
		onFaint() {
			this.add(
				`c:|${getName("Pulse_kS")}|If my model is accurate (it isn't)`,
			);
		},
	},
	pyro: {
		...
		onSwitchOut() {
			this.add(
				`c:|${getName("PYRO")}|Operation: Lifesaver is in effect, as of right now`,
			);
		},
		onFaint() {
			this.add(
				`c:|${getName("PYRO")}|Just remember ALL CAPS when you spell the man name...`,
			);
		},
		onSourceAfterFaint(length, target, source, effect) {
			if (effect?.effectType === "Move") {
				if (effect.id === "meatgrinder") {
					this.add(
						`c:|${getName("PYRO")}|Tripping off the beat kinda, dripping off the meat grinder`,
					);
					return;
				}
				if (!source.m.msgPlayed) {
					this.add(
						`c:|${getName("PYRO")}|This Villain was a ruthless mass conqueror, with aspirations to dominate the universe`,
					);
	...
	r8: {
		...
		onFaint() {
			this.add(
				`c:|${getName("R8")}|Getting KOed won't prevent me from making propaganda: https://www.smogon.com/forums/forums/national-dex-other-tiers.738/`,
			);
		},
	},
	rainshaft: {
		...
		onStart(pokemon) {
			this.add(
				`c:|${getName("Rainshaft")}|Hello ${pokemon.side.name} and ${pokemon.side.foe.name} :P`,
			);
		},
		onSwitchOut() {
		...
		onFaint() {
			this.add(
				`c:|${getName("Rainshaft")}|You weren't lucky enough, join <<survivor>> for more practice XD`,
			);
		},
	},
	ransei: {
	...
	returntomonkey: {
		...
		onFaint() {
			this.add(
				`c:|${getName("ReturnToMonkey")}|Reject the humanity...if you dare...`,
			);
		},
	},
	riovidal: {
		...
		onStart() {
			this.add(
				`c:|${getName("Rio Vidal")}|It's not my fault you're, like, in love with me!`,
			);
		},
		onSwitchOut() {
			this.add(
				`c:|${getName("Rio Vidal")}|That's why her hair is so big. It's full of secrets.`,
			);
		},
		onFoeSwitchOut() {
			this.add(
				`c:|${getName("Rio Vidal")}|You wanna do something fun? You wanna go to Taco Bell?`,
			);
		},
		onFaint() {
			this.add(
				`c:|${getName("Rio Vidal")}|Gretchen, I'm sorry I laughed at you that time you got diarrhea at Barnes & Noble. And I'm sorry for telling everyone about it. And I'm sorry for repeating it now.`,
			);
		},
	},
	rissoux: {
	...
	rumia: {
		...
		onStart(pokemon) {
			if (enemyStaff(pokemon) === "kolohe") {
				this.add(
					`c:|${getName("Rumia")}|OMG who could that be (⁠●⁠♡⁠∀⁠♡⁠)`,
				);
			} else {
				this.add(
					`c:|${getName("Rumia")}|is the mon in front of me the edible kind?`,
				);
			}
		},
		onSwitchOut(pokemon) {
			if (enemyStaff(pokemon) === "kolohe") {
				this.add(
					`c:|${getName("Rumia")}|i cant bring myself to do this...`,
				);
			} else {
		...
		onFaint(pokemon) {
			if (enemyStaff(pokemon) === "kolohe") {
				this.add(
					`c:|${getName("Rumia")}|this is the best way to go out...`,
				);
			} else {
	...
	scotteh: {
		...
		onStart() {
			this.add(
				`c:|${getName("Scotteh")}|\`\`Compilation completed successfully. Executing...\`\``,
			);
		},
		onSwitchOut() {
			this.add(
				`c:|${getName("Scotteh")}|\`\`Execution temporarily paused.\`\``,
			);
		},
		onFaint() {
			this.add(
				`c:|${getName("Scotteh")}|\`\`Segmentation fault (core dumped)\`\``,
			);
		},
	},
	sexymalasada: {
		...
		onStart(pokemon) {
			switch (this.toID(enemyStaff(pokemon))) {
				case "wigglytree":
					this.add(
						`c:|${getName("SexyMalasada")}|Hey Wiggles! I made pizza again! Wanna learn more RNG btw?`,
					);
					break;
				case "appletunalamode":
					this.add(
						`c:|${getName("SexyMalasada")}|And now you must learn how to RNG with nothing but a sundial for a timer! __Trust me!__`,
					);
					break;
				case "loethalion":
					this.add(
						`c:|${getName("SexyMalasada")}|For the hundredth time Loe, check. the. pins.`,
					);
					break;
				case "nicolic":
					this.add(
						`c:|${getName("SexyMalasada")}|Hi Nic! Why you keep postponing learning old-gen RNG? q_q`,
					);
					break;
				case "swiffix":
					this.add(
						`c:|${getName("SexyMalasada")}|....something smells in here`,
					);
					break;
				case "mex":
					this.add(
						`c:|${getName("SexyMalasada")}|Today is the day you finally learn RNG Mex, deal with it!`,
					);
					break;
				case "clefable":
					this.add(
						`c:|${getName("SexyMalasada")}|Oi! I'm not hacking, it's RNG!`,
					);
					break;
				case "billo":
					this.add(
						`c:|${getName("SexyMalasada")}|Billo help! The tool isn't working again q_q`,
					);
					break;
				default:
					this.add(
						`c:|${getName("SexyMalasada")}|Hello! Do you have some time to talk about RNGesus and its awesome teachings: The Art of RNG abuse??`,
					);
					break;
			}
		},
		onSwitchOut(pokemon) {
			switch (this.toID(enemyStaff(pokemon))) {
				...
				case "billo":
					this.add(
						`c:|${getName("SexyMalasada")}|Fiiiine I'll read the wiki...`,
					);
					break;
				default:
					this.add(
						`c:|${getName("SexyMalasada")}|Crap! I missed my frame... Resetting... q_q`,
					);
					break;
			}
		},
		onFaint(pokemon) {
			switch (this.toID(enemyStaff(pokemon))) {
				...
				default:
					this.add(
						`c:|${getName("SexyMalasada")}|Well then.. have fun soft-resetting for your shiny! >:( Cya on the flipside 🕶️`,
					);
					break;
			}
		},
	...
	siegfried: {
		...
		onSwitchOut() {
			this.add(
				`c:|${getName("Siegfried")}|Oh, I get by with a little help from my friends.`,
			);
		},
		onFaint() {
			this.add(
				`c:|${getName("Siegfried")}|Living is easy with eyes closed.`,
			);
		},
	},
	sificon: {
		...
		onStart() {
			this.add(
				`c:|${getName("Sificon~")}|gm (it's 4pm and I woke up just now)`,
			);
		},
		onSwitchOut() {
			this.add(
				`c:|${getName("Sificon~")}|guess I'll go to bed (we all know that I won't)`,
			);
		},
		onFaint() {
	...
	skies: {
		...
		onStart() {
			this.add(
				`c:|${getName("skies")}|the baddest in the room, so tell em to make room... `,
			);
		},
		onSwitchOut() {
	...
	steorra: {
		...
		onFaint() {
			this.add(
				`c:|${getName("Steorra")}|I'm dead but not really (lol ghost)`,
			);
		},
	},
	struchni: {
	...
	sulo: {
		...
		onStart() {
			this.add(
				`c:|${getName("Sulo")}|everybody is so damn dramatic. me included.`,
			);
		},
		onSwitchOut() {
	...
	swiffix: {
		...
		onSwitchOut() {
			this.add(
				`c:|${getName("Swiffix")}|brb, gonna get some ketchup for my pizza`,
			);
		},
		onFaint() {
	...
	syrinix: {
		...
		onStart() {
			this.add(
				`c:|${getName("Syrinix")}|You kept me like a secret but I kept you like an oath`,
			);
		},
		onSwitchOut() {
			this.add(
				`c:|${getName("Syrinix")}|They don't think it be like it is, but it do`,
			);
		},
		onFaint() {
	...
	tenshi: {
		...
		onStart(pokemon) {
			switch (this.toID(enemyStaff(pokemon))) {
				...
				case "clouds":
					this.add(
						`c:|${getName("Tenshi")}|birbs cannot save u from SAND`,
					);
					break;
				default:
		...
		onFaint(pokemon) {
			switch (this.toID(enemyStaff(pokemon))) {
				...
				case "clouds":
					this.add(
						`c:|${getName("Tenshi")}|SAND is no longer in the air tonight :(`,
					);
					break;
				default:
	...
	thejesucristoosama: {
		...
		onStart() {
			this.add(
				`c:|${getName("TheJesucristoOsAma")}|In the name of the Father, the Son and the Holy Spirit. I bless you, Amen.`,
			);
		},
		onSwitchOut() {
			this.add(
				`c:|${getName("TheJesucristoOsAma")}|Oh well, I think it's time to call my apostles.`,
			);
		},
		onFaint() {
			this.add(
				`c:|${getName("TheJesucristoOsAma")}|And that's how I've died for the third time, I'll go to host a game at eventos.`,
			);
		},
	},
	traceuser: {
		...
		onFaint() {
			this.add(
				`c:|${getName("trace")}|How disappointingly short a dream lasts.`,
			);
		},
	},
	tuthur: {
		...
		onFaint() {
			this.add(
				`c:|${getName("Tuthur")}|this wouldn't have gone like this if we'd played kunc`,
			);
		},
	},
	twoofroses: {
		...
		onStart(pokemon) {
			this.add(
				`c:|${getName("Two of Roses")}|I'm here! I'm uhh- Yes! Also hi! Happy to be here.`,
			);
			this.singleEvent(
				"WeatherChange",
				this.effect,
		...
		onSwitchOut() {
			this.add(
				`c:|${getName("Two of Roses")}|Pfft! I prefer lurking anyway.`,
			);
		},
		onFaint() {
			this.add(
		...
		innateName: "Wonderer",
		shortDesc:
			"This Pokemon's secondary type changes based on the active weather or terrain, monotype if neither.",
		onWeatherChange(target, source, sourceEffect) {
			...
			if (!currentWeather && !target.hasType("Dark")) {
				...
				if (
					["raindance", "primordialsea", "stormsurge"].includes(
						currentWeather,
					) &&
					...
					["sunnyday", "desolateland"].includes(currentWeather) &&
					!target.hasType("Fire")
				) {
					type = "Fire";
				} else if (
					["sandstorm", "deserteddunes"].includes(currentWeather) &&
					!target.hasType("Rock")
				) {
					type = "Rock";
				} else if (
					["hail", "snowscape"].includes(currentWeather) &&
					!target.hasType("Ice")
				) {
					type = "Ice";
				} else {
		...
		onTerrainChange(target, source, sourceEffect) {
			...
			if (!currentTerrain && !target.hasType("Dark")) {
				...
				if (currentTerrain === "electricterrain") {
					...
					currentTerrain === "psychicterrain" &&
					!target.hasType("Psychic")
				) {
					type = "Psychic";
				} else if (
					...
					currentTerrain === "mistyterrain" &&
					!target.hasType("Fairy")
				) {
					type = "Fairy";
	...
	ut: {
		...
		onStart() {
			this.add(
				`c:|${getName("UT")}|__being this young is art, aquamarine__`,
			);
		},
		onSwitchOut() {
	...
	valerian: {
		...
		onStart() {
			this.add(
				`c:|${getName("Valerian ✿ ♡")}|Lucario's shiny should've been red.`,
			);
		},
		onSwitchOut() {
			this.add(
				`c:|${getName("Valerian ✿ ♡")}|As a wise man once said, "I'll be back".`,
			);
		},
		onFaint() {
			this.add(
				`c:|${getName("Valerian ✿ ♡")}|My name is based on a flower, NOT the movie!`,
			);
		},
	},
	venous: {
		...
		onSwitchOut() {
			this.add(
				`c:|${getName("2013 hindi room")}|when i said tine wins i didnt mean now`,
			);
		...
		shortDesc:
			"Moves gain 20 additional Base Power per each positive stat boost.",
		onModifyMove(move, pokemon, target) {
	...
	violet: {
		...
		onStart() {
			this.add(
				`c:|${getName("Vio͜͡let")}|I'm not hating you just decided to be wrong`,
			);
		},
		onSwitchOut() {
		...
		onFaint() {
			this.add(
				`c:|${getName("Vio͜͡let")}|blatantly carried by cheating but you'll still find a way to downplay`,
			);
		},
		innateName: "Do No Evil",
		shortDesc:
			"When this Pokemon uses an attacking move, it transforms into the Ogerpon form of the corresponding type.",
	...
	vistar: {
		...
		onFaint() {
			this.add(
				`c:|${getName("Vistar")}|So... this is how my career ends...`,
			);
		},
	},
	vmnunes: {
	...
	warriorgallade: {
		...
		onStart(pokemon) {
			this.add(
				`c:|${getName("WarriorGallade")}|i wanted to proc berries, but it seems that i was better at proc rastinating instead. nom nom nom.`,
			);
			if (this.toID(enemyStaff(pokemon)) === "aqrator") {
				this.add(
					`c:|${getName("WarriorGallade")}|Hi Tori, how goes your conquest?`,
				);
			}
			// innate
			if (pokemon.illusion) return;
		...
		onSwitchOut() {
			this.add(
				`c:|${getName("WarriorGallade")}|amidst this tactical retreat, you didn't think i forgot about the pokeradar, did you? you can bet that my return with even more questions will be __eventful__ :3`,
			);
		},
		onFaint() {
			this.add(
				`c:|${getName("WarriorGallade")}|a wig flew, and now i must bid you adieu. farewell my berries accrued, for this is the end of my etude.`,
			);
		},
		onSourceAfterFaint() {
			this.add(
				`c:|${getName("WarriorGallade")}|Triumphant through trouncing tough, tenacious threats today, though testing 212 takeovers tarry. Theorizing these techniques tends to torrid, terribly tiresome tabulations, therefore torrential tactics traverse thorough thoughts.`,
			);
		},
	...
	waves: {
		...
		onStart() {
			this.add(
				`c:|${getName("Waves")}|Nice opinion, one small issue: 252+ SpA Wailord Water Spout (150 BP) vs. 0 HP / 0- SpD Your Argument in Rain: 1202-1416 (413 - 486.5%) -- guaranteed OHKO.`,
			);
		},
		onSwitchOut() {
	...
	xprienzo: {
		...
		onSwitchOut() {
			this.add(
				`c:|${getName("XpRienzo ☑◡☑")}|What? You don't trust me? >.>`,
			);
		},
		onFaint() {
	...
	yeetdabxd: {
		...
		onStart(pokemon) {
			this.add(
				`c:|${getName("yeet dab xd")}|Ah, welcome~! The merchandise you have chosen will cost your soul. Is that acceptable?`,
			);
		},
		onSwitchOut() {
		...
		onFaint(pokemon) {
			...
			this.add(
				`c:|${getName("yeet dab xd")}|wait no you didn't join QW yet`,
			);
		},
	},
	yellowpaint: {
		...
		onFaint() {
			this.add(
				`c:|${getName("Yellow Paint")}|The canvas is filled with different screams.`,
			);
		},
		onModifyMove(move) {
	...
	yuki: {
		...
		onStart(source) {
			...
			this.field.setWeather(
				"snowscape",
				source,
				this.dex.abilities.get("snowwarning"),
			);
		},
	},
	yveltalnl: {
		...
		onStart(pokemon) {
			this.add(
				`c:|${getName("YveltalNL")}|It's over ${pokemon.side.foe.name}, I have the high ground!`,
			);
		},
		onSwitchOut() {
	...
	za: {
		...
		onFaint() {
			this.add(
				`c:|${getName("za")}|!track Between the Buried and Me - Sun Of Nothing - 2020 Remix / Remaster`,
			);
		},
	},
	zalm: {
		...
		onSwitchOut() {
			this.add(
				`c:|${getName("Zalm")}|brb gonna check if my lasagne didn't explode e-e`,
			);
		},
		onFaint() {
			this.add(
				`c:|${getName("Zalm")}|I should have picked an actual fish pokémon like veluza instead...`,
			);
		},
	},
	zarel: {
	...
	zee: {
		...
		onFaint() {
			this.add(
				`c:|${getName("zee")}|Hey everyone it's been a great time working with you all in this Super Staff Bros battle but I think it's the right time for me to step down. Thank you all and see you around.`,
			);
		},
	},
	zoro: {
	...
	beefed: {
		...
		onModifyMove(move, pokemon, target) {
			if (
				!target ||
				!this.checkMoveMakesContact(move, pokemon, target) ||
	...
	wish: {
		...
		onStart(pokemon, source, effect) {
			...
			if (this.effectState.startingTurn === 255) {
				this.hint(
					`In Gen 8+, Wish will never resolve when used on the ${this.turn}th turn.`,
				);
			}
		},
		onResidualOrder: 4,
