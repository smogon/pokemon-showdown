...
class RandomTeams extends Dex.ModdedDex {
	randomSeasonalPolarTeam() {
		...
		while (pokemonPool.length && pokemonLeft < 6) {
			...
			if (weakCount > 2 && types === 'Ice/Water' && this.randomChance(4, 5)) continue;

			let tier = template.tier;
			switch (tier) {
			case 'Uber':
				// Ubers are limited to 1 but have a 20% chance of being added anyway.
				if (uberCount && this.randomChance(4, 5)) continue;
				break;
			case 'PU':
			...
			if (this.getItem(set.item).megaStone) teamDetails['megaStone'] = 1;
	...
	randomSeasonalDimensionalTeam() {
		...
		if (this.takenSide) {
			side = (this.takenSide === 'good' ? 'bad' : 'good');
		} else {
	...
	randomSeasonalFireworksTeam(side) {
		...
		while (team.length < 6) {
			...
			if (template.id === 'gligar') {
				template.randomBattleMoves = ['stealthrock', 'roost', 'knockoff', this.sample(['uturn', 'toxic'])];
			}
			if (template.id === 'gliscor') {
				template.randomBattleMoves = ['stealthrock', 'protect', 'knockoff', this.sample(['toxic', 'roost'])];
			}
			if (template.id === 'landorus') {
				template.randomBattleMoves = ['sludgewave', 'knockoff', 'rockslide', this.sample(['focusblast', 'psychic'])];
			}
			...
			for (let t = 0; t < types.length; t++) {
				if (types[t] !== 'Flying' && typeCount[types[t]] > 1 && this.randomChance(4, 5)) {
					skip = true;
					break;
				}
			}
